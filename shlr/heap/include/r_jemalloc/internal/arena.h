...
arena_miscelm_to_pageind (const arena_chunk_map_misc_t *miscelm) {
	...
	size_t pageind = ((uintptr_t)miscelm - ((uintptr_t)chunk + map_misc_offset)) / sizeof (arena_chunk_map_misc_t) + map_bias;
...
arena_rd_to_miscelm (arena_runs_dirty_link_t *rd) {
	...
	if (unlikely ((arena_miscelm_to_pageind (miscelm) <= map_bias) || arena_miscelm_to_pageind (miscelm) > chunk_npages))
		return (NULL);
	return (miscelm);
}
...
arena_mapbits_dirty_get (const arena_chunk_t *chunk, size_t pageind) {
	...
	mapbits = arena_mapbits_get (chunk, pageind);
	assert ((mapbits & CHUNK_MAP_DECOMMITTED) == 0 || (mapbits & (CHUNK_MAP_DIRTY | CHUNK_MAP_UNZEROED)) == 0);
...
arena_mapbits_unallocated_set (arena_chunk_t *chunk, size_t pageind, size_t size,
	...
	assert ((flags & CHUNK_MAP_DECOMMITTED) == 0 || (flags & (CHUNK_MAP_DIRTY | CHUNK_MAP_UNZEROED)) == 0);
...
arena_mapbits_unallocated_size_set (arena_chunk_t *chunk, size_t pageind,
	...
	assert ((size & PAGE_MASK) == 0);
	assert ((mapbits & (CHUNK_MAP_LARGE | CHUNK_MAP_ALLOCATED)) == 0);
	arena_mapbitsp_write (mapbitsp, arena_mapbits_size_encode (size) | (mapbits & ~CHUNK_MAP_SIZE_MASK));
}
...
arena_mapbits_large_set (arena_chunk_t *chunk, size_t pageind, size_t size,
	...
	assert ((flags & CHUNK_MAP_FLAGS_MASK) == flags);
	assert ((flags & CHUNK_MAP_DECOMMITTED) == 0 || (flags & (CHUNK_MAP_DIRTY | CHUNK_MAP_UNZEROED)) == 0);
	arena_mapbitsp_write (mapbitsp, arena_mapbits_size_encode (size) | CHUNK_MAP_BININD_INVALID | flags | CHUNK_MAP_LARGE | CHUNK_MAP_ALLOCATED);
}
...
arena_mapbits_large_binind_set (arena_chunk_t *chunk, size_t pageind,
	...
	assert (arena_mapbits_large_size_get (chunk, pageind) == LARGE_MINCLASS + large_pad);
...
arena_mapbits_small_set (arena_chunk_t *chunk, size_t pageind, size_t runind,
	...
	assert ((flags & CHUNK_MAP_UNZEROED) == flags);
	arena_mapbitsp_write (mapbitsp, (runind << CHUNK_MAP_RUNIND_SHIFT) | (binind << CHUNK_MAP_BININD_SHIFT) | flags | CHUNK_MAP_ALLOCATED);
}
...
arena_ptr_small_binind_get (const void *ptr, size_t mapbits) {
	...
	if (config_debug) {
		...
		assert (((uintptr_t)ptr - ((uintptr_t)rpages + (uintptr_t)bin_info->reg0_offset)) % bin_info->reg_interval == 0);
	}
...
#define SIZE_INV(s) (((ZU (1) << SIZE_INV_SHIFT) / (s)) + 1)
		...
		if (likely (interval <= ((sizeof (interval_invs) / sizeof (size_t)) + 2))) {
...
arena_prof_tctx_set (tsdn_t *tsdn, const void *ptr, size_t usize,
	...
	if (likely (chunk != ptr)) {
		...
		if (unlikely (usize > SMALL_MAXCLASS || (uintptr_t)tctx > (uintptr_t)1U)) {
			arena_chunk_map_misc_t *elm;
...
arena_prof_tctx_reset (tsdn_t *tsdn, const void *ptr, size_t usize,
	...
	if (unlikely (usize > SMALL_MAXCLASS || (ptr == old_ptr && (uintptr_t)old_tctx > (uintptr_t)1U))) {
...
arena_salloc (tsdn_t *tsdn, const void *ptr, bool demote) {
	...
	if (likely (chunk != ptr)) {
		...
		if (unlikely (binind == BININD_INVALID || (config_prof && !demote && arena_mapbits_large_get (chunk, pageind) != 0))) {
			/*
			 ...
			 * small allocation.
			 */
			assert (config_cache_oblivious || ((uintptr_t)ptr & PAGE_MASK) == 0);
...
arena_dalloc (tsdn_t *tsdn, void *ptr, tcache_t *tcache, bool slow_path) {
	...
	if (likely (chunk != ptr)) {
		...
		if (likely ((mapbits & CHUNK_MAP_LARGE) == 0)) {
			...
			if (likely (tcache != NULL) && size - large_pad <= tcache_maxclass) {
