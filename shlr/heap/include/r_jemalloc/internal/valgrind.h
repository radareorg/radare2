 ...
 * usable space.
 */
#define JEMALLOC_VALGRIND_MAKE_MEM_NOACCESS(ptr, usize)          \
	do {                                                     \
		if (unlikely (in_valgrind))                      \
			valgrind_make_mem_noaccess (ptr, usize); \
	} while (0)
#define JEMALLOC_VALGRIND_MAKE_MEM_UNDEFINED(ptr, usize)          \
	do {                                                      \
		if (unlikely (in_valgrind))                       \
			valgrind_make_mem_undefined (ptr, usize); \
	} while (0)
#define JEMALLOC_VALGRIND_MAKE_MEM_DEFINED(ptr, usize)          \
	do {                                                    \
		if (unlikely (in_valgrind))                     \
 ...
 */
#define JEMALLOC_VALGRIND_MALLOC(cond, tsdn, ptr, usize, zero)                   \
	do {                                                                     \
...
#define JEMALLOC_VALGRIND_REALLOC(moved, tsdn, ptr, usize, ptr_null,                                    \
	old_ptr, old_usize, old_rzsize, old_ptr_null, zero)                                             \
	do {                                                                                            \
		if (unlikely (in_valgrind)) {                                                           \
			...
			if (!JEMALLOC_VALGRIND_REALLOC_MOVED_##moved (ptr,                              \
				...
				if (!JEMALLOC_VALGRIND_REALLOC_OLD_PTR_NULL_##old_ptr_null (old_ptr)) { \
					valgrind_freelike_block (old_ptr,                               \
						old_rzsize);                                            \
				}                                                                       \
				if (!JEMALLOC_VALGRIND_REALLOC_PTR_NULL_##ptr_null (ptr)) {             \
