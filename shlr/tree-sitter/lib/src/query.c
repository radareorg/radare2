	...
	for (unsigned i = 0; i < self->steps.size; i++) {
		...
		if (step->capture_ids[0] != NONE) {
			...
			for (unsigned j = i + 1; j < self->steps.size; j++) {
				...
				if (s->depth == PATTERN_DONE_MARKER || s->depth <= depth)
					break;
				if (s->capture_ids[0] != NONE)
					step->contains_captures = true;
			}
		}
	}
		...
		if (self->ascending) {
			...
			for (unsigned i = 0; i < self->query->wildcard_root_pattern_count; i++) {
				...
				if (step->field && field_id != step->field)
					continue;
				if (!ts_query_cursor__add_state (self, pattern))
					break;
			}
	...
	for (;;) {
		...
		if (self->finished_states.size > 0) {
			...
			if (first_finished_state_index != -1) {
				QueryState *state = &self->finished_states.contents[first_finished_state_index];
				match->id = state->id;
				match->pattern_index = state->pattern_index;
				const CaptureList *captures = capture_list_pool_get (
					&self->capture_list_pool,
