...
grub_reiserfs_set_key_offset (struct grub_reiserfs_key *key,
	...
	if (grub_reiserfs_get_key_version (key) == 1)
		...
		key->u.v2.offset_type = ((key->u.v2.offset_type & grub_cpu_to_le64 (15ULL << 60)) | grub_cpu_to_le64 (value & (~0ULL >> 4)));
}
...
grub_reiserfs_set_key_type (struct grub_reiserfs_key *key,
	...
	if (version == 1)
		...
		key->u.v2.offset_type = ((key->u.v2.offset_type & grub_cpu_to_le64 (~0ULL >> 4)) | grub_cpu_to_le64 ((grub_uint64_t)type << 60));
...
grub_reiserfs_compare_keys (const struct grub_reiserfs_key *key1,
	...
	if ((type1 == GRUB_REISERFS_ANY && (type2 == GRUB_REISERFS_DIRECT || type2 == GRUB_REISERFS_INDIRECT)) || (type2 == GRUB_REISERFS_ANY && (type1 == GRUB_REISERFS_DIRECT || type1 == GRUB_REISERFS_INDIRECT)))
		return 0;
	if (type1 < type2)
		return -1;
	if (type1 > type2)
...
grub_reiserfs_get_item (struct grub_reiserfs_data *data,
	...
	do {
		grub_disk_read (data->disk,
			...
			(((grub_off_t)block_number * block_size) & (GRUB_DISK_SECTOR_SIZE - 1)),
		...
		if (current_level > 1) {
			/* Internal node. Navigate to the child that should contain
             the searched key.  */
			struct grub_reiserfs_key *keys = (struct grub_reiserfs_key *)(block_header + 1);
			struct grub_reiserfs_disk_child *children = ((struct grub_reiserfs_disk_child *)(keys + item_count));

			for (i = 0;
				i < item_count && grub_reiserfs_compare_keys (key, &(keys[i])) >= 0;
				i++) {
#ifdef GRUB_REISERFS_DEBUG
			...
			if ((i < item_count) && (key->directory_id == keys[i].directory_id) && (key->object_id == keys[i].object_id))
				item->next_offset = grub_reiserfs_get_key_offset (&(keys[i]));
#ifdef GRUB_REISERFS_DEBUG
			if (i == item_count || grub_reiserfs_compare_keys (key, &(keys[i])) == 0)
				grub_printf (">");
			else
				grub_printf ("<");
			if (i < item_count) {
			...
			for (i = 0;
				i < item_count && (grub_reiserfs_compare_keys (key, &(item_headers[i].key)) != 0);
				i++) {
#ifdef GRUB_REISERFS_DEBUG
...
grub_reiserfs_iterate_dir (grub_fshelp_node_t item,
	...
	do {
      ...
      if (grub_le_to_cpu16 (block_header->level) != 1)
		...
		item_headers = (struct grub_reiserfs_item_header *)(block_header + 1);
		directory_headers = ((struct grub_reiserfs_directory_header *)((char *)block_header + grub_le_to_cpu16 (item_headers[block_position].item_location)));
		entry_count = grub_le_to_cpu16 (item_headers[block_position].u.entry_count);
		for (entry_number = 0; entry_number < entry_count; entry_number++) {
			struct grub_reiserfs_directory_header *directory_header = &directory_headers[entry_number];
			grub_uint16_t entry_state = grub_le_to_cpu16 (directory_header->state);
			...
			if (entry_state & GRUB_REISERFS_VISIBLE_MASK) {
				...
				entry_name = (((char *)directory_headers) + grub_le_to_cpu16 (directory_header->location));
				...
				if (grub_reiserfs_get_item (data, &entry_key, entry_item) != GRUB_ERR_NONE) {
				...
				if (entry_item->type == GRUB_REISERFS_DIRECTORY)
					...
					if (entry_item->block_number != 0) {
						...
						entry_version = grub_le_to_cpu16 (entry_item->header.version);
...
#endif
							if ((grub_le_to_cpu16 (entry_v1_stat.mode) & S_IFLNK) == S_IFLNK)
...
#endif
							if ((grub_le_to_cpu16 (entry_v2_stat.mode) & S_IFLNK) == S_IFLNK)
								entry_type = GRUB_FSHELP_SYMLINK;
							else
								entry_type = GRUB_FSHELP_REG;
						}
...
grub_reiserfs_open (struct grub_file *file, const char *name) {
	...
	if (entry_version == 0) /* Version 1 stat item. */
	{
		...
		grub_disk_read (data->disk,
			block_number * (block_size >> GRUB_DISK_SECTOR_BITS),
			entry_location + (((grub_off_t)block_number * block_size) & (GRUB_DISK_SECTOR_SIZE - 1)),
	...
	while (current_position < final_position) {
		...
		case GRUB_REISERFS_DIRECT:
			...
			if (initial_position < current_position + item_size) {
				...
				length = (MIN (item_size, final_position - current_position) - offset);
				grub_dprintf ("reiserfs",
		...
		case GRUB_REISERFS_INDIRECT:
			...
			for (indirect_block = 0;
				indirect_block < indirect_block_count && current_position < final_position;
				indirect_block++) {
				...
				if (current_position + block_size >= initial_position) {
					offset = MAX ((signed)(initial_position - current_position),
						0);
					length = (MIN (block_size, final_position - current_position) - offset);
					grub_dprintf ("reiserfs",
