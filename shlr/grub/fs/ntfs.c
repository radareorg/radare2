...
retry:
		while (at->attr_nxt < at->attr_end) {
			...
			if (((unsigned char)*at->attr_cur == attr) || (attr == 0)) {
				...
				if (at->flags & AF_MMFT) {
					if ((grub_disk_read (at->mft->data->disk, v32at (at->attr_cur, 0x10), 0,
						    512, at->emft_buf)) ||
						(grub_disk_read (at->mft->data->disk, v32at (at->attr_cur, 0x14), 0,
							512, at->emft_buf + 512)))
						return NULL;

					if (fixup (at->mft->data, at->emft_buf, at->mft->data->mft_size,
						    "FILE"))
						return NULL;
				} else {
	...
	if (at->attr_end) {
		...
		if (pa[8]) {
			...
			n = ((u32at (pa, 0x30) + GRUB_DISK_SECTOR_SIZE - 1) & (~(GRUB_DISK_SECTOR_SIZE - 1)));
...
grub_ntfs_read_block (grub_fshelp_node_t node, grub_disk_addr_t block) {
	...
	if (block >= ctx->next_vcn) {
		...
		return (ctx->flags & RF_BLNK) ? 0 : (block - ctx->curr_vcn + ctx->curr_lcn);
}
...
read_mft (struct grub_ntfs_data *data, char *buf, grub_uint32_t mftno,
	...
	if (read_attr (&data->mmft.attr, buf, mftno * ((grub_disk_addr_t)data->mft_size << BLK_SHR),
	...
	if (hook)
       ...
       */
			if ((ns) && (namespace != 2)) {
				...
				type =
					(u32at (pos, 0x48) & ATTR_DIRECTORY) ? GRUB_FSHELP_DIR : GRUB_FSHELP_REG;
...
grub_ntfs_iterate_dir (grub_fshelp_node_t dir,
	...
	while ((cur_pos = find_attr (at, AT_BITMAP)) != NULL) {
		...
		if ((cur_pos[9] == 4) &&
			...
			bitmap_len = ((is_resident) ? u32at (cur_pos, 0x10) : u32at (cur_pos, 0x28));
	...
	if (bitmap) {
		...
		for (i = 0; i < (grub_disk_addr_t)bitmap_len * 8; i++) {
			if (*bitmap & v) {
				if ((read_attr (at, indx, i * (mft->data->idx_size << BLK_SHR),
...
grub_ntfs_mount (grub_disk_t disk) {
	...
	if (grub_disk_read (disk, data->mft_start, 0, data->mft_size << BLK_SHR, data->mmft.buf))
		goto fail;
