...
grub_fat_mount (grub_disk_t disk) {
	...
	if (grub_strncmp ((const char *)bpb.version_specific.fat12_or_fat16.fstype, "FAT12", 5) && grub_strncmp ((const char *)bpb.version_specific.fat12_or_fat16.fstype, "FAT16", 5) && grub_strncmp ((const char *)bpb.version_specific.fat32.fstype, "FAT32", 5))
		goto fail;
...
grub_fat_iterate_dir (grub_disk_t disk, struct grub_fat_data *data,
	...
	for (offset = 0;; offset += sizeof (dir)) {
		...
		if ((grub_fat_read_data (disk, data, 0, 0, 0,
			     offset, sizeof (dir), (char *)&dir) != sizeof (dir) ||
			    dir.name[0] == 0))
			break;
