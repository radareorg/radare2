...
grub_ufs_read_inode (struct grub_ufs_data *data, int ino, char *inode) {
...
#ifndef MODE_UFS2
	grpblk += grub_num_to_cpu32 (sblock->cylg_offset, data->be) * (group & (~grub_num_to_cpu32 (sblock->cylg_mask, data->be)));
#endif
	...
	grub_disk_read (data->disk,
		...
		(grpino % UFS_INODE_PER_BLOCK) * sizeof (struct grub_ufs_inode),
	...
	do {
...
#endif
		{
			...
			if (!grub_strcmp (name, filename)) {
				dirino = data->ino;
				grub_ufs_read_inode (data, grub_num_to_cpu32 (dirent.ino, data->be), 0);
		...
		while (pos < INODE_SIZE (data)) {
...

			{
				...
				info.dir = ((grub_num_to_cpu16 (inode.mode, data->be) &
						    GRUB_UFS_ATTR_TYPE) == GRUB_UFS_ATTR_DIR);
