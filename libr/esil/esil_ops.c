...
static bool esil_swap(REsil *esil) {
	...
	if (!esil->stack[stackptr - 1][0] || !esil->stack[stackptr - 2][0]) {
		return false;
	}
	char tmp[R_ESIL_TOKEN_SIZE];
	memcpy (tmp, esil->stack[stackptr - 1], R_ESIL_TOKEN_SIZE);
	memcpy (esil->stack[stackptr - 1], esil->stack[stackptr - 2], R_ESIL_TOKEN_SIZE);
	memcpy (esil->stack[stackptr - 2], tmp, R_ESIL_TOKEN_SIZE);
	return true;
}
