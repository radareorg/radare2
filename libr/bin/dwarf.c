...
static const ut8 *get_section_bytes(RBin *bin, RBinSection *section) {
  ...
  if (section->bytes.len && section->bytes.ptr) {
    ...
    RSlice buf = r_arena_scalloc (bin->arena, dst_len + 1);
			...
			if (i) {
				...
				if (id_idx > 0) {
					...
					if (include_dir && include_dir[0] != '/') {
						...
						if (comp_dir) {
							include_dir = r_arena_push_strf (bin->arena, "%s/%s", comp_dir, include_dir);
						}
					} else {
						// XXX
					}
				...
				if (hdr->file_names) {
					hdr->file_names[count].name = r_arena_push_strf (bin->arena, "%s/%s", r_str_get (include_dir), fn);
...
static const ut8 *parse_comp_unit(RBin *bin, RBinDwarfDebugInfo *info, Sdb *sdb, const ut8 *buf_start, const ut8 *buf_end, RBinDwarfCompUnit *unit, const RVecDwarfAbbrevDecl *abbrevs, size_t first_abbr_idx) {
  ...
  size_t abbrevs_count = RVecDwarfAbbrevDecl_length(abbrevs);
    ...
    RBinDwarfAbbrevDecl *abbrev= RVecDwarfAbbrevDecl_at(abbrevs, abbr_idx - 1);
		...
		if (!init_die (bin->arena, &die, abbr_code, abbrevs_count)) {
			return NULL; // error
		}
		die.tag = abbrev->tag;
...
R_API RBinDwarfDebugInfo *r_bin_dwarf_parse_info(RBin *bin, RVecDwarfAbbrevDecl *da, int mode) {
  ...
  info = parse_info_raw (bin, bf->sdb_addrinfo, da, buf, section->bytes.len);
  if (!info) return NULL;
...
R_API RList *r_bin_dwarf_parse_line(RBin *bin, int mode) {
	...
	if (bf && section) {
		...
		parse_line_raw (bin, buf, section->bytes.len, mode);
