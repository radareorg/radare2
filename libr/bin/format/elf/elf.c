...
static bool init_ehdr(ELFOBJ *eo) {
	...
	sdb_set (eo->kv, "elf_type.cparse", "enum elf_type { ET_NONE=0, ET_REL=1,"
			" ET_EXEC=2, ET_DYN=3, ET_CORE=4, ET_LOOS=0xfe00, ET_HIOS=0xfeff,"
			" ET_LOPROC=0xff00, ET_HIPROC=0xffff };", 0);
	sdb_set (eo->kv, "elf_machine.cparse", "enum elf_machine {EM_NONE=0, EM_M32=1,"
	...
	sdb_set (eo->kv, "elf_class.cparse", "enum elf_class {ELFCLASSNONE=0, ELFCLASS32=1, ELFCLASS64=2};", 0);
	sdb_set (eo->kv, "elf_data.cparse", "enum elf_data {ELFDATANONE=0, ELFDATA2LSB=1, ELFDATA2MSB=2};", 0);
	sdb_set (eo->kv, "elf_hdr_version.cparse", "enum elf_hdr_version {EV_NONE=0, EV_CURRENT=1};", 0);
	sdb_set (eo->kv, "elf_obj_version.cparse", "enum elf_obj_version {EV_NONE=0, EV_CURRENT=1};", 0);
...
ut64 Elf_(r_bin_elf_get_phnum)(ELFOBJ *obj) {
	...
	if (obj->ehdr.e_phnum == UT16_MAX) {
		...
		if (obj->ehdr.e_shnum > 0) {
			Elf_(Shdr) shdr = {0};
			(void)r_buf_read_at (obj->b, obj->ehdr.e_shoff, (ut8 *)&shdr, sizeof (shdr));
...
static ut64 get_import_addr_mips(ELFOBJ *eo, RBinElfReloc *rel) {
	ut64 jmprel_addr = eo->dyn_info.dt_jmprel;
	ut64 got_addr = eo->dyn_info.dt_mips_pltgot;
	...
	ut8 buf[1024];
	ut64 plt_addr = jmprel_addr + eo->dyn_info.dt_pltrelsz;
	ut64 p_plt_addr = Elf_(r_bin_elf_v2p_new) (eo, plt_addr);
	int res = r_buf_read_at (eo->b, p_plt_addr, buf, sizeof (buf));
	if (res != sizeof (buf)) {
		return UT64_MAX;
	}
...
static ut64 get_import_addr_riscv(ELFOBJ *eo, RBinElfReloc *rel) {
	ut64 got_addr = eo->dyn_info.dt_pltgot;
	if (got_addr == R_BIN_ELF_ADDR_MAX) {
		return UT64_MAX;
	}
...
static ut64 get_import_addr_loongarch(ELFOBJ *eo, RBinElfReloc *rel) {
	ut64 got_addr = eo->dyn_info.dt_pltgot;
	if (got_addr == R_BIN_ELF_ADDR_MAX) {
		return UT64_MAX;
	}
...
static ut64 get_import_addr_x86(ELFOBJ *eo, RBinElfReloc *rel) {
	...
	RBinElfSection *pltsec_section = get_section_by_name (eo, ".plt.sec");
	if (pltsec_section) {
		ut64 got_addr = eo->dyn_info.dt_pltgot;
		ut64 pos = COMPUTE_PLTGOT_POSITION (rel, got_addr, 0x3);
...
int Elf_(r_bin_elf_has_relro)(ELFOBJ *eo) {
	r_return_val_if_fail (eo, R_ELF_NO_RELRO);
	bool have_bind_now = false;
	bool have_gnu_relro = false;

	if (eo->dyn_info.dt_bind_now) {
		have_bind_now = true;
	} else if (eo->dyn_info.dt_flags != R_BIN_ELF_XWORD_MAX && eo->dyn_info.dt_flags != R_BIN_ELF_XWORD_MAX) {
		have_bind_now = eo->dyn_info.dt_flags_1 & DF_1_NOW;
	}

	if (eo->phdr) {
		size_t i;
		for (i = 0; i < eo->ehdr.e_phnum; i++) {
			if (eo->phdr[i].p_type == PT_GNU_RELRO) {
				have_gnu_relro = true;
				break;
			}
		}
...
ut64 Elf_(r_bin_elf_get_init_offset)(ELFOBJ *eo) {
	...
	if (!eo || entry == UT64_MAX) {
		return UT64_MAX;
	}

	ut8 buf[128];
	if (r_buf_read_at (eo->b, entry + 16, buf, sizeof (buf)) < 1) {
...
ut64 Elf_(r_bin_elf_get_entry_offset)(ELFOBJ *eo) {
	...
	if (!Elf_(r_bin_elf_is_executable) (eo)) {
		return UT64_MAX;
	}
...
ut64 Elf_(r_bin_elf_get_main_offset)(ELFOBJ *eo) {
	...
	ut64 m = lookup_main_symbol_offset (eo);
	if (m != UT64_MAX) {
		return m;
	}
...
char* Elf_(r_bin_elf_get_file_type)(ELFOBJ *eo) {
	...
	ut32 e_type = (ut32)eo->ehdr.e_type; // cast to avoid warn in iphone-gcc, must be ut16
	switch (e_type) {
...
const RVector* Elf_(r_bin_elf_load_sections)(RBinFile *bf, ELFOBJ *eo) {
	...
	const RVector *elf_bin_sections = _load_elf_sections (eo);
	r_vector_init (&eo->cached_sections, sizeof (RBinSection), (RVectorFree) _fini_bin_section, NULL);
	_cache_bin_sections (bf, eo, elf_bin_sections);
...
static RVector* load_symbols_from_phdr(ELFOBJ *eo, int type) {
	...
	ReadPhdrSymbolState state = { .type = type, .ret = ret, .sym = &sym, .nsym = nsym, .addr_sym_table = addr_sym_table, };
...
bool Elf_(r_bin_elf_is_executable)(ELFOBJ *eo) {
