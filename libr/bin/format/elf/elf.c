...
//XXX this also trust on shdr maybe can be achieved with phdr
static size_t get_relocs_num(struct Elf_(r_bin_elf_obj_t) *bin) {
	int class;
	size_t i, ret = 0;

	if (!g_sections) return 0;
	class = bin->ehdr.e_ident[EI_CLASS];
	for (i = 0; !g_sections[i].last; i++) {
		if (!strncmp (g_sections[i].name, ".rela.", strlen (".rela."))) {
			ret += class == 1 ? (g_sections[i].size) / (sizeof (ut32) * 3) :
					(g_sections[i].size) / (sizeof (ut64) * 3);
		} else if (!strncmp (g_sections[i].name, ".rel.", strlen (".rel."))){
			ret += class == 1 ? (g_sections[i].size) / (sizeof (ut32) * 2) :
					(g_sections[i].size) / (sizeof (ut64) * 2);
		}
	}
	return ret;
...
static int read_reloc(struct Elf_(r_bin_elf_obj_t) *bin, RBinElfReloc *r,
	...
	if (bin->ehdr.e_ident[EI_CLASS] == 1) {
		fmt = bin->endian ? "I" : "i";
		if (offset + sizeof (ut32) > bin->size ||
			(offset + sizeof (ut32) * 2) > bin->size)
			return -1;
		...
		r->last = 0;
		if (is_rela) r->addend = i3;
		return is_rela ? sizeof (ut32) * 3 : sizeof (ut32) * 2;
	} else {
		fmt = bin->endian ? "L" : "l";
		if (offset + sizeof (ut64) > bin->size ||
			offset + sizeof (ut64) * 2 > bin->size)
			return -1;
