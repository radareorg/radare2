...
static Elf_(Dyn) *Elf_(r_bin_elf_seek_dyn_tag)(struct Elf_(r_bin_elf_obj_t) *bin, const ut32 tag) {
	int i;
...
// and the DT_GNU_HASH one if the former isn't present.
// http://deroko.phearless.org/dt_gnu_hash.txt
static int r_bin_elf_get_symbols_count (struct Elf_(r_bin_elf_obj_t) *bin) {
	Elf_(Dyn) *hash;
	
	hash = Elf_(r_bin_elf_seek_dyn_tag) (bin, DT_HASH);

	if (hash) {
		Elf_Hash dt_hash;

		if (r_buf_fread_at (bin->b, hash->d_un.d_ptr - bin->baddr, (ut8*)&dt_hash, 
					bin->endian?"2I":"2i", 1) < 0) {
...
static int Elf_(r_bin_elf_init_shdr)(struct Elf_(r_bin_elf_obj_t) *bin) {
	...
	if (!bin->ehdr.e_shnum && bin->dyn_entries) {
		...
		{
			Elf_(Dyn) *strtab, *strsz;

			strtab = Elf_(r_bin_elf_seek_dyn_tag) (bin, DT_STRTAB);
			strsz = Elf_(r_bin_elf_seek_dyn_tag) (bin, DT_STRSZ);
		...
		}
		{
			...
			symtab = Elf_(r_bin_elf_seek_dyn_tag) (bin, DT_SYMTAB);
			symcount = r_bin_elf_get_symbols_count (bin);
...
static int Elf_(r_bin_elf_init_strtab)(struct Elf_(r_bin_elf_obj_t) *bin) {
	Elf_(Dyn) *dyn_strtab, *dyn_strsz;
	ut32 off, size;

	if (bin->strtab)
		return R_TRUE;

	if (!bin->shstrtab && bin->shdr) {
	...
	if (!bin->ehdr.e_shnum && dyn_strtab && dyn_strsz) {
	...
	sdb_num_set (bin->kv, "elf_strtab.offset", off, 0);
...
char *Elf_(r_bin_elf_get_rpath)(struct Elf_(r_bin_elf_obj_t) *bin) {
	...
	if (!dy)
		dy = Elf_(r_bin_elf_seek_dyn_tag) (bin, DT_RUNPATH);
	if (!dy)
		return NULL;
...
struct r_bin_elf_section_t* Elf_(r_bin_elf_get_sections)(struct Elf_(r_bin_elf_obj_t) *bin) {
	...
	for (i = 0; i < bin->ehdr.e_shnum; i++) {
		...
		nidx = bin->shdr[i].sh_name;
		if (nidx<0 || !bin->shstrtab_size || nidx > bin->shstrtab_size) {
