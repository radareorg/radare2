...
typedef void(*RExportsIterator)(struct MACHO_(obj_t) * mo, const char *name, ut64 flags, ut64 offset, void *ctx);
...
static bool segment_filebacked_size(struct MACHO_(obj_t) * mo, int seg_idx, R_OUT ut64 *size) {
	R_RETURN_VAL_IF_FAIL (mo && size, false);
	if (seg_idx < 0 || seg_idx >= mo->nsegs) {
		return false;
	}
	struct MACHO_(segment_command) *seg = &mo->segs[seg_idx];
...
static ut64 entry_to_vaddr(struct MACHO_(obj_t) * bin) {
...
static ut64 addr_to_offset(struct MACHO_(obj_t) * mo, ut64 addr) {
	if (mo->segs) {
		size_t i;
		for (i = 0; i < mo->nsegs; i++) {
			struct MACHO_(segment_command) *seg = &mo->segs[i];
...
static ut64 offset_to_vaddr(struct MACHO_(obj_t) * mo, ut64 offset) {
	if (mo->segs) {
		size_t i;
		for (i = 0; i < mo->nsegs; i++) {
			struct MACHO_(segment_command) *seg = &mo->segs[i];
...
static ut64 pa2va(RBinFile *bf, ut64 offset) {
	...
	struct MACHO_(obj_t) *mo = bf->bo->bin_obj;
	return mo? offset_to_vaddr (mo, offset): offset;
}

static void init_sdb_formats(struct MACHO_(obj_t) * mo) {
	Sdb *kv = mo->kv;
	/*
	 * These definitions are used by r2 -nn
	 * must be kept in sync with libr/bin/d/macho
	 */
	sdb_set (kv, "macho_build_platform.cparse", "enum macho_build_platform"
		"{MACOS=1, IOS=2, TVOS=3, WATCHOS=4, BRIDGEOS=5, IOSMAC=6, IOSSIMULATOR=7, TVOSSIMULATOR=8, WATCHOSSIMULATOR=9};",
		0);
	sdb_set (kv, "macho_build_tool.cparse", "enum macho_build_tool"
		"{CLANG=1, SWIFT=2, LD=3};",
		0);
	sdb_set (kv, "macho_load_command_type.cparse", "enum macho_load_command_type"
		...
		0);
	sdb_set (kv, "macho_header_filetype.cparse", "enum macho_header_filetype"
		...
		0);
	sdb_set (kv, "macho_header_flags.cparse", "enum macho_header_flags"
		...
		0);
	sdb_set (kv, "macho_section_types.cparse", "enum macho_section_types"
		...
		0);
	sdb_set (kv, "macho_section_attrs.cparse", "enum macho_section_attrs"
		...
		0);
	sdb_set (kv, "macho_header.format", "xxx[4]Edd[4]B "
		"magic cputype cpusubtype (macho_header_filetype)filetype ncmds sizeofcmds (macho_header_flags)flags",
		0);
	sdb_set (kv, "macho_segment.format", "[4]Ed[16]zxxxxoodx "
		"(macho_load_command_type)cmd cmdsize segname vmaddr vmsize fileoff filesize maxprot initprot nsects flags",
		0);
	sdb_set (kv, "macho_segment64.format", "[4]Ed[16]zqqqqoodx "
		"(macho_load_command_type)cmd cmdsize segname vmaddr vmsize fileoff filesize maxprot initprot nsects flags",
		0);
	sdb_set (kv, "macho_symtab_command.format", "[4]Edxdxd "
		"(macho_load_command_type)cmd cmdsize symoff nsyms stroff strsize",
		0);
	sdb_set (kv, "macho_dysymtab_command.format", "[4]Edddddddddddxdxdxxxd "
		"(macho_load_command_type)cmd cmdsize ilocalsym nlocalsym iextdefsym nextdefsym iundefsym nundefsym tocoff ntoc moddtaboff nmodtab extrefsymoff nextrefsyms inddirectsymoff nindirectsyms extreloff nextrel locreloff nlocrel",
		0);
	sdb_set (kv, "macho_section.format", "[16]z[16]zxxxxxx[1]E[3]Bxx "
		"sectname segname addr size offset align reloff nreloc (macho_section_types)flags_type (macho_section_attrs)flags_attr reserved1 reserved2",
		0);
	sdb_set (kv, "macho_section64.format", "[16]z[16]zqqxxxx[1]E[3]Bxxx "
		"sectname segname addr size offset align reloff nreloc (macho_section_types)flags_type (macho_section_attrs)flags_attr reserved1 reserved2 reserved3",
		0);
	sdb_set (kv, "macho_dylib.format", "xxxxz "
		"name_offset timestamp current_version compatibility_version name",
		0);
	sdb_set (kv, "macho_dylib_command.format", "[4]Ed? "
		"(macho_load_command_type)cmd cmdsize (macho_dylib)dylib",
		0);
	sdb_set (kv, "macho_id_dylib_command.format", "[4]Ed? "
		"(macho_load_command_type)cmd cmdsize (macho_dylib)dylib",
		0);
	sdb_set (kv, "macho_uuid_command.format", "[4]Ed[16]b "
		"(macho_load_command_type)cmd cmdsize uuid",
		0);
	sdb_set (kv, "macho_rpath_command.format", "[4]Edxz "
		"(macho_load_command_type)cmd cmdsize path_offset path",
		0);
	sdb_set (kv, "macho_entry_point_command.format", "[4]Edqq "
		"(macho_load_command_type)cmd cmdsize entryoff stacksize",
		0);
	sdb_set (kv, "macho_encryption_info64_command.format", "[4]Edxddx "
		"(macho_load_command_type)cmd cmdsize offset size id padding",
		0);
	sdb_set (kv, "macho_encryption_info_command.format", "[4]Edxdd "
		"(macho_load_command_type)cmd cmdsize offset size id",
		0);
	sdb_set (kv, "macho_code_signature_command.format", "[4]Edxd "
		"(macho_load_command_type)cmd cmdsize offset size",
		0);
	sdb_set (kv, "macho_dyld_info_only_command.format", "[4]Edxdxdxdxdxd "
		"(macho_load_command_type)cmd cmdsize rebase_off rebase_size bind_off bind_size weak_bind_off weak_bind_size lazy_bind_off lazy_bind_size export_off export_size",
		0);
	sdb_set (kv, "macho_load_dylinker_command.format", "[4]Edxz "
		"(macho_load_command_type)cmd cmdsize name_offset name",
		0);
	sdb_set (kv, "macho_id_dylinker_command.format", "[4]Edxzi "
		"(macho_load_command_type)cmd cmdsize name_offset name",
		0);
	sdb_set (kv, "macho_build_version_command.format", "[4]Ed[4]Exxd "
		"(macho_load_command_type)cmd cmdsize (macho_build_platform)platform minos sdk ntools",
		0);
	sdb_set (kv, "macho_build_version_tool.format", "[4]Ex "
		"(macho_build_tool)tool version",
		0);
	sdb_set (kv, "macho_source_version_command.format", "[4]Edq "
		"(macho_load_command_type)cmd cmdsize version",
		0);
	sdb_set (kv, "macho_function_starts_command.format", "[4]Edxd "
		"(macho_load_command_type)cmd cmdsize offset size",
		0);
	sdb_set (kv, "macho_data_in_code_command.format", "[4]Edxd "
		"(macho_load_command_type)cmd cmdsize offset size",
		0);
	sdb_set (kv, "macho_version_min_command.format", "[4]Edxx "
		"(macho_load_command_type)cmd cmdsize version reserved",
		0);
	sdb_set (kv, "macho_segment_split_info_command.format", "[4]Edxd "
		"(macho_load_command_type)cmd cmdsize offset size",
		0);
	sdb_set (kv, "macho_unixthread_command.format", "[4]Eddd "
		"(macho_load_command_type)cmd cmdsize flavor count",
		0);
	sdb_set (kv, "macho_aot_metadata.format", "[4]Eddddddd "
		"(macho_load_command_type)cmd cmdsize imagepathoffset imagepathlen field10 field14 x64code field1c",
		0);
}

static bool init_hdr(struct MACHO_(obj_t) * mo) {
	ut8 machohdrbytes[sizeof (struct MACHO_(mach_header))] = { 0 };
...
#endif
	init_sdb_formats (mo);
	sdb_num_set (mo->kv, "macho_header.offset", 0, 0); // wat about fatmacho?
	return true;
}

static bool parse_segments(struct MACHO_(obj_t) * mo, ut64 off) {
	size_t i, j, k, sect, len;
	ut32 size_sects;
	ut8 segcom[sizeof (struct MACHO_(segment_command))] = { 0 };
	ut8 sec[sizeof (struct MACHO_(section))] = { 0 };

	if (!UT32_MUL (&size_sects, mo->nsegs, sizeof (struct MACHO_(segment_command)))) {
		return false;
	}
	if (!size_sects || size_sects > mo->size) {
		return false;
	}
	if (off > mo->size || off + sizeof (struct MACHO_(segment_command)) > mo->size) {
		return false;
	}
	if (! (mo->segs = realloc (mo->segs, mo->nsegs * sizeof (struct MACHO_(segment_command))))) {
		r_sys_perror ("realloc (seg)");
		return false;
	}
	j = mo->nsegs - 1;
	len = r_buf_read_at (mo->b, off, segcom, sizeof (struct MACHO_(segment_command)));
	if (len != sizeof (struct MACHO_(segment_command))) {
		R_LOG_ERROR ("read (seg)");
		return false;
	}
	const ut8 *scp = (const ut8 *)&segcom;
	const bool be = mo->big_endian;
	struct MACHO_(segment_command) *seg = &mo->segs[j];
...
#if R_BIN_MACHO64
	sdb_num_setf (mo->kv, off, 0, "macho_segment64_%u.offset", (ut32)j);
#else
	sdb_num_setf (mo->kv, off, 0, "macho_segment_%u.offset", (ut32)j);
#endif
	sdb_num_set (mo->kv, "macho_segments.count", 0, 0);
	...
	if (seg->nsects > 0) {
		...
		if (mo->nsects > MACHO_MAX_SECTIONS) {
			...
			R_LOG_WARN ("macho header contains too many sections (%d). Wrapping to %d",
				mo->nsects,
				new_nsects);
		...
		if (!UT32_MUL (&size_sects, mo->nsects - sect, sizeof (struct MACHO_(section)))) {
			mo->nsects = sect;
			return false;
		}
		if (!size_sects || size_sects > mo->size) {
		...
		if (seg->cmdsize != sizeof (struct MACHO_(segment_command)) + (sizeof (struct MACHO_(section)) * seg->nsects)) {
			mo->nsects = sect;
			return false;
		}

		if (off + sizeof (struct MACHO_(segment_command)) > mo->size ||
			off + sizeof (struct MACHO_(segment_command)) + size_sects > mo->size) {
			mo->nsects = sect;
			return false;
		}

		if (! (mo->sects = realloc (mo->sects, mo->nsects * sizeof (struct MACHO_(section))))) {
		...
		for (k = sect, j = 0; k < mo->nsects; k++, j++) {
			ut64 offset = off + sizeof (struct MACHO_(segment_command)) + j * sizeof (struct MACHO_(section));
			len = r_buf_read_at (mo->b, offset, sec, sizeof (struct MACHO_(section)));
			if (len != sizeof (struct MACHO_(section))) {
				R_LOG_ERROR ("read sects");
				mo->nsects = sect;
				return false;
			}

			struct MACHO_(section) *sk = &mo->sects[k];
			i = 0;
			...
			sdb_num_setf (mo->kv, offset, 0,
				"macho_section_%.16s_%.16s.offset", sk->segname, sk->sectname);
#if R_BIN_MACHO64
			sdb_setf (mo->kv, "macho_section64", 0,
				"macho_section_%.16s_%.16s.format", sk->segname, sk->sectname);
#else
			sdb_setf (mo->kv, "macho_section", 0,
				"macho_section_%.16s_%.16s.format", sk->segname, sk->sectname);
#endif
...
static void reset_symtab(struct MACHO_(obj_t) * mo) {
	R_FREE (mo->symtab);
	R_FREE (mo->symstr);
	mo->nsymtab = 0;
	mo->symstrlen = 0;
...
static bool defer_fail(struct MACHO_(obj_t) * mo, ut8 *symdata) {
	free (symdata);
	reset_symtab (mo);
	return false;
}

static bool parse_symtab(struct MACHO_(obj_t) * mo, ut64 off) {
	size_t i;
	ut8 *symdata = NULL;
	...
	ut32 size_sym;
	if (!UT32_MUL (&size_sym, nsyms, sizeof (struct MACHO_(nlist))) || !size_sym || size_sym > ST32_MAX) {
		return defer_fail (mo, symdata);
	}
	...
	ut64 max_nsymtab = (bsz - symoff) / sizeof (struct MACHO_(nlist));
	if (nsyms > max_nsymtab) {
		return defer_fail (mo, symdata);
	}
	mo->symtab = calloc (nsyms, sizeof (struct MACHO_(nlist)));
	if (!mo->symtab) {
		return defer_fail (mo, symdata);
	}
	mo->nsymtab = nsyms;
	...
	const size_t nlist_size = sizeof (struct MACHO_(nlist));
	const size_t count = mo->nsymtab;
	if (be) {
		for (i = 0; i < count; i++) {
			const ut8 *nlst_ptr = symdata + (i * nlist_size);
			struct MACHO_(nlist) *sti = &mo->symtab[i];
...
static bool parse_aot_metadata(struct MACHO_(obj_t) * mo, ut64 off) {
	ut32 words[8];
...
static bool parse_dysymtab(struct MACHO_(obj_t) * mo, ut64 off) {
	size_t len, i;
	ut32 size_tab;
	ut8 dysym[sizeof (struct dysymtab_command)] = { 0 };
	ut8 dytoc[sizeof (struct dylib_table_of_contents)] = { 0 };
	ut8 dymod[sizeof (struct MACHO_(dylib_module))] = { 0 };
	...
	ut64 max_nmodtab = (mo->size - ds->modtaboff) / sizeof (struct MACHO_(dylib_module));
	if (mo->nmodtab > 0 && mo->nmodtab <= max_nmodtab) {
		if (!UT32_MUL (&size_tab, mo->nmodtab, sizeof (struct MACHO_(dylib_module)))) {
			return false;
		}
		if (!size_tab) {
			return false;
		...
		if (! (mo->modtab = calloc (mo->nmodtab, sizeof (struct MACHO_(dylib_module))))) {
			r_sys_perror ("calloc (modtab)");
			return false;
		}
		for (i = 0; i < mo->nmodtab; i++) {
			len = r_buf_read_at (mo->b, ds->modtaboff + i * sizeof (struct MACHO_(dylib_module)), dymod, sizeof (struct MACHO_(dylib_module)));
			if (len == -1) {
				R_LOG_ERROR ("read (modtab)");
				R_FREE (mo->modtab);
				return false;
			}
			struct MACHO_(dylib_module) *mt = &mo->modtab[i];
...
static void set_malformed_entitlement(struct MACHO_(obj_t) * mo) {
	free (mo->signature);
	mo->signature = (ut8 *)strdup ("Malformed entitlement");
}

static ut8 *parse_der(struct MACHO_(obj_t) *mo, ut64 slot_off, ut32 super_length, ut32 idx_offset, ut32 *out_size) {
	struct blob_t der_blob = { 0 };
	if (!fits_in (mo->size, slot_off, sizeof (struct blob_t))) {
		return NULL;
	}
	der_blob.magic = r_buf_read_ble32_at (mo->b, slot_off, macho_endian);
	if (der_blob.magic != CSMAGIC_DER_ENTITLEMENTS) {
		return NULL;
	}
	der_blob.length = r_buf_read_ble32_at (mo->b, slot_off + 4, macho_endian);
...
static bool parse_signature(struct MACHO_(obj_t) * mo, ut64 off) {
	int i, len;
	ut32 data;
	...
	super.blob.magic = r_buf_read_ble32_at (mo->b, data, macho_endian);
	super.blob.length = r_buf_read_ble32_at (mo->b, data + 4, macho_endian);
	super.count = r_buf_read_ble32_at (mo->b, data + 8, macho_endian);
	...
	for (i = 0; i < slots; i++, index_off += sizeof (struct blob_index_t)) {
		...
		idx.type = r_read_ble32 (&bi.type, macho_endian);
		idx.offset = r_read_ble32 (&bi.offset, macho_endian);
		...
		if (idx.type == CSSLOT_CODEDIRECTORY || (idx.type >= CSSLOT_ALTERNATE_CODEDIRECTORIES && idx.type < CSSLOT_ALTERNATE_CODEDIRECTORY_LIMIT)) {
			ut32 slot_magic = r_buf_read_ble32_at (mo->b, slot_off, macho_endian);
			ut32 slot_size = r_buf_read_ble32_at (mo->b, slot_off + 4, macho_endian);
			...
			if (slot_ok && idx.type == CSSLOT_CODEDIRECTORY) {
				mo->cs_flags = r_buf_read_ble32_at (mo->b, slot_off + 12, macho_endian);
				ut8 platform = 0;
		...
		case CSSLOT_ENTITLEMENTS:
			{
				...
				entitlements.magic = r_buf_read_ble32_at (mo->b, slot_off, macho_endian);
				entitlements.length = r_buf_read_ble32_at (mo->b, slot_off + 4, macho_endian);
		...
		case CSSLOT_SIGNATURESLOT: // ASN1/DER certificate
			{
				...
				ut32 sig_length = r_buf_read_ble32_at (mo->b, slot_off + 4, macho_endian);
		...
		case CSSLOT_REQUIREMENTS: // 2
		{
			...
			if (ident) {
				sdb_set (mo->kv, "macho.ident", ident, 0);
				free (ident);
			}
		}
		break;
...
static int parse_thread(struct MACHO_(obj_t) * mo, struct load_command *lc, ut64 off, bool is_first_thread) {
	...
	if (is_first_thread) {
		...
		if (pc_offset != UT64_MAX) {
			sdb_num_set (mo->kv, "macho.entry.offset", pc_offset, 0);
		}
	}
	return true;
wrong_read:
...
static bool parse_function_starts(struct MACHO_(obj_t) * mo, ut64 off) {
...
static int parse_dylib(struct MACHO_(obj_t) * mo, ut64 off) {
	...
	char lib[R_BIN_MACHO_STRING_LENGTH] = { 0 };
	...
	if (offname + R_BIN_MACHO_STRING_LENGTH > mo->size) {
		return false;
	}

	len = r_buf_read_at (mo->b, off + dl.dylib.name, (ut8 *)lib, R_BIN_MACHO_STRING_LENGTH - 1);
	if (len < 1) {
		R_LOG_ERROR ("read (dylib str)");
		return false;
	}

	char *name = r_str_ndup (lib, R_BIN_MACHO_STRING_LENGTH);
	RVecMachoLib_push_back (&mo->libs_cache, &name);
	return true;
}
...
static const char *cmd_to_pf_definition(ut32 cmd) {
	switch (cmd) {
	case LC_BUILD_VERSION:
		return "macho_build_version_command";
	case LC_CODE_SIGNATURE:
		return "macho_code_signature_command";
	case LC_DATA_IN_CODE:
		return "macho_data_in_code_command";
	case LC_DYLD_INFO:
	case LC_DYLD_INFO_ONLY:
		return "macho_dyld_info_only_command";
	case LC_DYLD_ENVIRONMENT:
		return "macho_load_dylinker_command";
	case LC_DYLIB_CODE_SIGN_DRS:
		return NULL;
	case LC_DYSYMTAB:
		return "macho_dysymtab_command";
	case LC_ENCRYPTION_INFO:
		return "macho_encryption_info_command";
	case LC_ENCRYPTION_INFO_64:
		return "macho_encryption_info64_command";
	case LC_FUNCTION_STARTS:
		return "macho_function_starts_command";
	case LC_FVMFILE:
		return NULL;
	case LC_ID_DYLIB:
		return "macho_id_dylib_command";
	case LC_ID_DYLINKER:
		return "macho_id_dylinker_command";
	case LC_IDENT:
		return NULL;
	case LC_IDFVMLIB:
		return NULL;
	...
	case LC_LOAD_WEAK_DYLIB:
	case LC_LOAD_DYLIB:
		return "macho_dylib_command";
	case LC_LOADFVMLIB:
		return NULL;
	case LC_MAIN:
		return "macho_entry_point_command";
	case LC_NOTE:
		return NULL;
	case LC_PREBIND_CKSUM:
		return NULL;
	...
	case LC_ROUTINES_64:
		return NULL;
	case LC_RPATH:
		return "macho_rpath_command";
	case LC_SEGMENT:
		return "macho_segment";
	case LC_SEGMENT_64:
		return "macho_segment64";
	case LC_SEGMENT_SPLIT_INFO:
		return "macho_segment_split_info_command";
	case LC_SOURCE_VERSION:
		return "macho_source_version_command";
	case LC_SUB_FRAMEWORK:
		return NULL;
	case LC_SUB_UMBRELLA:
		return NULL;
	...
	case LC_SUB_LIBRARY:
		return NULL;
	case LC_SYMTAB:
		return "macho_symtab_command";
	case LC_SYMSEG:
		return NULL;
	case LC_TWOLEVEL_HINTS:
		return NULL;
	case LC_UUID:
		return "macho_uuid_command";
	case LC_VERSION_MIN_MACOSX:
	case LC_VERSION_MIN_IPHONEOS:
	case LC_VERSION_MIN_TVOS:
	case LC_VERSION_MIN_WATCHOS:
		return "macho_version_min_command";
	case LC_THREAD:
		return NULL;
	case LC_UNIXTHREAD:
		return "macho_unixthread_command";
	case LC_AOT_METADATA:
		return "macho_aot_metadata";
	}
	return NULL;
}
...
static size_t get_word_size(struct MACHO_(obj_t) * mo) {
	const size_t word_size = MACHO_(get_bits) (mo) / 8;
	return R_MAX (word_size, 4);
}

static void free_chained_starts(struct MACHO_(obj_t) * mo) {
...
static bool parse_chained_fixups(struct MACHO_(obj_t) * mo, ut32 offset, ut32 size) {
...
static bool parse_threaded_rebase(struct MACHO_(obj_t) * mo) {
...
static int init_items(struct MACHO_(obj_t) * mo) {
	...
	ut64 cmds_begin = sizeof (struct MACHO_(mach_header)) + mo->header_at;
	ut64 cmds_end = 0;
	ut64 off = 0LL;
	int i, len;
	...
	RVecMachoLib_init (&mo->libs_cache);
	...
	for (i = 0, off = cmds_begin;
		...
		if (lc.cmdsize < 1 || !fits_in (mo->size, off, lc.cmdsize)) {
			R_LOG_WARN ("macho_header %d = cmdsize<1. (0x%" PFMT64x " vs 0x%" PFMT64x ")", i, off + lc.cmdsize, mo->size);
			break;
		}
		sdb_num_setf (mo->kv, off, 0, "macho_cmd_%d.offset", i);
		const char *format_name = cmd_to_pf_definition (lc.cmd);
		if (format_name) {
			sdb_setf (mo->kv, format_name, 0, "macho_cmd_%d.format", i);
		} else {
			sdb_setf (mo->kv, "[4]Ed (mach_load_command_type)cmd size", 0, "macho_cmd_%d.format", i);
		}

		snprintf (cmd_flagname, sizeof (cmd_flagname), "macho_cmd_%d.cmd", i);
		switch (lc.cmd) {
		case LC_DATA_IN_CODE:
		...
		case LC_ENCRYPTION_INFO:
			...
			{
				struct MACHO_(encryption_info_command) eic = { 0 };
				ut8 seic[sizeof (struct MACHO_(encryption_info_command))] = { 0 };
				if (off + sizeof (struct MACHO_(encryption_info_command)) > mo->size) {
					R_LOG_DEBUG ("encryption info out of bounds");
					return false;
				}
				if (r_buf_read_at (mo->b, off, seic, sizeof (struct MACHO_(encryption_info_command))) != -1) {
				...
				if (r_buf_read_at (mo->b, off, sdy, sizeof (struct dylinker_command)) != -1) {
					...
					if (name_off < dy.cmdsize && off + dy.cmdsize <= mo->size && dy.cmdsize < ST32_MAX) {
						...
						if (buf) {
							...
							buf[str_len] = 0;
							sdb_setf (mo->kv, buf, 0, "macho.dyld_environment.%d", i);
							free (buf);
						}
					}
				}
		...
		case LC_LOAD_DYLINKER:
			{
				...
				R_FREE (mo->intrp);
				R_LOG_DEBUG ("[macho] load dynamic linker");
				struct dylinker_command dy = { 0 };
				ut8 sdy[sizeof (struct dylinker_command)] = { 0 };
				if (off + sizeof (struct dylinker_command) > mo->size) {
		...
		case LC_MAIN:
			{
				...
				sdb_num_set (mo->kv, "macho.entry.offset", off + 8, 0);
		...
		case LC_SOURCE_VERSION:
			...
			// bprintf ("macho: TODO: Show source version\n");
			break;
		case LC_SEGMENT_SPLIT_INFO:
	...
	for (i = 0, off = sizeof (struct MACHO_(mach_header)) + mo->header_at;
		i < mo->hdr.ncmds;
		i++, off += lc.cmdsize) {
		len = r_buf_read_at (mo->b, off, loadc, sizeof (struct load_command));
		if (len < 1) {
		...
		if (lc.cmdsize < 1 || off + lc.cmdsize > mo->size) {
			R_LOG_DEBUG ("macho_header %d = cmdsize<1. (0x%" PFMT64x " vs 0x%" PFMT64x ")", i, (ut64) (off + lc.cmdsize), (ut64) (mo->size));
			break;
		}
		sdb_num_setf (mo->kv, off, 0, "macho_cmd_%d.offset", i);
		const char *format_name = cmd_to_pf_definition (lc.cmd);
...
static const char *macho_signing_class(struct MACHO_(obj_t) * mo);
...
static bool macho_has_restrict_section(struct MACHO_(obj_t) * mo) {
	ut32 i;
	for (i = 0; i < mo->nsects; i++) {
		struct MACHO_(section) *sect = &mo->sects[i];
...
static bool init(struct MACHO_(obj_t) * mo) {
	if (!init_hdr (mo)) {
		return false;
	}
	if (!init_items (mo)) {
	...
	mo->baddr = MACHO_(get_baddr) (mo);
	mo->libs_loaded = true;
	RVecMachoLib_shrink_to_fit (&mo->libs_cache);
	sdb_set (mo->kv, "macho.signing", macho_signing_class (mo), 0);
	if (mo->cs_present && mo->cs_flags) {
		sdb_num_set (mo->kv, "macho.signing.flags", mo->cs_flags, 0);
	}
	sdb_bool_set (mo->kv, "macho.app_extension_safe",
		(mo->hdr.flags & MH_APP_EXTENSION_SAFE) != 0, 0);
	sdb_bool_set (mo->kv, "macho.restrict", macho_has_restrict_section (mo), 0);
	return true;
}

void *MACHO_(macho_free)(struct MACHO_(obj_t) * mo) {
	if (!mo) {
		return NULL;
	}
	...
	free (mo->modtab);
	if (mo->libs_loaded) {
		RVecMachoLib_fini (&mo->libs_cache);
	}
	free (mo->func_start);
	free (mo->signature);
	free (mo->signature_der);
...
struct MACHO_(obj_t) * MACHO_(new_buf)(RBinFile *bf, RBuffer *buf, struct MACHO_(opts_t) * options) {
	...
	struct MACHO_(obj_t) *mo = R_NEW0 (struct MACHO_(obj_t));
	mo->b = r_ref (buf);
	mo->main_addr = UT64_MAX;
	RVecRBinReloc_init (&mo->reloc_fixups);
	mo->kv = sdb_new (NULL, "bin.macho", 0);
...
RVecSegment *MACHO_(get_segments_vec)(RBinFile *bf, struct MACHO_(obj_t) * mo) {
	...
	const int ws = R_BIN_MACHO_WORD_SIZE;
	if (mo->nsects > 0) {
...
RList *MACHO_(get_segments)(RBinFile *bf, struct MACHO_(obj_t) * macho) {
	...
	RVecSegment *segments = MACHO_(get_segments_vec) (bf, macho);
...
static bool parse_import_stub(struct MACHO_(obj_t) * bin, struct symbol_t *symbol, int idx) {
	...
	for (i = 0; i < bin->nsects; i++) {
		struct MACHO_(section) *sect = &bin->sects[i];
		if ((sect->flags & SECTION_TYPE) == S_SYMBOL_STUBS && sect->reserved2 > 0) {
			...
			for (j = 0; j < nsyms; j++) {
				...
				symbol->type = R_BIN_MACHO_SYMBOL_TYPE_LOCAL;
...
static char *get_name(struct MACHO_(obj_t) * mo, ut32 stridx, bool filter) {
	size_t i = 0;
...
static int walk_exports_trie(struct MACHO_(obj_t) * bin, ut64 trie_off, ut64 size, RExportsIterator iterator, void *ctx) {
	size_t count = 0;
	ut8 *p = NULL;
...
static int walk_exports(struct MACHO_(obj_t) * bin, RExportsIterator iterator, void *ctx) {
...
static void _update_main_addr_if_needed(struct MACHO_(obj_t) * mo, const RBinSymbol *sym) {
...
static void _enrich_symbol(RBinFile *bf, struct MACHO_(obj_t) * bin, HtPP *symcache, RBinSymbol *sym) {
	int wordsize = MACHO_(get_bits) (bin);
...
static bool has_nonruntime_public_symbols(struct MACHO_(obj_t) *mo) {
...
static void _fill_exports(struct MACHO_(obj_t) * mo, const char *name, ut64 flags, ut64 offset, void *ctx) {
	FillCtx *context = ctx;
...
static bool dysym_bounds(struct MACHO_(obj_t) * mo, size_t s, ut64 *f, ut64 *t) {
	ut32 from = 0;
	ut32 to = 0;
	struct dysymtab_command *ds = &mo->dysymtab;
	switch (s) {
...
static void parse_symbols(RBinFile *bf, struct MACHO_(obj_t) * mo, HtPP *symcache) {
	...
	j = 0; // symbol_idx
	mo->main_addr = UT64_MAX;
	int bits = MACHO_(get_bits_from_hdr) (&mo->hdr);
	...
	for (s = 0; s < 2; s++) {
		...
		for (i = from; i < to && j < symbols_count; i++) {
			struct MACHO_(nlist) *nl = &mo->symtab[i];
	...
	for (i = ds->iundefsym; i < to; i++) {
		...
		if (j > symbols_count) {
			R_LOG_WARN ("macho-get-symbols: error");
			break;
		}
		...
		if (parse_import_stub (mo, &symbol, i) && symbol.addr >= 100) {
			...
			sym->type = symbol.type == R_BIN_MACHO_SYMBOL_TYPE_LOCAL? "LOCAL": "EXT";
	...
	for (i = 0; i < mo->nsymtab && i < symbols_count; i++) {
		struct MACHO_(nlist) *st = &mo->symtab[i];
		if (st->n_type & N_STAB) {
			continue;
		}
		// 0 is for imports
...
static bool parse_function_start_symbols(RBinFile *bf, struct MACHO_(obj_t) * mo, HtPP *symcache) {
	RBinObject *obj = bf? bf->bo: NULL;
	if (!obj) {
		return false;
	}
	...
	int wordsize = MACHO_(get_bits) (mo);
...
static inline bool is_debug_build(RBinFile *bf, struct MACHO_(obj_t) *mo) {
	return RVecSegment_find (mo->segments_vec, NULL, is_debug_segment) != NULL;
}
#else
static bool is_debug_build(RBinFile *bf, struct MACHO_(obj_t) * mo) {
	RList *sections = MACHO_(get_segments) (bf, mo);
	if (!sections) {
		return false;
	}
...
static struct reloc_t *parse_import_ptr(struct MACHO_(obj_t) * mo, int jota) {
...
static void fill_import(RBin *rbin, const char *orig_name, RBinImport *imp) {
	...
	char *s = r_str_ndup (name, R_BIN_MACHO_STRING_LENGTH - 1);
...
static void check_for_special_import_names(struct MACHO_(obj_t) * bin, RBinImport *import) {
...
static void parse_relocation_info(struct MACHO_(obj_t) * mo, RSkipList *relocs, ut32 offset, ut32 num) {
...
static void walk_bind_chains(struct MACHO_(obj_t) * mo, RSkipList *relocs) {
	...
	MACHO_(iterate_chained_fixups) (mo, 0, UT64_MAX, R_FIXUP_EVENT_MASK_BIND_ALL, &walk_bind_chains_callback, &ctx);
...
static void macho_reloc_ref_fini(struct reloc_t **reloc) {
	free (*reloc);
}

R_VEC_TYPE_WITH_FINI(RVecRelocRef, struct reloc_t *, macho_reloc_ref_fini);
...
static HtPP *build_symbol_ordinal_cache(struct MACHO_(obj_t) * mo) {
...
static void insert_bind_reloc(struct MACHO_(obj_t) * mo, RVecRelocRef *threaded_binds, RBindOpState *state, ut8 op, ut8 rel_type) {
	...
	if (state->seg_idx >= 0) {
		struct MACHO_(segment_command) *seg = &mo->segs[state->seg_idx];
...
static void apply_threaded_bind(struct MACHO_(obj_t) * mo, RVecRelocRef *threaded_binds, RBindOpState *state, ut8 op, size_t wordsize) {
	if (!threaded_binds) {
		return;
	}
	int cur_seg_idx = (state->seg_idx != -1)? state->seg_idx: 0;
	struct MACHO_(segment_command) *cur_seg = &mo->segs[cur_seg_idx];
...
static bool parse_bind_op_threaded(struct MACHO_(obj_t) * mo, RVecRelocRef **threaded_binds, RBindOpState *state, ut8 op, ut8 imm, size_t wordsize, ut8 **p, ut8 *end) {
	switch (imm) {
	case BIND_SUBOPCODE_THREADED_SET_BIND_ORDINAL_TABLE_SIZE_ULEB:
		{
...
static bool parse_bind_op_do_bind(struct MACHO_(obj_t) * mo, RVecRelocRef **threaded_binds, RBindOpState *state, ut8 op, ut8 imm, ut8 rel_type, size_t wordsize, ut8 **p, ut8 *end) {
	switch (op) {
	case BIND_OPCODE_DO_BIND:
...
static bool parse_bind_op(struct MACHO_(obj_t) * mo, RVecRelocRef **threaded_binds, HtPP *ord_cache, RBindOpState *state, ut8 rel_type, size_t wordsize, bool in_lazy_binds, ut8 **p, ut8 *end) {
...
static bool _load_relocations(struct MACHO_(obj_t) * mo) {
	...
	if (mo->dyld_info) {
		...
		for (pidx = 0; pidx < R_ARRAY_SIZE (partition_sizes); pidx++) {
			...
			struct MACHO_(segment_command) *seg0 = &mo->segs[0];
			state.seg_idx = -1;
			state.sym_ord = -1;
...
const RSkipList *MACHO_(load_relocs)(struct MACHO_(obj_t) * mo) {
	R_RETURN_VAL_IF_FAIL (mo, NULL);
...
struct addr_t *MACHO_(get_entrypoint)(struct MACHO_(obj_t) * mo) {
	R_RETURN_VAL_IF_FAIL (mo, NULL);
	...
	entry->haddr = sdb_num_get (mo->kv, "macho.entry.offset", 0);
	sdb_num_set (mo->kv, "macho.entry.vaddr", entry->addr, 0);
	sdb_num_set (mo->kv, "macho.entry.paddr", mo->entry, 0);
	...
	if (entry->offset == 0 && mo->sects) {
		...
		for (i = 0; i < mo->nsects; i++) {
			...
			if (r_str_startswith (sect->sectname, "__text")) {
				...
				sdb_num_set (mo->kv, "macho.entry", entry->offset, 0);
...
void MACHO_(kv_loadlibs)(struct MACHO_(obj_t) * mo) {
	int i;
	for (i = 0; i < mo->nlibs; i++) {
		char **lib = RVecMachoLib_at (&mo->libs_cache, i);
		sdb_setf (mo->kv, lib? *lib: NULL, 0, "libs.%d.name", i);
	}
}

const RVecMachoLib *MACHO_(load_libs)(struct MACHO_(obj_t) * mo) {
	R_RETURN_VAL_IF_FAIL (mo, NULL);
	if (!mo->nlibs) {
		return NULL;
	}
	...
	MACHO_(kv_loadlibs) (mo);
	return &mo->libs_cache;
}

ut64 MACHO_(get_baddr)(struct MACHO_(obj_t) * mo) {
	int i;
...
char *MACHO_(get_class)(struct MACHO_(obj_t) * mo) {
#if R_BIN_MACHO64
	return strdup ("MACHO64");
#else
	return strdup ("MACHO");
#endif
}
...
int MACHO_(get_bits)(struct MACHO_(obj_t) * mo) {
	if (mo) {
		int bits = MACHO_(get_bits_from_hdr) (&mo->hdr);
...
bool MACHO_(is_big_endian)(struct MACHO_(obj_t) * mo) {
	if (mo) {
...
const char *MACHO_(get_intrp)(struct MACHO_(obj_t) * mo) {
	return mo? mo->intrp: NULL;
}

const char *MACHO_(get_os)(struct MACHO_(obj_t) * mo) {
	if (mo) {
		switch (mo->os) {
		case 1: return "macos";
		case 2: return "ios";
...
const char *MACHO_(get_cputype)(struct MACHO_(obj_t) * mo) {
	return mo? MACHO_(get_cputype_from_hdr) (&mo->hdr): "unknown";
}
...
char *MACHO_(get_cpusubtype)(struct MACHO_(obj_t) * mo) {
	return mo? MACHO_(get_cpusubtype_from_hdr) (&mo->hdr): strdup ("Unknown");
}

bool MACHO_(is_pie)(struct MACHO_(obj_t) * mo) {
...
bool MACHO_(has_nx)(struct MACHO_(obj_t) * mo) {
...
char *MACHO_(get_filetype)(struct MACHO_(obj_t) * mo) {
	return mo? MACHO_(get_filetype_from_hdr) (&mo->hdr): strdup ("Unknown");
}
...
static char *walk_codesig(RBinFile *bf, ut32 addr, ut32 size) {
	...
	struct MACHO_(obj_t) *bo = bf->bo->bin_obj;
	SuperBlob sblob = { 0 };
...
#define p(f, ...) r_strbuf_appendf(sb, f, ## __VA_ARGS__)
	...
	int n = 0;
	struct MACHO_(mach_header) *mh = MACHO_(get_hdr) (buf);
	if (!mh) {
		r_strbuf_free (sb);
		return NULL;
	}
	ut64 pvaddr = pa2va (bf, 0);
	p ("pf.macho_header @ 0x%08" PFMT64x "\n", pvaddr);
		...
		case LC_BUILD_VERSION:
			{
				...
				while (off < (lcSize - 8) && ntools--) {
					p ("pf.macho_build_version_tool @ 0x%08" PFMT64x "\n", pvaddr + off);
		...
		case LC_SEGMENT_64:
			{
				...
				while (off < lcSize && nsects--) {
					if (is64) {
						p ("pf.macho_section64 @ 0x%08" PFMT64x "\n", pvaddr - 8 + off);
						off += 80;
					} else {
						p ("pf.macho_section @ 0x%08" PFMT64x "\n", pvaddr - 8 + off);
						off += 68;
					}
				}
			}
...
RList *MACHO_(mach_fields)(RBinFile *bf) {
	...
	r_list_append (ret, r_bin_field_new (paddr, addr, -1, 1, "header", "macho_header", "macho_header", true));
	addr += 0x20 - 4;
	paddr += 0x20 - 4;
	bool is64 = mh->cputype >> 16;
	if (is64) {
		...
		case LC_BUILD_VERSION:
			{
				...
				while (off < lcSize && ntools--) {
					...
					RBinField *f = r_bin_field_new (at, addr + off, 1, -1, tool_flagname, "macho_build_version_tool", "macho_build_version_tool", true);
					r_list_append (ret, f);
					off += 8;
				}
				break;
		...
		case LC_SEGMENT_64:
			{
				...
				for (i = 0; i < nsects && (addr + off) < length && off < lcSize; i++) {
					const char *sname = is64? "macho_section64": "macho_section";
...
struct MACHO_(mach_header) * MACHO_(get_hdr)(RBuffer *buf) {
	ut8 hdr[sizeof (struct MACHO_(mach_header))] = { 0 };
	struct MACHO_(mach_header) *mh = R_NEW0 (struct MACHO_(mach_header));
	bool be = false;
	int len = r_buf_read_at (buf, 0, hdr, sizeof (hdr));
	if (len != sizeof (struct MACHO_(mach_header))) {
		free (mh);
		return NULL;
	}
	if (!magic_endian (r_read_be32 (hdr), &be)) {
		/* Keep parsing to support metadata extraction from non-macho slices. */
		be = false;
	}
...
void MACHO_(iterate_chained_fixups)(struct MACHO_(obj_t) * mo, ut64 limit_start, ut64 limit_end, ut32 event_mask, RFixupCallback callback, void *context) {
	int i;
