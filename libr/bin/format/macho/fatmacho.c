...
static bool r_bin_fatmacho_init(struct r_bin_fatmacho_obj_t* bin) {
	ut32 size;
	ut32 i;
...
struct r_bin_fatmacho_arch_t *r_bin_fatmacho_extract(struct r_bin_fatmacho_obj_t* bin, int idx, int *narch) {
	...
	struct r_bin_fatmacho_arch_t *ret = R_NEW0 (struct r_bin_fatmacho_arch_t);
...
struct r_bin_fatmacho_obj_t* r_bin_fatmacho_new(const char* file) {
	struct r_bin_fatmacho_obj_t *bin = R_NEW0 (struct r_bin_fatmacho_obj_t);
	bin->file = file;
	size_t binsz;
	ut8 *buf = (ut8 *)r_file_slurp (file, &binsz);
	if (!buf) {
		return r_bin_fatmacho_free (bin);
	}
	bin->size = binsz;
	bin->b = r_buf_new ();
...
struct r_bin_fatmacho_obj_t* r_bin_fatmacho_from_buffer_new(RBuffer *b) {
	...
	struct r_bin_fatmacho_obj_t *bo = R_NEW0 (struct r_bin_fatmacho_obj_t);
	if (bo) {
		bo->b = r_ref (b);
		bo->size = r_buf_size (bo->b); // XXX implicit in bo->b
		if (!r_bin_fatmacho_init (bo)) {
			return r_bin_fatmacho_free (bo);
		}
	}
	return bo;
}
