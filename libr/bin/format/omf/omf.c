...
int r_bin_checksum_omf_ok(const char *buf, ut64 buf_size) {
	...
	if (buf_size < 3) {
		R_LOGFI ("Invalid record (too short)\n");
		return false;
	}
	size = ut8p_bw (buf + 1);
	if (buf_size < size + 3) {
	...
	if (checksum) {
		// R_LOGFI ("Invalid record checksum\n");
	}
	return !checksum ? true : false;
}

static ut16 omf_get_idx(const char *buf, int buf_size) {
	if (buf_size < 2) {
		return 0;
	}
	if (*buf & 0x80) {
...
static int load_omf_lnames(OMF_record *record, const char *buf, ut64 buf_size) {
	...
	while ((int)tmp_size < (int)(record->size - 1)) {
		if (ct_name >= ret->nb_elem) {
			R_LOGFI ("load_omf_lnames: prevent overflow\n");
			break;
		}
		...
		if (record->size + 3 < tmp_size + cb) {
			R_LOGFI ("Invalid Lnames record (bad size)\n");
			free (ret);
			return false;
		}
...
static int load_omf_segdef(OMF_record *record, const char *buf, ut64 buf_size) {
	...
	if (record->size < 2) {
		R_LOGFI ("Invalid Segdef record (bad size)\n");
		return false;
	}
...
static int load_omf_symb(OMF_record *record, ut32 ct, const char *buf, int buf_size, int bits, ut16 seg_idx) {
	ut32 nb_symb = 0;
	ut8 str_size = 0;
	OMF_symbol *symbol;
	...
	while (nb_symb < ((OMF_multi_datas *)record->content)->nb_elem) {
		...
		if (record->size - 1 < ct - 2) {
			R_LOGFI ("Invalid Pubdef record (bad size)\n");
			return false;
		}
		str_size = buf[ct];
...
static int load_omf_pubdef(OMF_record *record, const char *buf, int buf_size) {
	OMF_multi_datas *ret = NULL;
	ut16 seg_idx;
	ut16 ct = 0;
	ut16 base_grp;
...
static int load_omf_data(const char *buf, int buf_size, OMF_record *record, ut64 global_ct) {
	ut16 seg_idx;
	ut32 offset;
	ut16 ct = 4;
	OMF_data *ret;
	...
	if ((!(record->type & 1) && record->size < 4) || (record->size < 6)) {
		R_LOGFI ("Invalid Ledata record (bad size)\n");
		return false;
	}
...
static int load_omf_content(OMF_record *record, const char *buf, ut64 global_ct, ut64 buf_size) {
	...
	if (!record->size) {
		R_LOGFI("Invalid record (size to short)\n");
		return false;
	}
...
static int get_omf_data_info(r_bin_omf_obj *obj) {
	...
	while ((tmp = get_next_omf_record_type (tmp, OMF_LEDATA))) {
		if (((OMF_data *)((OMF_record *)tmp)->content)->seg_idx - 1 >= obj->nb_section) {
			R_LOGFI ("Invalid Ledata record (bad segment index)\n");
			return false;
		}
	...
	while (ct_sym < obj->nb_symbol) {
		if (!strcmp (obj->symbols[ct_sym]->name, "_start")) {
			if (obj->symbols[ct_sym]->seg_idx - 1 > obj->nb_section) {
				R_LOGFI ("Invalid segment index for symbol _start\n");
				return false;
			}
...
ut64 r_bin_omf_get_vaddr_sym(r_bin_omf_obj *obj, OMF_symbol *sym) {
	...
	if (sym->seg_idx - 1 > obj->nb_section) {
		R_LOGFI ("Invalid segment index for symbol %s\n", sym->name);
		return 0;
	}
