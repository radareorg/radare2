...
struct r_bin_mdmp_obj *r_bin_mdmp_new_buf(struct r_buf_t *buf)
{
	...
	if (!r_buf_set_bytes(obj->b, buf->buf, buf->length)) {
		eprintf("Error in r_bin_mdmp_new_buf: r_buf_set_bytes failed\n");
		r_bin_mdmp_free(obj);
		return NULL;
	}
...
int r_bin_mdmp_create_lists(struct r_bin_mdmp_obj *obj)
{
	if (!(obj->streams = r_list_new()))
		return R_FALSE;

	if (!(obj->threads = r_list_new()))
		return R_FALSE;

	if (!(obj->thread_info = r_list_new()))
		return R_FALSE;

	if (!(obj->threads_ex = r_list_new()))
		return R_FALSE;

	if (!(obj->modules = r_list_new()))
		return R_FALSE;

	if (!(obj->unloaded_modules = r_list_new()))
		return R_FALSE;

	if (!(obj->memory = r_list_new()))
		return R_FALSE;

	if (!(obj->memory64 = r_list_new()))
		return R_FALSE;

	if (!(obj->memory_info = r_list_new()))
		return R_FALSE;

	if (!(obj->exceptions = r_list_new()))
		return R_FALSE;

	if (!(obj->comments_a = r_list_new()))
		return R_FALSE;

	if (!(obj->comments_w = r_list_new()))
		return R_FALSE;

	if (!(obj->handle_streams = r_list_new()))
		return R_FALSE;

	if (!(obj->function_tables = r_list_new()))
		return R_FALSE;

	if (!(obj->misc_info = r_list_newf(free)))
		return R_FALSE;

	if (!(obj->handle_operations = r_list_new()))
		return R_FALSE;

	if (!(obj->token_info = r_list_new()))
		return R_FALSE;
