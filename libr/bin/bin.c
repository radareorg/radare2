...
static bool r_bin_file_object_new_from_xtr_data(RBin *bin, RBinFile *bf,
	...
	if (!idx && xtr && bytes) {
		...
		if (xtr_data) {
			if (!r_bin_file_object_new_from_xtr_data (
				    bin, bf, baseaddr, loadaddr, xtr_data)) {
				eprintf (
					"Error: failed to load the Extracted "
					"Objects with %s for %s.\n",
					xtr->name, bf->file);
			}
		}
