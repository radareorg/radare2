...
R_API RBin *r_bin_new(void) {
	...
	for (int i = (sizeof (bin_static_plugins) / sizeof (RBinPlugin *)) - 1; i;) {
	...
	if (bin->binxtrs) {
		...
		for (int i = 0; bin_xtr_static_plugins[i]; i++) {
	...
	if (bin->binldrs) {
		...
		for (int i = 0; bin_ldr_static_plugins[i]; i++) {
