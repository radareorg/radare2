...
typedef struct {
	...
	bool chunk_valid;
	bool failed;
	struct MACHO_(obj_t) *obj;
} RFixupRebaseContext;

extern RBinWrite r_bin_write_macho;

static bool rebase_buffer_callback2(void * context, RFixupEventDetails * event_details);
static RBinInfo *info(RBinFile *bf);
static RBuffer *swizzle_io_read(RBinFile *bf, struct MACHO_(obj_t) *obj, RIO *io);

#define IS_PTR_AUTH(x) ((x & (1ULL << 63)) != 0)
#define IS_PTR_BIND(x) ((x & (1ULL << 62)) != 0)

static Sdb *get_sdb(RBinFile *bf) {
	struct MACHO_(obj_t) *mo = (struct MACHO_(obj_t) *) R_UNWRAP3 (bf, bo, bin_obj);
	return mo? mo->kv: NULL;
}

static char *entitlements(RBinFile *bf, bool json) {
	struct MACHO_(obj_t) *mo = R_UNWRAP3 (bf, bo, bin_obj);
	if (!mo) {
		return NULL;
	}
...
static bool load(RBinFile *bf, RBuffer *buf, ut64 laddr) {
	...
	struct MACHO_(obj_t) *mo = MACHO_(new_buf) (bf, buf, &opts);
	if (mo) {
		bf->bo->bin_obj = mo;
...
static void process_constructors(RBinFile *bf, RList *ret, int bits, int limit) {
	struct MACHO_(obj_t) *mo = bf->bo->bin_obj;
	RVecSegment *secs = MACHO_(get_segments_vec) (bf, mo);
	RBinSection *sec;
	int i, type;
...
static RList *entries(RBinFile *bf) {
	...
	RBinObject *bo = bf->bo;
	struct MACHO_(obj_t) *mo = bo->bin_obj;
	...
	int bits = MACHO_(get_bits) (mo);
	if (!(entry = MACHO_(get_entrypoint) (mo))) {
		return ret;
	}
	RBinAddr *ptr = R_NEW0 (RBinAddr);
...
static bool imports_vec(RBinFile *bf) {
	...
	struct MACHO_(obj_t) *mo = bo->bin_obj;
	RVecRBinImport *imports = MACHO_(load_imports) (bf, mo);
	if (!imports) {
		return false;
	}
...
static RList *relocs(RBinFile *bf) {
	R_RETURN_VAL_IF_FAIL (bf && bf->bo && bf->bo->bin_obj, NULL);
	struct MACHO_(obj_t) *mo = bf->bo->bin_obj;
	const RSkipList *relocs = MACHO_(load_relocs) (mo);
	if (!relocs) {
		return NULL;
	}
...
static RList *libs(RBinFile *bf) {
	...
	const RVecMachoLib *libs = MACHO_(load_libs) (obj->bin_obj);
	if (!libs) {
		return NULL;
	}
...
static RBinInfo *info(RBinFile *bf) {
	...
	ret->bclass = MACHO_(get_class) (mo);
	...
	const char *intrp = MACHO_(get_intrp) (mo);
	ret->intrp = intrp? strdup (intrp): NULL;
	ret->compiler = strdup ("clang");
	ret->rclass = strdup ("macho");
	ret->os = strdup (MACHO_(get_os) (mo));
	ret->subsystem = strdup ("darwin");
	ret->arch = strdup (MACHO_(get_cputype) (mo));
	ret->machine = MACHO_(get_cpusubtype) (mo);
	ret->has_lit = true;
	ret->type = MACHO_(get_filetype) (mo);
	ret->big_endian = MACHO_(is_big_endian) (mo);
	ret->has_crypto = mo->has_crypto;
	ret->bits = MACHO_(get_bits) (mo);
	ret->has_va = true;
	ret->has_pi = MACHO_(is_pie) (mo);
	ret->has_nx = MACHO_(has_nx) (mo);
	return ret;
}

static bool _patch_reloc(struct MACHO_(obj_t) *mo, RIOBind *iob, struct reloc_t *reloc, ut64 symbol_at) {
	ut64 pc = reloc->addr;
	ut64 ins_len = 0;
	...
	default:
		R_LOG_WARN ("unsupported architecture for patching relocs, please file a bug. %s",
				MACHO_(get_cputype_from_hdr)(&mo->hdr));
		return false;
	}
...
static RList* patch_relocs(RBinFile *bf) {
	...
	struct MACHO_(obj_t) *mo = obj->bin_obj;

	const RSkipList *all_relocs = MACHO_(load_relocs)(mo);
	if (!all_relocs) {
		return NULL;
	}
...
#define MACHO_SWIZZLE_DEFAULT_PAGE_SIZE 0x1000

static ut64 swizzle_max_fixup_page_size(struct MACHO_(obj_t) *obj) {
	ut64 chunk_capacity = MACHO_SWIZZLE_DEFAULT_PAGE_SIZE;
	...
	for (i = 0; i < count; i++) {
		...
		ut64 page_size = starts->page_size? starts->page_size: MACHO_SWIZZLE_DEFAULT_PAGE_SIZE;
...
static RBuffer *swizzle_io_read(RBinFile *bf, struct MACHO_(obj_t) *obj, RIO *io) {
	...
	MACHO_(iterate_chained_fixups) (obj, off, off + count,
...
static bool rebase_buffer_callback2(void *context, RFixupEventDetails * event_details) {
	...
	struct MACHO_(obj_t) *obj = ctx->obj;
...
static RList *classes(RBinFile *bf) {
	// 8s / 16s
	return MACHO_(parse_classes) (bf, NULL);
}

static bool load_resources(RBinFile *bf) {
	R_RETURN_VAL_IF_FAIL (bf && bf->bo && bf->bo->bin_obj, false);
	struct MACHO_(obj_t) *mo = bf->bo->bin_obj;
	RVecSegment *sections = MACHO_(get_segments_vec) (bf, mo);
	if (!sections) {
		return false;
	}
	ut32 index = 0;
...
#ifndef R_BIN_MACHO64
	if (opt->bits == 64) {
		R_LOG_TODO ("Please use macho64 instead of macho");
		free (buf);
		return NULL;
	}
#endif
...
RBinPlugin r_bin_plugin_macho = {
	...
	.size = &size,
	.info = &info,
	.header = MACHO_(mach_headerfields),
	.fields = MACHO_(mach_fields),
	.libs = &libs,
	.relocs = &relocs,
	.patch_relocs = &patch_relocs,
	.create = &create,
	.classes = &classes,
	.write = &r_bin_write_macho,
};

#ifndef R2_PLUGIN_INCORE
R_API RLibStruct radare_plugin = {
	.type = R_LIB_TYPE_BIN,
	.data = &r_bin_plugin_macho,
	.version = R2_VERSION
};
#endif
#endif
