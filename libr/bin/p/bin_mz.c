...
static void header(RBinFile *bf) {
	...
	R_LOGFI("[0000:0000]  Signature           %c%c\n",
		mz->dos_header->signature & 0xFF,
		mz->dos_header->signature >> 8);
	R_LOGFI("[0000:0002]  BytesInLastBlock    0x%04x\n",
	   mz->dos_header->bytes_in_last_block);
	R_LOGFI("[0000:0004]  BlocksInFile        0x%04x\n",
	    mz->dos_header->blocks_in_file);
	R_LOGFI("[0000:0006]  NumRelocs           0x%04x\n",
	    mz->dos_header->num_relocs);
	R_LOGFI("[0000:0008]  HeaderParagraphs    0x%04x\n",
	    mz->dos_header->header_paragraphs);
	R_LOGFI("[0000:000a]  MinExtraParagraphs  0x%04x\n",
	    mz->dos_header->min_extra_paragraphs);
	R_LOGFI("[0000:000c]  MaxExtraParagraphs  0x%04x\n",
	    mz->dos_header->max_extra_paragraphs);
	R_LOGFI("[0000:000e]  InitialSs           0x%04x\n",
	    mz->dos_header->ss);
	R_LOGFI("[0000:0010]  InitialSp           0x%04x\n",
	    mz->dos_header->sp);
	R_LOGFI("[0000:0012]  Checksum            0x%04x\n",
	    mz->dos_header->checksum);
	R_LOGFI("[0000:0014]  InitialIp           0x%04x\n",
	    mz->dos_header->ip);
	R_LOGFI("[0000:0016]  InitialCs           0x%04x\n",
	    mz->dos_header->cs);
	R_LOGFI("[0000:0018]  RelocTableOffset    0x%04x\n",
	    mz->dos_header->reloc_table_offset);
	R_LOGFI("[0000:001a]  OverlayNumber       0x%04x\n",
