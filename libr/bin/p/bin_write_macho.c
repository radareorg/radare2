...
static MachoPointers findLastCommand(RBinFile *bf) {
	struct MACHO_(obj_t) *bin = bf->bo->bin_obj;
	int i = 0;
	ut64 off;
	MachoPointers mp = {0};
...
static bool MACHO_(write_addlib)(RBinFile *bf, const char *lib) {
...
static bool addlib(RBinFile *bf, const char *lib) {
	return MACHO_(write_addlib) (bf, lib);
}
...
static bool seg_perms(RBinFile *bf, const char *name, int perms) {
	...
	ut64 off = sizeof (struct MACHO_(mach_header)) + mo->header_at;
	...
	for (i = 0; i < ncmds; i++) {
		...
		if (cmd == seg_lc) {
			...
			if (seg_name_matches (mo->segs[seg_idx].segname, name)) {
				...
				ut64 maxprot_off = off + r_offsetof (struct MACHO_(segment_command), maxprot);
				ut64 initprot_off = off + r_offsetof (struct MACHO_(segment_command), initprot);
