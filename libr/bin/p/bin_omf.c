...
static void *load_bytes(RBinFile *arch, const ut8 *buf, ut64 size,
			ut64 loadaddrn, Sdb *sdb) {
...
static int load(RBinFile *arch) {
  ...
  if (!arch || arch->o)
    return R_FALSE;
  if (!(arch->o->bin_obj = load_bytes(arch, byte, size, arch->o->loadaddr, arch->sdb)))
    return R_FALSE;
...
static int check_bytes(const ut8 *buf, ut64 length) {
  if ((*buf == 0x80 || *buf == 0x82) && r_bin_checksum_omf_ok((char *)buf))
    return R_TRUE;
...
static RList *sections(RBinFile *arch) {
  ...
  while (ct_omf_sect < obj->nb_section)
    if (!r_bin_omf_send_sections(ret, obj->sections[ct_omf_sect++], arch->o->bin_obj))
      return ret;
  return ret;
}
