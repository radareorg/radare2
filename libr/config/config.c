...
static bool config_rm_node(RConfig *cfg, RConfigNode *node) {
	if (node) {
...
static RConfigNode *create_node(RConfig *cfg, const char *name, const char *value) {
	RConfigNode *node = r_config_node_new (name, value);
	if (!node) {
	...
	if (is_bool (node->value)) {
		node->flags |= CN_BOOL;
		node->i_value = is_true (node->value)? 1: 0;
	}
	return node;
}
...
R_API RConfigNode* r_config_set(RConfig *cfg, const char *name, const char *value) {
	...
	RConfigNode *onode = r_config_node_get (cfg, name);
	// do not allow to add new config keys if RConfig has been locked
	if (!onode && cfg->lock) {
		eprintf ("r_config_set: RConfig has been locked! Cannot create new config '%s'.\n", name);
		return NULL;
	}

	RConfigNode *node = create_node (cfg, name, value);
	if (!node) {
		return NULL;
	}

	// run the setter, if present, and continue only if it returns true
	if (onode && onode->setter) {
		int ret = onode->setter (cfg->user, node);
		if (ret == false) {
	...
	config_rm_node (cfg, onode);

	// insert the new node in the list/ht
...
R_API bool r_config_rm(RConfig *cfg, const char *name) {
	RConfigNode *node = r_config_node_get (cfg, name);
	return config_rm_node (cfg, node);
}
