...
void decode_addressing_modes(tms320_dasm_t *dasm) {
	...
	if (field_valid (dasm, AAAAAAAI)) {
		...
		if (field_value (dasm, AAAAAAAI) & 1) {
			if (strstr (tmp, "k16")) {
				substitute (tmp, "k16", "0x%04X", r_read_be16 (dasm->stream + dasm->length));
				dasm->length += 2;
			} else if (strstr (tmp, "k23")) {
				substitute (tmp, "k23", "0x%06X", be24 (r_read_le32 (dasm->stream + dasm->length) & 0x7FFFFF));
				dasm->length += 3;
...
static void init_dasm(tms320_dasm_t *dasm, const ut8 *stream, int len) {
	...
	dasm->opcode64 = (tms320_f_get_cpu (dasm) == TMS320_F_CPU_C64X)? r_read_be64 (dasm->stream): r_read_le64 (dasm->stream);
	dasm->head = NULL;
	dasm->insn = NULL;
}
