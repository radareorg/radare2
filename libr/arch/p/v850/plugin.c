...
static int v850e0_op(RArchSession *a, RAnalOp *op, ut64 addr, const ut8 *buf, int len, RAnalOpMask mask) {
	...
	case V850_TST:
		op->type = R_ANAL_OP_TYPE_CMP;
		// compare (a & b) against 0 to set esil->cur so $s/$z reflect the AND
	...
	case V850_SHR_IMM5:
		op->type = R_ANAL_OP_TYPE_SHR;
		{
			...
			reg2 = F2_RN2 (word1);
			// CY = last bit shifted out = bit(shift-1) of original reg2; compute before the shift
			if (shift) {
				r_strbuf_appendf (&op->esil, "%s,%u,>>,1,&,cy,:=,%u,%s,>>=", reg2, shift - 1, shift, reg2);
			} else {
				r_strbuf_appendf (&op->esil, "0,cy,:=,0,%s,>>=", reg2);
			}
		}
		update_flags (op, V850_FLAG_S | V850_FLAG_Z);
		clear_flags (op, V850_FLAG_OV);
	...
	case V850_SAR_IMM5:
		op->type = R_ANAL_OP_TYPE_SAR;
		{
			...
			reg2 = F2_RN2(word1);
			// CY = bit(imm5-1) of original reg2; compute before the shift
			if (imm5) {
	...
	case V850_SHL_IMM5:
		op->type = R_ANAL_OP_TYPE_SHL;
		{
			ut8 shift = (ut8)F2_IMM(word1);
			reg2 = F2_RN2(word1);
			// CY = bit(32-shift) of original reg2; compute before the shift
			if (shift) {
				r_strbuf_appendf (&op->esil, "%s,%u,>>,1,&,cy,:=,%u,%s,<<=", reg2, 32 - shift, shift, reg2);
			} else {
				r_strbuf_appendf (&op->esil, "0,cy,:=,0,%s,<<=", reg2);
			}
		}
	...
	case V850_EXT1:
		...
		case V850_EXT_SHL:
			op->type = R_ANAL_OP_TYPE_SHL;
			// CY = bit(32-reg1) of original reg2; precompute using the register before the shift. For shift==0, 32-0=32 overshifts and esil_lsr clamps to 63 yielding 0, so CY=0 per V850 spec
			r_strbuf_appendf (&op->esil, "%s,32,%s,-,>>,1,&,cy,:=,%s,%s,<<=",
				F9_RN2 (word1), F9_RN1 (word1), F9_RN1 (word1), F9_RN2 (word1));
			update_flags (op, V850_FLAG_S | V850_FLAG_Z);
			clear_flags (op, V850_FLAG_OV);
			break;
		case V850_EXT_SHR:
			op->type = R_ANAL_OP_TYPE_SHR;
			// CY = bit(reg1-1) of original reg2; for shift==0, reg1-1 underflows to UT64_MAX, esil_lsr clamps to 63 yielding 0, so CY=0 per V850 spec
			r_strbuf_appendf (&op->esil, "%s,%s,1,-,>>,1,&,cy,:=,%s,%s,>>=",
				F9_RN2 (word1), F9_RN1 (word1), F9_RN1 (word1), F9_RN2 (word1));
			update_flags (op, V850_FLAG_S | V850_FLAG_Z);
			clear_flags (op, V850_FLAG_OV);
			break;
		case V850_EXT_SAR:
			...
			reg1 = F9_RN1 (word1);
			reg2 = F9_RN2 (word1);
			// CY = bit(reg1-1) of original reg2; compute first (before reg2 is overwritten)
			r_strbuf_appendf (&op->esil, "%s,%s,1,-,>>,1,&,cy,:=,", reg2, reg1);
