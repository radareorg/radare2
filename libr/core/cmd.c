...
next_arroba:
		...
		if (ptr[1]=='@') {
			...
			bool tmpseek = false;
			char *fromvars[] = { "anal.from", "diff.from", "graph.from", "io.buffer.from",
				 "lines.from", "search.from", "zoom.from", NULL };
			char *tovars[] = { "anal.to", "diff.to", "graph.to", "io.buffer.to",
				"lines.to", "search.to", "zoom.to", NULL };
			...
			if (ptr[1] == '.' && ptr[2] == '.') {
				...
				*p = '\x00';
				...
				// save current ranges
				for (i = 0; fromvars[i] != NULL; i++) {
					curfrom[i] = r_config_get_i (core->config, fromvars[i]);
				}
				for (i = 0; tovars[i] != NULL; i++) {
