...
static RCoreHelpMessage help_msg_echo = {
	"Usage:", "echo [-n] [arg ...]", "",
	"echo", " [arg ...]", "print decoded arguments followed by a newline",
	"echo -n", " [arg ...]", "print decoded arguments without a newline",
	"echo?", "", "show this help",
	NULL
};
...
static RCmdResult echo_callback(RCmdContext *ctx) {
	...
	if (r_strs_equals_str (ctx->subcmd, "64")) {
		...
		size_t length = nul? nul - decoded: size;
		if (length) {
	...
	if (ctx->raw) {
		...
		// Decode the whole tail in place of joining the parsed args, so
		// the spacing between words is preserved. Backslash escapes are
		// decoded, quotes removed and single-quoted spans kept literal
		const char *src = r_str_trim_head_ro (ctx->subcmd.b);
		if (r_str_startswith (src, "-n") && (!src[2] || isspace ((ut8)src[2]))) {
			newline = false;
			src = r_str_trim_head_ro (src + 2);
		}
		const char *end = src + strlen (src);
		char *buf = malloc (end - src + 1);
		if (!buf) {
			return (RCmdResult) { .status = 1 };
		}
		char *dst = buf;
		...
		while (src < end) {
			const char ch = *src++;
			if (quote == '\'') {
				// single quotes keep escapes literal, except for the
				// quote itself and the backslash
				if (ch == '\\' && src < end && (*src == '\'' || *src == '\\')) {
					*dst++ = *src++;
				} else if (ch == '\'') {
					quote = 0;
				} else {
			...
			if (ch == '\\') {
				src = r_cmd_decode_escape (src, end, &dst);
				continue;
			}
			if (ch == '\'' || ch == '"') {
				if (!quote) {
		...
		size_t length = dst - buf;
		const char *nul = memchr (buf, 0, length);
		length = nul? nul - buf: length;
		if (length) {
			r_cons_write (ctx->cons, buf, length);
		}
		free (buf);
	}
	if (newline) {
...
// eprintf ("JAJ %s %d\n", cmd_prefix, core->cons->context->buffer_len);
	...
	r_cons_print (r_core_get_cons (core), s);
...
static int cmd_join(void *data, const char *input) { // "join"
	...
	*end = '\0';
	const char *arg2 = r_str_trim_head_ro (end + 1);
	if (!*arg2) {
		goto out;
	}
	switch (*input) {
...
static DWORD WINAPI cons_flush_thread(LPVOID param) {
	r_cons_flush (param);
	return 0;
}
...
static void r_w32_cmd_pipe(RCore *core, char *radare_cmd, char *shell_cmd) {
	...
	HANDLE th = CreateThread (NULL, 0, cons_flush_thread, cons, 0, NULL);
...
R_API int r_core_cmd_pipe(RCore *core, char *radare_cmd, char *shell_cmd) {
	...
	const bool si = r_cons_is_interactive (r_core_get_cons (core));
	r_config_set_b (core->config, "scr.interactive", false);
	int pipecolor = -1;
...
R_API int r_core_cmd_foreach3(RCore *core, const char *cmd, char *each) { // "@@@"
	...
	case '=': // "@@@="
		...
		break;
	case '?': // "@@@?"
		r_cons_cmd_help (cons, help_msg_at_at_at);
		break;
	case 'c': // "@@@c"
		if (glob) {
	...
	case 'i':
		{
			...
			r_list_foreach (list, iter, item) {
				...
				if (item->name) {
					r_cons_printf (cons, "%s: ", item->name);
				}
...
R_API int r_core_cmd_foreach(RCore *core, const char *cmd, char *each) {
	...
	r_cons_break_push (cons, NULL, NULL); //pop on return
	switch (each[0]) {
	case '/': // "@@/"
		{
	...
	case 0:
		...
		break;
	case '?': // "@@?"
		r_cons_cmd_help (cons, help_msg_at_at);
		break;
		...
		while (str[i]) {
			...
			str[i] = ch;
			{
				...
				r_list_foreach (match_flag_items, iter, flag) {
					...
					r_cons_push (cons);
					r_core_cmd (core, cmd, 0);
					tmp = r_cons_get_buffer (cons, NULL);
...
static char *core_cmd_str_context(RCore *core, RCmdContext *parent, const char *cmd) {
	RCons *cons = parent? parent->cons: core_cmd_cons (core);
...
R_API RBuffer *r_core_cmd_tobuf(RCore *core, const char *cmd) {
	...
	r_cons_filter (cons);
	size_t bsz;
	const char *buf = r_cons_get_buffer (cons, &bsz);
...
R_API void r_core_cmd_init(RCore *core) {
	...
	if (core->rcmd) {
		...
		if (!r_cmd_register (core->rcmd, "echo", echo_callback, NULL)
			|| !r_cmd_register (core->rcmd, "?e", question_echo_callback, NULL)) {
			r_cmd_unregister (core->rcmd, "echo");
