...
static RCmdResult echo_callback(RCmdContext *ctx) {
	...
	if (!ctx->raw) {
		RStrs *arg = RVecRStrs_at (&ctx->args, 0);
		if (arg && r_strs_equals_str (*arg, "-n")) {
			first_arg++;
			newline = false;
		}
	}
	if (ctx->raw) {
		...
		size_t i;
		for (i = first_arg; i < RVecRStrs_length (&ctx->args); i++) {
			RStrs *arg = RVecRStrs_at (&ctx->args, i);
			if (i > first_arg) {
				r_cons_print (ctx->cons, " ");
			}
			size_t length = r_strs_len (*arg);
			const char *nul = memchr (arg->a, 0, length);
			length = nul? nul - arg->a: length;
			if (length) {
				r_cons_write (ctx->cons, arg->a, length);
			}
			if (nul) {
				break;
			}
