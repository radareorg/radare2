...
bool ranal2_list(RCore *core, const char *arch, int fmt) {
	...
	if (R_STR_ISNOTEMPTY (arch)) {
		r_list_foreach (r_anal_plugins (a), iter, h) {
		...
		if (!any) {
			...
			RArchPlugin *arp;
			r_list_foreach (r_arch_plugins (ai), iter, arp) {
...
static void update_archdecoder_options(RCore *core, RConfigNode *node) {
	...
	RListIter *it;
	RArchPlugin *ap;
	r_list_foreach (r_arch_plugins (core->anal->arch), it, ap) {
		if (ap->meta.name) {
...
static void update_asmcpu_options(RCore *core, RConfigNode *node) {
	...
	r_config_node_purge_options (node);
	RArchPlugin *h;
	r_list_foreach (r_arch_plugins (core->anal->arch), iter, h) {
...
static void update_asmparser_options(RCore *core, RConfigNode *node) {
	RListIter *iter;
	RList *plugins = core && core->rasm? r_asm_sessions (core->rasm): NULL;
...
static bool cb_cmdpdc(void *user, void *data) {
	...
	if (*node->value == '?') {
		...
		/* Check for r2sleigh analysis plugin with decompiler support */
		{
			RListIter *it;
			RAnalPlugin *ap;
			r_list_foreach (r_anal_plugins (core->anal), it, ap) {
...
static bool cb_anal_types_parser(void *user, void *data) {
	...
	if (!strcmp (node->value, "?")) {
		RAnalPlugin *p;
		RListIter *iter;
		r_list_foreach (r_anal_plugins (core->anal), iter, p) {
