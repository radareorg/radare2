...
static int cb_searchin(void *user, void *data) {
	...
	if (node->value[0] == '?') {
		if (strlen (node->value) > 1 && node->value[1] == '?') {
			...
			"io.maps.write     search in all writable marked maps\n"
			"io.maps.exec      search in all executable marked maps\n"
			"io.maps.readonly  search in all readonly marked maps\n"
			...
			"io.sections.readonly  search in all readonly marked sections\n"
