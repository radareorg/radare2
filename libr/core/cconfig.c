...
// TODO: This should be specified at first by the debug backend when attaching
	...
	/* rop */
	SETI ("rop.len", 5, "maximum gadget length");
	SETB ("rop.sdb", "false", "cache results in sdb (experimental)");
	SETB ("rop.db", "true", "categorize gadgets in sdb");
	SETB ("rop.subchains", "false", "display every length gadget from rop.len=X to 2 in /Gl");
	SETB ("rop.cond", "false", "include conditional jump, calls and returns in gadget search");
	SETB ("rop.comments", "false", "display comments in gadget search output");
