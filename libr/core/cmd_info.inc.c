...
static RCoreHelpMessage help_msg_iz = {
	...
	"iz-", " ([addr]) ([len]) ([type])", "delete string (or current seek)",
...
static void parse_iz_args(RCore *core, const char *input, int skip, IzArgs *args) {
	args->addr = core->addr;
	args->len = 0;
	args->type = 0;
	if (input[skip] == ' ') {
		char *s = strdup (r_str_trim_head_ro (input + skip + 1));
		int nwords = r_str_word_set0 (s);
		if (nwords > 0) {
			const char *a0 = r_str_word_get0 (s, 0);
			if (R_STR_ISNOTEMPTY (a0)) {
				args->addr = r_num_get (NULL, a0);
			}
		}
		if (nwords > 1) {
			const char *a1 = r_str_word_get0 (s, 1);
			if (R_STR_ISNOTEMPTY (a1)) {
				args->len = r_num_get (NULL, a1);
			}
		}
		if (nwords > 2) {
			const char *a2 = r_str_word_get0 (s, 2);
			if (R_STR_ISNOTEMPTY (a2)) {
				args->type = *a2;
			}
		}
		free (s);
...
static void cmd_izplus(RCore *core, const char *input) {
	...
	IzArgs args;
	parse_iz_args (core, input, 2, &args);
	ut64 paddr = r_io_v2p (core->io, args.addr);
	ut64 vaddr = args.addr;
	if (paddr == UT64_MAX) {
		paddr = args.addr;
	}
	RBinString *bs = r_bin_file_string_add (bf, paddr, vaddr, args.len, args.type);
	if (bs) {
		R_LOG_INFO ("Added string at 0x%08" PFMT64x, vaddr);
	}
}
...
static void cmd_iSm(RCore *core, const char *input, PJ **_pj, int mode, const bool va, const bool is_array) {
	...
	R_VEC_FOREACH (&bo->sections_vec, sec) {
		...
		if (pj) {
			...
			r_cons_printf (core->cons, "0x%08" PFMT64x "-0x%08" PFMT64x " %8s %s", sec->vaddr, sec->vaddr + sec->vsize, hsz, sec->name);
			free (hsz);
			if (countmode) {
				int count = 0;
				R_VEC_FOREACH (symbols, sym) {
...
static void cmd_idl(RCore *core, const char *input) {
	...
	case 0:
		...
		break;
	case 'd': // "idld" // download debug info file from remote dwarf symbols server
		if (linkname) {
...
static bool is_entrypoint_symbol(const char *name) {
	// TODO: On Swift any method can be an entrypoint if @main is used as attribute
	const char *words[] = {
		// clang-format off
