...
#define CROSSING_MATRIX_MAX_CELLS (1 << 20)
...
#define graph_foreach_anode_vec(vec, it, pos, anode) \
	if (vec) \
		R_VEC_FOREACH (vec, it) \
			if (((pos) = *(it)) && ((anode) = (RANode *) (pos)->data))
...
static void ptr_ht_set(HtUP *ht, const void *k, void *v) {
	ht_up_insert (ht, (ut64)(size_t)k, v);
}

static void *ptr_ht_get(HtUP *ht, const void *k) {
	return ht_up_find (ht, (ut64)(size_t)k, NULL);
}

static RList *ptr_ht_get_rlist(HtUP *ht, const void *k) {
	return ptr_ht_get (ht, k);
}

static void ptr_ht_set_int(HtUU *ht, const void *k, int v) {
	ht_uu_insert (ht, (ut64)(size_t)k, (ut64)(st64)v);
}

static int ptr_ht_get_int(HtUU *ht, const void *k) {
	return (int)(st64)ht_uu_find (ht, (ut64)(size_t)k, NULL);
}

static void ptr_ht_rlist_free(HtUPKv *kv) {
	r_list_free ((RList *)kv->value);
}
...
static RGraphNode *agraph_get_title(const RAGraph *g, RANode *n, bool in) {
	...
	const RVecGraphNodePtr *outnodes = in? &n->gnode->in_nodes: &n->gnode->out_nodes;
	RGraphNode *gn;
	RGraphNode **iter;
	RANode *an;
	graph_foreach_anode_vec (outnodes, iter, gn, an) {
	...
	graph_foreach_anode (nodes, it, gn, n) {
		if (is_mini) {
			...
			if (edgemode == 2 && !callgraph) {
				if (!layout) {
					n->w = R_MAX (n->w, (RVecGraphNodePtr_length (&n->gnode->out_nodes) * 2 + 1) + R_EDGES_X_INC * 2);
					n->w = R_MAX (n->w, (RVecGraphNodePtr_length (&n->gnode->in_nodes) * 2 + 1) + R_EDGES_X_INC * 2);
				} else {
					n->h = R_MAX (n->h, (RVecGraphNodePtr_length (&n->gnode->out_nodes) + 1) + R_EDGES_X_INC);
					n->h = R_MAX (n->h, (RVecGraphNodePtr_length (&n->gnode->in_nodes) + 1) + R_EDGES_X_INC);
				}
			}
		}
	}
...
static char *get_node_bgcolor(RCons *cons, int color, int cur) {
	if (color == -1) {
...
static int **get_crossing_matrix(RCons *cons, const RGraph *g, const struct layer_t layers[], int maxlayer, int i, int from_up, int *n_rows) {
	...
	if (i > 0 && from_up) {
		...
		for (j = 0; j < layers[i - 1].n_nodes; j++) {
			...
			const RVecGraphNodePtr *neigh = r_graph_get_neighbours (g, gj);
			RGraphNode *gk;
			RGraphNode **itk;
			const RANode *ak;

			graph_foreach_anode_vec (neigh, itk, gk, ak) {
				int s;
				// skip self-loop
				if (gj == gk) {
					continue;
				...
				for (s = 0; s < j; s++) {
					...
					const RVecGraphNodePtr *neigh_s = r_graph_get_neighbours (g, gs);
					RGraphNode *gt;
					RGraphNode **itt;
					const RANode *at;

					graph_foreach_anode_vec (neigh_s, itt, gt, at) {
						if (gt == gk || gt == gs) {
							continue;
						}
		...
		for (j = 0; j < layers[i].n_nodes; j++) {
			...
			graph_foreach_anode_vec (neigh, itk, gk, ak) {
				...
				for (s = 0; s < layers[i].n_nodes; s++) {
					...
					const RVecGraphNodePtr *neigh_s;
					RGraphNode *gt;
					RGraphNode **itt;
...
static int layer_sweep(RCons *cons, const RGraph *g, const struct layer_t layers[], int maxlayer, int i, int from_up) {
	...
	if ((ut64)len * (ut64)len > CROSSING_MATRIX_MAX_CELLS) {
		return false;
	}
...
static int crossing_max_changes(const RAGraph *g) {
	return g->graph->n_nodes > 4096? 64: 4096;
}
...
static void assign_layers(const RAGraph *g) {
	...
	graph_foreach_anode (topological_sort, it, gn, n) {
		const RVecGraphNodePtr *innodes = r_graph_innodes (g->graph, gn);
		RGraphNode **it;
		RGraphNode *prev;
		RANode *preva;

		n->layer = 0;
		graph_foreach_anode_vec (innodes, it, prev, preva) {
...
static void minimize_crossings(const RAGraph *g) {
	int i, cross_changed, max_changes = crossing_max_changes (g);
	...
	max_changes = crossing_max_changes (g);
...
static HtUP *compute_vertical_nodes(const RAGraph *g) {
	HtUP *res = ht_up_new (NULL, ptr_ht_rlist_free, NULL);
	if (!res) {
		return NULL;
	}
	int i, j;
	...
	for (i = 0; i < g->n_layers; i++) {
		for (j = 0; j < g->layers[i].n_nodes; j++) {
			...
			const RList *Ln = ptr_ht_get_rlist (res, gn);
...
static RList **compute_classes(const RAGraph *g, HtUP *v_nodes, int is_left, int *n_classes) {
	int i, j, c;
	...
	for (i = 0; i < g->n_layers; i++) {
		...
		for (j = is_left? 0: g->layers[i].n_nodes - 1;
			...
			if (aj->klass == -1) {
				const RList *laj = ptr_ht_get_rlist (v_nodes, gj);
...
static int adjust_class_val(const RAGraph *g, const RGraphNode *gn, const RGraphNode *sibl, HtUU *res, int is_left) {
	if (is_left) {
		return ptr_ht_get_int (res, sibl) - ptr_ht_get_int (res, gn) - dist_nodes (g, gn, sibl);
	}
	return ptr_ht_get_int (res, gn) - ptr_ht_get_int (res, sibl) - dist_nodes (g, sibl, gn);
}

/* adjusts the position of previously placed left/right classes */
/* tries to place classes as close as possible */
static void adjust_class(const RAGraph *g, int is_left, RList **classes, HtUU *res, int c) {
	const RGraphNode *gn;
	const RListIter *it;
	const RANode *an;
	int dist = 0;
	...
	if (is_first) {
		...
		graph_foreach_anode (classes[c], it, gn, an) {
			const RVecGraphNodePtr *neigh = r_graph_all_neighbours (g->graph, gn);
			const RGraphNode *gk;
			RGraphNode **itk;
			const RANode *ak;
	...
	graph_foreach_anode (classes[c], it, gn, an) {
		const int old_val = ptr_ht_get_int (res, gn);
		const int new_val = is_left? old_val + dist: old_val - dist;
		ptr_ht_set_int (res, gn, new_val);
	}
}

static int place_nodes_val(const RAGraph *g, const RGraphNode *gn, const RGraphNode *sibl, HtUU *res, int is_left) {
	if (is_left) {
		return ptr_ht_get_int (res, sibl) + dist_nodes (g, sibl, gn);
	}
	return ptr_ht_get_int (res, sibl) - dist_nodes (g, gn, sibl);
}
...
static void place_nodes(const RAGraph *g, const RGraphNode *gn, int is_left, HtUP *v_nodes, RList **classes, HtUU *res, SetU *placed) {
	const RList *lv = ptr_ht_get_rlist (v_nodes, gn);
	...
	graph_foreach_anode (lv, itk, gk, ak) {
		...
		if (ak->klass == sibl_anode->klass) {
			if (!set_u_contains (placed, (ut64)(size_t)sibling)) {
	...
	graph_foreach_anode (lv, itk, gk, ak) {
		ptr_ht_set_int (res, gk, p);
		set_u_add (placed, (ut64)(size_t)gk);
	}
}

/* computes the position to the left/right of all the nodes */
static HtUU *compute_pos(const RAGraph *g, int is_left, HtUP *v_nodes) {
	int n_classes, i;
	...
	for (i = 0; i < n_classes; i++) {
		...
		r_list_foreach (classes[i], it, gn) {
			if (!set_u_contains (placed, (ut64)(size_t)gn)) {
...
static void place_dummies(const RAGraph *g) {
	...
	HtUP *vertical_nodes = compute_vertical_nodes (g);
	if (!vertical_nodes) {
		return;
	}
	HtUU *xminus = compute_pos (g, true, vertical_nodes);
	if (!xminus) {
		goto xminus_err;
	}
	HtUU *xplus = compute_pos (g, false, vertical_nodes);
	if (!xplus) {
		goto xplus_err;
	}
	...
	const RList *nodes = r_graph_get_nodes (g->graph);
	graph_foreach_anode (nodes, it, gn, n) {
		n->x = (ptr_ht_get_int (xminus, gn) + ptr_ht_get_int (xplus, gn)) / 2;
	}
...
static void adjust_directions(const RAGraph *g, int i, int from_up, HtUU *D, HtUU *P) {
	...
	for (j = 0; j < g->layers[i + d].n_nodes; j++) {
		...
		if (from_up) {
			const RVecGraphNodePtr *innodes = r_graph_innodes (g->graph, vp);
			RGraphNode **first = innodes? RVecGraphNodePtr_at (innodes, 0): NULL;
			wp = first? *first: NULL;
		} else {
...
static void place_single(const RAGraph *g, int l, const RGraphNode *bm, const RGraphNode *bp, int from_up, int va) {
	...
	const RVecGraphNodePtr *neigh = from_up
		? r_graph_innodes (g->graph, v)
		: r_graph_get_neighbours (g->graph, v);

	int len = neigh? RVecGraphNodePtr_length (neigh): 0;
	if (len == 0) {
		return;
	}
...
static void collect_changes(const RAGraph *g, int l, const RGraphNode *b, int from_up, int s, int e, RList *list, int is_left) {
	...
	for (i = is_left? s: e - 1; (is_left && i < e) || (!is_left && i >= s); i = is_left? i + 1: i - 1) {
		...
		graph_foreach_anode_vec (neigh, it, v, av) {
...
static void original_traverse_l(const RAGraph *g, HtUU *D, HtUU *P, int from_up) {
	int i, k, va, vr;
	...
	for (i = from_up? 0: g->n_layers - 1;
		...
		while (bm) {
			...
			} else if (ptr_ht_get_int (D, bm) == from_up) {
				bpa = get_anode (bp);
				va = bma->pos_in_layer + 1;
				vr = bpa->pos_in_layer;
				place_sequence (g, i, bm, bp, from_up, va, vr);
				ptr_ht_set_int (P, bm, true);
			}
			bm = bp;
		}
...
static void place_original(RAGraph *g) {
	...
	graph_foreach_anode (nodes, itn, gn, an) {
		...
		if (right_v && right) {
			ptr_ht_set_int (D, gn, 0);
			int dt_eq = right->x - an->x == dist_nodes (g, gn, right_v);
			ptr_ht_set_int (P, gn, dt_eq);
		}
	}
...
static void set_layer_gap(RAGraph *g) {
	...
	RGraphNode *ga, *gb;
	RANode *a, *b;
	const RVecGraphNodePtr *outnodes;
	...
	for (i = 0; i < g->n_layers; i++) {
		...
		for (j = 0; j < g->layers[i].n_nodes; j++) {
			...
			graph_foreach_anode_vec (outnodes, itn, gb, b) {
...
static void fix_back_edge_dummy_nodes(RAGraph *g, RANode *from, RANode *to) {
	...
	const RVecGraphNodePtr *neighbours = r_graph_get_neighbours (g->graph, to->gnode);
	graph_foreach_anode_vec (neighbours, it, gv, v) {
		tmp = v;
		while (tmp->is_dummy) {
			tmp = get_anode (r_graph_nth_neighbour (g->graph, tmp->gnode, 0));
		}
	...
	if (tmp) {
		...
		while (tmp->gnode->idx != from->gnode->idx) {
			v = tmp;
			tmp = get_anode (r_graph_nth_neighbour (g->graph, v->gnode, 0));
...
static int get_edge_number(const RAGraph *g, RANode *src, RANode *dst, bool outgoing) {
	...
	if (outgoing && src->is_dummy) {
		if (src->gnode) {
			RGraphNode **first = RVecGraphNodePtr_at (&src->gnode->in_nodes, 0);
			RANode *in = first? get_anode (*first): NULL;
			if (in) {
				cur_nth = get_edge_number (g, in, src, outgoing);
			}
		}
	} else {
		const RVecGraphNodePtr *neighbours = outgoing
			? r_graph_get_neighbours (g->graph, src->gnode)
			: r_graph_innodes (g->graph, dst->gnode);
		const int exit_edges = neighbours? RVecGraphNodePtr_length (neighbours): 0;
		graph_foreach_anode_vec (neighbours, itn, gv, v) {
			cur_nth = nth;
			if (g->is_callgraph) {
				cur_nth = 0;
...
static ut64 edge_number_key(const RANode *src, const RANode *dst) {
	return ((ut64)src->gnode->idx << 32) | (ut64)dst->gnode->idx;
}

static int get_edge_number_cached(const RAGraph *g, RANode *src, RANode *dst, bool outgoing, HtUU *cache) {
	if (!cache || !src || !dst || !src->gnode || !dst->gnode) {
		return get_edge_number (g, src, dst, outgoing);
	}
	const ut64 key = edge_number_key (src, dst);
	bool found = false;
	ut64 cached = ht_uu_find (cache, key, &found);
	if (found) {
		return (int)cached - 2;
	}
	int nth = get_edge_number (g, src, dst, outgoing);
	ht_uu_insert (cache, key, nth + 2);
	return nth;
}
...
static void backedge_info(RAGraph *g) {
	...
	for (i = 0; i < g->n_layers; i++) {
		...
		for (j = 0; j < g->layers[i].n_nodes; j++) {
			...
			const RVecGraphNodePtr *neighbours = r_graph_get_neighbours (g->graph, a->gnode);
			RGraphNode *gb;
			RANode *b;
			RGraphNode **itm;

			if (i == 0) {
				const RVecGraphNodePtr *innodes = r_graph_innodes (g->graph, a->gnode);
				inedge += innodes? RVecGraphNodePtr_length (innodes): 0;
			} else if (i == g->n_layers - 1) {
				outedge += neighbours? RVecGraphNodePtr_length (neighbours): 0;
			}

			graph_foreach_anode_vec (neighbours, itm, gb, b) {
				if (b->layer > a->layer) {
					continue;
				}
...
static void graph_node_vec_delete(RVecGraphNodePtr *vec, const RGraphNode *node) {
	size_t i;
...
static void delete_dup_edges(RAGraph *g) {
	...
	r_list_foreach (g->graph->nodes, it, n) {
		size_t i, j;
		for (i = 0; i < RVecGraphNodePtr_length (&n->out_nodes); i++) {
			a = *RVecGraphNodePtr_at (&n->out_nodes, i);
			for (j = i + 1; j < RVecGraphNodePtr_length (&n->out_nodes); j++) {
				b = *RVecGraphNodePtr_at (&n->out_nodes, j);
				if (a->idx == b->idx) {
					RVecGraphNodePtr_remove (&n->out_nodes, j--);
					graph_node_vec_delete (&n->all_neighbours, b);
					graph_node_vec_delete (&b->in_nodes, n);
					graph_node_vec_delete (&b->all_neighbours, n);
					g->graph->n_edges--;
				}
			}
		}
	...
	graph_foreach_anode (nodes, it, gn, n) {
		...
		if (isNear) {
			...
			if ((is_next && resn->y > n->y) || (!is_next && resn->y < n->y)) {
				resgn = gn;
			} else if ((is_next && resn->y == n->y && resn->x > n->x) || (!is_next && resn->y == n->y && resn->x < n->x)) {
				resgn = gn;
			}
		}
	}
...
static void update_graph_sizes(RAGraph *g) {
	...
	graph_foreach_anode (r_graph_get_nodes (g->graph), it, gk, ak) {
		...
		len = nd? RVecGraphNodePtr_length (nd) + 1: 0;
		if (ak->y - len < g->y) {
			g->y = ak->y - len;
			min_gn = ak;
		}
		...
		len = nd? RVecGraphNodePtr_length (nd) + 2: 0;
	...
	if (min_gn) {
		const RVecGraphNodePtr *neigh = r_graph_innodes (g->graph, min_gn->gnode);
		if (neigh && RVecGraphNodePtr_length (neigh) > 0) {
			g->y--;
			max_y++;
		}
		if (max_gn) {
			const RVecGraphNodePtr *neigh = r_graph_get_neighbours (g->graph, min_gn->gnode);
			if (neigh && RVecGraphNodePtr_length (neigh) > 0) {
				max_y++;
			}
		}
...
static void agraph_print_edges_simple(RAGraph *g) {
	...
	graph_foreach_anode (nodes, iter, gn, n) {
		graph_foreach_anode_vec (&n->gnode->out_nodes, iter2, gn2, n2) {
			int sx = n->w / 2;
			int sy = n->h;
			int sx2 = n2->w / 2;
			if (g->is_tiny) {
...
static int first_x_cmp(RGraphNode *const *ga, RGraphNode *const *gb) {
...
static void agraph_print_edges(RAGraph *g) {
	...
	HtUU *out_edge_numbers = ht_uu_new0 ();
	HtUU *in_edge_numbers = ht_uu_new0 ();
	struct tmplayer *tl, *tm;

	graph_foreach_anode (nodes, itm, ga, a) {
		const RGraphNode *gb;
		RANode *b;
		RVecGraphNodePtr *neighbours = &ga->out_nodes;
		...
		bool many = RVecGraphNodePtr_length (neighbours) > 2;

		if (many && !g->is_callgraph) {
			RVecGraphNodePtr_sort (neighbours, first_x_cmp);
		}

		graph_foreach_anode_vec (neighbours, itn, gb, b) {
			out_nth = get_edge_number_cached (g, a, b, true, out_edge_numbers);
			in_nth = get_edge_number_cached (g, a, b, false, in_edge_numbers);

			bool parent_many = false;
			if (a->is_dummy) {
				RGraphNode **first = RVecGraphNodePtr_at (&ga->in_nodes, 0);
				RANode *in = first? get_anode (*first): NULL;
				while (in && in->is_dummy) {
					first = RVecGraphNodePtr_at (&in->gnode->in_nodes, 0);
					in = first? get_anode (*first): NULL;
				}
				if (in && in->gnode) {
					parent_many = RVecGraphNodePtr_length (&in->gnode->out_nodes) > 2;
				} else {
					parent_many = false;
				}
			}
			...
			default:
				...
				if (many && !g->is_callgraph) {
					int t = R_EDGES_X_INC + 2 *(RVecGraphNodePtr_length (neighbours) + 1);
					ax = a->is_dummy? a->x: (a->x + a->w / 2 + (t / 2 - a_x_inc));
					bendpoint = bx < ax? RVecGraphNodePtr_length (neighbours) - out_nth: out_nth;
				} else {
					ax = a->is_dummy? a->x: (a->x + a_x_inc);
					bendpoint = tm->edgectr;
				}

				if (!a->is_dummy && out_nth == 0 && bx > ax) {
...
static void agraph_follow_innodes(RCore *core, RAGraph *g, bool in) {
	...
	const RVecGraphNodePtr *list = in? &an->gnode->in_nodes: &an->gnode->out_nodes;
	int nth = -1;
	if (RVecGraphNodePtr_length (list) == 0) {
		return;
	}
	RCons *cons = core->cons;
	...
	if (RVecGraphNodePtr_length (list) == 1) {
		nth = 0;
	} else if (RVecGraphNodePtr_length (list) < 10) {
		// just 1 key
...
static void check_function_modified(RCore *core, RAnalFunction *fcn) {
	if (r_anal_function_was_modified (fcn)) {
		if (r_config_get_b (core->config, "anal.onchange") || r_cons_yesno (core->cons, 'y', "Function was modified. Reanalyze? (Y/n)")) {
...
static void agraph_sdb_init(const RAGraph *g) {
	...
	sdb_set_enc (g->db, "agraph.color_box3", ctx->pal.graph_box3, 0);
	sdb_set_enc (g->db, "agraph.color_true", ctx->pal.graph_true, 0);
	sdb_set_enc (g->db, "agraph.color_false", ctx->pal.graph_false, 0);
}
...
R_API void r_agraph_print_json(RAGraph *g, PJ *pj) {
	R_RETURN_IF_FAIL (g && pj);
	RList *nodes = g->graph->nodes;
	RListIter *it;
	...
	r_list_foreach (nodes, it, node) {
		...
		pj_a (pj);
		RGraphNode **vit;
		R_VEC_FOREACH (&anode->gnode->out_nodes, vit) {
			pj_i (pj, (*vit)->idx);
		}
		pj_end (pj);
		pj_end (pj);
...
R_API void r_core_visual_find(RCore *core, RAGraph *g) {
	...
	find_next:
		...
		if (addr > 0) {
			if (c == ';') {
				char buf[256];
				r_line_set_prompt (cons->line, "[comment]> ");
				if (r_cons_fgets (cons, buf, sizeof (buf), 0, NULL) > 0) {
					r_core_cmdf (core, "'CC %s", buf);
				}
				r_line_set_prompt (cons->line, "[find]> ");
				if (g) {
					g->need_reload_nodes = true;
				}
			}
...
R_API bool r_core_visual_graph(RCore *core, RAGraph *g, RAnalFunction *_fcn, int mode) {
	...
	while (!exit_graph && !is_error && !r_cons_is_breaked (core->cons)) {
...
R_API RAGraph *r_agraph_new_from_graph(void *_core, const RGraph *graph, RAGraphTransitionCBs *cbs, void *user) {
	...
	r_list_foreach (graph->nodes, iter, node) {
		...
		RGraphNode **neighbour_iter;
		R_VEC_FOREACH (&node->in_nodes, neighbour_iter) {
			RGraphNode *neighbour = *neighbour_iter;
