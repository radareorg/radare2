...
static void type_match(RCore *core, char *fcn_name, ut64 addr, ut64 baddr, const char* cc,
	...
	if (verbose && !strncmp (fcn_name, "sym.imp.", 8)) {
		R_LOG_WARN ("Warning: Missing function definition for '%s'", fcn_name + 8);
	}
	if (!max) {
		if (!in_stack) {
...
repeat:
	...
	for (j = 0; j < bblist_size; j++) {
		...
		if (!bb) {
			R_LOG_WARN ("Warning: basic block at 0x%08"PFMT64x" was removed during analysis.", bblist[j]);
			retries--;
			free (bblist);
			goto repeat;
		}
		...
		while (1) {
			...
			if (i < bblist_size) {
				...
				if (!bb) {
					R_LOG_WARN ("Warning: basic block at 0x%08"PFMT64x" was removed during analysis.", bblist[i]);
					retries--;
					free (bblist);
					goto repeat;
				}
