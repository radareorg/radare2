...
static RCoreHelpMessage help_msg_dmh_jemalloc = {
	...
	"dmhc", "*|[arena_t]", "show chunks in all arenas or given arena",
...
static RCoreHelpMessage help_msg_dc = {
	...
	"dce", "", "continue execution (pass exception)",
	"dcf", "", "continue until fork (TODO)",
	"dck", " <sig> <pid>", "continue sending signal to process",
	"dcp", "", "continue until mapped io section",
	"dcr", "", "continue until ret (uses step over)",
	"dcs", "[?] <num>", "continue until syscall",
	"dct", " <len>", "traptrace from curseek to len",
	"dcu", "[?] [addr] ([end])", "continue until address (or range)",
	"dcut", " ([ms])", "interrupt child after 100ms (or ms)",
	/*"TODO: dcu/dcr needs dbg.untilover=true??",*/
	/*"TODO: same for only user/libs side",*/
...
static RCoreHelpMessage help_msg_dm = {
	...
	"dm", " address size", "allocate <size> bytes at <address> in child",
	"dm=", "", "list memory maps of target (ascii-art bars)",
	"dm.", "", "show map name of current address",
	"dm*", "[-*]", "list memmaps in r2 commands",
	"dm-", " address", "deallocate memory map of <address>",
	"dmd", "[aw] [file]", "dump debug map region to file (see Sd)",
	...
	"dmp", "[?] <addr> <size> <perms>", "change page protection at addr",
...
static RCoreHelpMessage help_msg_dmh_glibc = {
	...
	"dmhb", " @[malloc_state]", "display parsed double linked list of arena bins",
	"dmhb", " [bin_num|bin_num:malloc_state]", "display parsed bins from arena",
	"dmhbg", " [bin_num]", "display bin graph (under development)",
	"dmhc", " @[chunk_addr]", "display malloc_chunk struct for given chunk",
	"dmhf", " @[malloc_state]", "display parsed fastbins of arena",
	"dmhf", " [fastbin_num(:malloc_state)]", "display fastbin linked list from arena",
