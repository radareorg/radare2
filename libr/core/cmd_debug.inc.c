		...
		case 's': // "dts"
			...
			case 'r': { // "dtsr"
				...
				if (input[3] == 'a') { // "dtsra"
					...
					const char *tok = argc > 0? r_str_word_get0 (args, 0): NULL;
					if (!tok) {
			...
			case 'w': { // "dtsw*"
				...
				if (input[3] == 'c') {
					...
					int argc = r_str_word_set0 (args);
					const char *id_s = argc > 0? r_str_word_get0 (args, 0): NULL;
					if (!id_s) {
						R_LOG_ERROR ("Missing checkpoint id");
						free (args);
						break;
				...
				const char *id_s = argc > 0? r_str_word_get0 (args, 0): NULL;
				const char *fd_s = argc > 1? r_str_word_get0 (args, 1): NULL;
				const char *hex_s = argc > 2? r_str_word_get0 (args, 2): NULL;
				if (!id_s || !fd_s || !hex_s) {
