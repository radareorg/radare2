...
static RCoreHelpMessage help_msg_dg = {
	...
	"dgm", " [file]", "generate a Memory Slice (.msl) of the debuggee (skips huge maps)",
...
static int cmd_debug_continue(RCore *core, const char *input) {
	...
	case 0: // "dc"
		if (!r_config_get_b (core->config, "cfg.debug")) {
			// Emulated target (ESIL / msl): no live process to resume, so
			// continue via the ESIL VM. Mirrors how "ds" dispatches to "aes"
			// when cfg.debug is unset; stops at a breakpoint, an invalid
			// instruction or a memory fault.
...
// ---------------------------------------------------------------------------
// Memory Slice (.msl) dump of the live debug session.
//
// Writes the producer side of the format consumed by the io/bin/debug "msl"
// plugins and by memslicer (https://github.com/MemorySlice/memslicer): a file
// header, Process Identity, a Thread Context with the current registers, and
// one Memory Region per debug map (three-state page map + captured bytes). The
// integrity chain uses SHA-256 (HashAlgo 0x01) since radare2 has no BLAKE3.
// ---------------------------------------------------------------------------
...
static void cmd_dg(RCore *core, const char *input) {
	if (input[1] == '?') {
		...
		free (corefile);
	} else if (input[1] == 'm') {  // "dgm"/"dgma" - generate a Memory Slice (.msl)
		bool full = (input[2] == 'a');
		const char *filename = strchr (input, ' ');
