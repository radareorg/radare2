	...
	if ((mode & R_CORE_BIN_SET)) {
		...
		r_list_foreach (list, iter, string) {
			...
			r_meta_add (r->anal, R_META_TYPE_STRING,
				va?string->vaddr:string->paddr,
				(va?string->vaddr:string->paddr)+string->size,
				string->string);
		...
		r_list_foreach (list, iter, string) {
			...
			if (mode) {
				r_cons_printf ("f str.%s %"PFMT64d" @ 0x%08"PFMT64x"\n"
					...
					string->string, string->size, va? string->vaddr: string->paddr,
					string->size, va? string->vaddr: string->paddr);
...
static int bin_main (RCore *r, int mode, ut64 baddr, int va) {
	...
	if ((mode & R_CORE_BIN_SIMPLE) || mode & R_CORE_BIN_JSON) {
		r_cons_printf ("%"PFMT64d, va? binmain->vaddr:binmain->paddr);
	} else
	if ((mode & R_CORE_BIN_SET)) {
		r_flag_space_set (r->flags, "symbols");
		r_flag_set (r->flags, "main", va? binmain->vaddr: binmain->paddr,
				r->blocksize, 0);
	} else {
		if (mode) {
			r_cons_printf ("fs symbols\n");
			r_cons_printf ("f main @ 0x%08"PFMT64x"\n",
				va? binmain->vaddr: binmain->paddr);
		} else {
			r_cons_printf ("[Main]\n");
			r_cons_printf ("addr=0x%08"PFMT64x" off=0x%08"PFMT64x"\n",
					binmain->vaddr, binmain->paddr);
		}
	}
	return R_TRUE;
}
...
static int bin_entry (RCore *r, int mode, ut64 baddr, int va) {
	...
	if ((mode & R_CORE_BIN_SET)) {
		r_list_foreach (entries, iter, entry) {
			...
			r_flag_set (r->flags, str, va? entry->vaddr: entry->paddr,
				r->blocksize, 0);
		}
		/* Seek to the last entry point */
		if (entry)
			r_core_seek (r, va? entry->vaddr: entry->paddr, 0);
	} else {
		...
		r_list_foreach (entries, iter, entry) {
			if (mode) {
				r_cons_printf ("f entry%i @ 0x%08"PFMT64x"\n",
					i, va? entry->vaddr: entry->paddr);
				r_cons_printf ("s entry%i\n", i);
			} else r_cons_printf ("addr=0x%08"PFMT64x
					" off=0x%08"PFMT64x
					" baddr=0x%08"PFMT64x"\n",
					va? entry->vaddr: entry->paddr, entry->paddr, baddr);
			i++;
		}
...
static ut64 impaddr(RBin *bin, int va, const char *name) {
	...
	r_list_foreach (symbols, iter, symbol) {
		...
		if (!strcmp (symbol->name+4, name))
			return va? symbol->vaddr: symbol->paddr;
	}
	return 0;
}
	...
	if (mode & R_CORE_BIN_JSON) {
		...
		r_list_foreach (symbols, iter, symbol) {
			r_cons_printf ("%s{\"name\":\"%s\","
				"\"size\":%"PFMT64d","
				"\"paddr\":%"PFMT64d"}",
				iter->p?",":"", symbol->name, symbol->size, va? symbol->vaddr: symbol->paddr);
		}
		r_cons_printf ("]");
	} else
	if ((mode & R_CORE_BIN_SIMPLE)) {
		r_list_foreach (symbols, iter, symbol) {
			...
			r_cons_printf ("0x%08"PFMT64x" %"PFMT64d" %s\n",
				va? symbol->vaddr: symbol->paddr, symbol->size, name);
			free (name);
		}
	} else
	if ((mode & R_CORE_BIN_SET)) {
		...
		r_flag_space_set (r->flags, "symbols");
		r_list_foreach (symbols, iter, symbol) {
			ut64 addr = va? symbol->vaddr: symbol->paddr;
...
static int bin_sections (RCore *r, int mode, ut64 baddr, int va, ut64 at, const char *name) {
	...
	if ((mode & R_CORE_BIN_SIMPLE)) {
		r_list_foreach (sections, iter, section) {
			ut64 addr = va? section->vaddr: section->paddr;
			...
			if (at) {
				...
				if (mode) {
					...
					r_cons_printf ("f section_end.%s %"PFMT64d" 0x%08"PFMT64x"\n",
							section->name, (ut64)0, section->size+(va?section->vaddr:section->paddr));
...
static int bin_fields (RCore *r, int mode, ut64 baddr, int va) {
	...
	if (mode & R_CORE_BIN_JSON) {
		...
		r_list_foreach (fields, iter, field) {
			ut64 addr = va? field->vaddr: field->paddr;
			r_cons_printf ("%s{\"name\":\"%s\","
				"\"paddr\":%"PFMT64d"}",
				iter->p?",":"",
				field->name, addr);
