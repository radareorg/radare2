...
static int r_resolve_main_arena(RCore *core, ut64 *m_arena, RHeap_MallocState *main_arena) {
	...
	if (*m_arena == UT64_MAX) {
		...
		if (!core || !core->dbg || !core->dbg->maps) return 1;
...
int  print_heap_chunk(RCore *core) {
	RHeapChunk *cnk = R_NEW0 (RHeapChunk);
	memset(cnk, 0x0, sizeof(RHeapChunk));

	core->offset  -= sizeof(size_t)*2;
	
	r_core_read_at(core, core->offset, (ut8 *) cnk , sizeof(RHeapChunk));
	eprintf("struct malloc_chunk @ 0x%"PFMT64x"\n", core->offset);
	eprintf("\tprev_size = 0x%"PFMT64x"\n", (ut64)(size_t)(cnk->prev_size));
	
	eprintf("\tsize = 0x%"PFMT64x"\n", (ut64)(size_t)(cnk->size));
	eprintf("\tfd = 0x%"PFMT64x"\n", (ut64)(size_t)(cnk->fd));
	eprintf("\tbk = 0x%"PFMT64x"\n", (ut64)(size_t)(cnk->bk));
	eprintf("\tfd-nextsize = 0x%"PFMT64x"\n", (ut64)(size_t)(cnk->fd_nextsize));
	eprintf("\tbk_nextsize = 0x%"PFMT64x"\n", (ut64)(size_t)(cnk->bk_nextsize));
	eprintf("}\n");
	return 0;
}
...
static int cmd_debug_map_heap(RCore *core, const char *input) {
	...
	const char* help_msg[] = {
		...
		"dmhc", "@[addr]", "Print malloc_struct for a given malloc chunk",
