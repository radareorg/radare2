			...
			if (off) {
				if (IS_MODE_RAD (mode)) {
					...
					if (buf) {
						...
						free (buf);
						if (res < 0) {
							R_LOG_WARN ("Warning: Cannot register invalid format (%s)", v);
						}
					}
				}
			}
...
R_API void r_core_anal_cc_init(RCore *core) {
	...
	}
	{
		...
		if (s) {
			if (!r_anal_cc_set (core->anal, s)) {
				R_LOG_WARN ("Warning: Invalid CC from reg profile.");
			}
			free (s);
		} else {
			R_LOG_WARN ("Warning: Cannot derive CC from reg profile.");
		}
	}
#else
	const char *dir_prefix = r_config_get (core->config, "dir.prefix");
	...
	if (anal_arch && sdb_isempty (core->anal->sdb_cc)) {
		R_LOG_WARN ("Warning: Missing calling conventions for '%s' %d. Deriving it from the regprofile.", anal_arch, bits);
	}
	free (anal_arch);
	free (dbpath);
	free (dbhomepath);
...
R_API bool r_core_pdb_info(RCore *core, const char *file, PJ *pj, int mode) {
	...
	if (core->bin->cur && core->bin->cur->o && core->bin->cur->o->baddr) {
		...
		R_LOG_WARN ("Warning: Cannot find base address, flags will probably be misplaced");
		baddr = 0LL;
	}
			...
			for (num_string = 0; sdb; num_string++) {
				...
				if (sdb) {
					...
					if (r_str_utf16_to_utf8 (key_utf8, lenkey * 2, key_utf16, lenkey, true) < 0
						...
						R_LOG_WARN ("Warning: Cannot decode utf16 to utf8");
					} else if (IS_MODE_JSON (mode)) {
