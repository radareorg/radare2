...
static int get_depth (RGraphNode *path[], RGraphNode *n) {
	int res = 0;
	while (path[n->idx] != NULL) {
		n = path[n->idx];
		res++;
	}
	return res;
...
static void set_layer (RGraphNode *from, RGraphNode *to, RGraphVisitor *vis) {
	RGraphNode **path = (RGraphNode **)vis->data;
	int bdepth, adepth;
...
static void assign_layers (AGraph *g) {
	...
	RGraphNode **path_layers = R_NEWS0 (RGraphNode *, g->graph->n_nodes);
	RGraphNode *gn;
	RListIter *it;
	ANode *n;
