...
static void GH(get_brks)(RCore *core, GHT *brk_start, GHT *brk_end) {
	if (r_config_get_i (core->config, "cfg.debug")) {
		...
		RIOSection *s;
		SdbListIter *iter1, *iter2;
		ls_foreach_safe (core->io->sections, iter1, iter2, s) {
			if (strstr (s->name, "[heap]")) {
				*brk_start = s->vaddr;
				*brk_end = s->vaddr + s->size;
				break;
			}
		}
	}
...
static bool GH(r_resolve_main_arena)(RCore *core, GHT *m_arena) {
	...
	if (!r_config_get_i (core->config, "cfg.debug")) {
		...
		SdbListIter *iter1, *iter2;
		ls_foreach_safe (core->io->sections, iter1, iter2, s) {
			if (strstr (s->name, "arena")) {
				libc_addr_sta = s->vaddr;
				libc_addr_end = libc_addr_sta + s->vsize;
				break;
			}
		}
	} else {
