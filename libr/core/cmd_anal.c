...
static const char *help_msg_afis[] = {
	"Usage:", "afis[ft]", "",
	"afis", "", "show information of the function",
...
static bool cmd_anal_aaft(RCore *core) {
	...
	if (r_config_get_b (core->config, "cfg.debug")) {
		R_LOG_ERROR ("TOFIX: aaft can't run in debugger mode.");
		return false;
	}
	if (!io_cache) {
...
static void find_refs(RCore *core, const char *glob) {
	...
	if (*glob == '?') {
		r_cons_printf ("Usage: axF [flag-str-filter]\n");
		return;
	}
	r_cons_printf ("Finding references of flags matching '%s'...\n", glob);
...
static void var_help(RCore *core, char ch) {
	...
	case '?':
		...
		break;
	default:
		r_cons_printf ("See afv?, afvb?, afvr? and afvs?\n");
	}
}
...
static void var_accesses_list(RAnalFunction *fcn, RAnalVar *var, PJ *pj, int access_type, const char *name) {
	...
	if (r_vector_empty (&var->accesses)) {
		R_LOG_WARN ("Warning: Variable '%s' has no references?", name);
	}
	if (pj) {
		pj_o (pj);
		pj_ks (pj, "name", name);
...
static int var_cmd(RCore *core, const char *str) {
	...
	case '=': // "afv="
		if (fcn) {
			...
			R_LOG_WARN ("afv: Cannot find function in 0x%08"PFMT64x, core->offset);
			free (ostr);
			return false;
		}
	case 'a': // "afva"
		...
		if (str[1]) { // "afvn"
			...
			if (!old_name) {
				...
				if (var) {
					...
					R_LOG_WARN ("Cannot find var @ 0x%08"PFMT64x, core->offset);
					r_anal_op_free (op);
					free (ostr);
					return false;
				}
			...
			if (fcn) {
				...
				if (v1) {
			        ...
			        R_LOG_WARN ("Cant find var by name");
				}
			} else {
			    R_LOG_WARN ("afv: Cannot find function in 0x%08"PFMT64x, core->offset);
				r_anal_op_free (op);
	...
	case 'd': // "afvd"
		if (!fcn) {
			R_LOG_WARN ("afvd: Cannot find function.");
		} else if (str[1]) {
			p = strchr (ostr, ' ');
			if (!p) {
				free (ostr);
	...
	case 't':
		if (fcn) { // "afvt"
			...
			*type++ = 0;
			v1 = r_anal_function_get_var_byname (fcn, p);
			if (!v1) {
				R_LOG_WARN ("Cant find get by name %s", p);
				free (ostr);
				return false;
			}
			...
			R_LOG_WARN ("afvt: Cannot find function");
			return false;
		}
	}
	...
	case '*': // "afv[bsr]*"
		if (fcn) {
			...
			R_LOG_WARN ("afv: Cannot find function");
		}
		break;
	case 'j':  // "afv[bsr]j"
		pj = r_core_pj_new (core);
	...
	case '-': // "afv[bsr]-"
		...
		break;
	case 'd': // "afv[bsr]d"
		R_LOG_WARN ("This command is deprecated, use afvd instead");
		break;
	case 't': // "afv[bsr]t"
		R_LOG_WARN ("This command is deprecated use afvt instead");
		break;
	case 's': // "afv[bsr]s"
	case 'g': // "afv[bsr]g"
		if (str[2] != '\0') {
			...
			if (!var) {
				R_LOG_WARN ("Cannot find variable with delta %d", idx);
				res = false;
				break;
			}
	...
	case ' ': { // "afv[bsr]"
		...
		if (!fcn) {
			R_LOG_WARN ("Missing function at 0x%08"PFMT64x, core->offset);
			break;
		}
		*p++ = 0;
		r_str_trim_head (p);
...
static void core_anal_bytes(RCore *core, const ut8 *buf, int len, int nops, int fmt) {
	...
	for (i = idx = ret = 0; idx < len && (!nops || (nops && i < nops)); i++, idx += ret) {
		...
		if (ret < 1 && fmt != 'd') {
			R_LOG_WARN ("Oops at 0x%08" PFMT64x " (", core->offset + idx);
			for (i = idx, j = 0; i < core->blocksize && j < 3; i++, j++) {
				R_LOG_WARN ("%02x ", buf[i]); // not sure which R_LOG_ function to use for these
			}
			R_LOG_WARN ("...)");
			free (mnem);
			break;
		...
		if (fmt == 'd') {
			...
			if (R_STR_ISNOTEMPTY (esilstr)) {
				if (core->anal->esil) {
					...
					R_LOG_WARN ("Error: ESIL is not initialized. Run `aei`.");
					break;
				}
			} else {
...
static bool anal_fcn_list_bb(RCore *core, const char *input, bool one) {
	...
	if (!fcn) {
		...
		R_LOG_WARN ("Cannot find function in 0x%08"PFMT64x, addr);
		return false;
	}
	if (mode == '*') {
...
static bool anal_fcn_del_bb(RCore *core, const char *input) {
	...
	if (fcn) {
		if (!strcmp (input, "*")) {
			...
			R_LOG_WARN ("Cannot find basic block");
		}
	}
	return false;
}
...
static int cmd_afbplus(RCore *core, const char *input) {
	...
	case 6:
		...
		if (!(diff = r_anal_diff_new ())) {
			R_LOG_ERROR ("error: Cannot init RAnalDiff");
			free (ptr);
			return false;
		}
		if (ptr2[0] == 'm') {
	...
	if (fcn) {
		...
		{
			R_LOG_WARN ("afb+: Cannot add basic block at 0x%08"PFMT64x" with size %d", addr, (int)size);
		}
	} else {
		R_LOG_WARN ("afb+ No function at 0x%" PFMT64x " from 0x%08"PFMT64x" -> 0x%08"PFMT64x,
				fcnaddr, addr, jump);
	}
...
static void afCc(RCore *core, const char *input) {
	...
	if (fcn) {
		...
		R_LOG_WARN ("afCc: Cannot find function");
	}
}
...
static Sdb *__core_cmd_anal_fcn_stats(RCore *core, const char *input) {
	...
	RAnalFunction *fcn = r_anal_get_fcn_in (core->anal, core->offset, -1);
	if (!fcn) {
		R_LOG_WARN ("Cannot find any function at 0x%08"PFMT64x, core->offset);
		return NULL;
	}
	Sdb *db = sdb_new0 ();
	RAnalBlock *bb;
	...
	r_list_foreach (dbs, iter, db) {
		...
		ls_foreach (ls, it, kv) {
			...
			if (idx != -1) {
				...
				R_LOG_WARN ("Invalid column name (%s)", key);
			}
		}
		RList *items = r_list_newf (free);
...
static void cmd_afsr(RCore *core, const char *input) {
	...
	if ((f = r_anal_get_fcn_in (core->anal, addr, R_ANAL_FCN_TYPE_NULL))) {
		...
		R_LOG_WARN ("No function defined at 0x%08"PFMT64x, addr);
	}
}
...
static void cmd_afbc(RCore *core, const char *input) {
	...
	if (*ptr == '?') {
		r_cons_printf ("Usage: afbc red @ addrOfBlock\n");
	} else if (!*ptr) {
		...
		if (color.r || color.g || color.b) {
			...
			R_LOG_WARN ("Invalid color? (%s)", ptr);
		}
	}
	free (ptr);
}
...
R_API void r_core_af(RCore *core, ut64 addr, const char *name, bool anal_calls) {
	...
	if (fcn) {
		...
		if (core->anal->verbose) {
			R_LOG_DEBUG ("Warning: Unable to analyze function at 0x%08"PFMT64x, addr);
		}
	}
	if (anal_calls) {
		...
		if (fcn) {
			...
			r_list_foreach (refs, iter, ref) {
				...
				if (f) {
					...
					if (!f) {
						R_LOG_WARN ("af: Cannot find function at 0x%08"PFMT64x, fcn->addr);
					}
				}
#endif
			}
	...
	if (name) {
		if (*name && !__setFunctionName (core, addr, name, true)) {
			R_LOG_WARN ("af: Cannot find function at 0x%08"PFMT64x, addr);
		}
	}
#if SLOW_ANALYSIS
...
static int cmd_af(RCore *core, const char *input) {
	...
	case 'd': // "afd"
		...
		if (input[2] == 'j') { // afdj
			...
			if (fcn) {
				...
				R_LOG_WARN ("afd: Cannot find function");
			}
		}
		}
		break;
	case 'u': // "afu"
		...
		if (addr_end < addr) {
			R_LOG_WARN ("Invalid address ranges");
		} else {
			ut64 a, b;
			const char *c;
	...
	case 'o': // "afo"
		switch (input[2]) {
		case '?':
			r_cons_printf ("Usage: afo[?sj] ([name|offset])\n");
			break;
		case 'j':
			{
	...
	case 's': // "afs"
		...
		case 'r': { // "afsr"
			...
			if (fcn) {
				...
				R_LOG_WARN ("There's no function defined in here.");
			}
			break;
		}
		case '*': // "afs*"
	...
	case 'C': // "afC"
		if (input[2] == 'c') {
			...
			if ((fcn = r_anal_get_fcn_in (core->anal, core->offset, 0))) {
				...
				R_LOG_WARN ("Error: Cannot find function at 0x08%"PFMT64x, core->offset);
			}
		} else if (input[2] == 'l') {
			RAnalFunction *fcn;
			if ((fcn = r_anal_get_fcn_in (core->anal, core->offset, 0))) {
	...
	case 'c': { // "afc"
		...
		if (!input[2] || input[2] == ' ' || input[2] == 'i' || input[2] == 'r' || input[2] == 'a') {
			fcn = r_anal_get_fcn_in (core->anal, core->offset, 0);
			if (!fcn) {
				R_LOG_WARN ("afc: Cannot find function here");
				break;
			}
		}
		switch (input[2]) {
		...
		case ' ': { // "afc "
				  ...
				  if (!r_anal_cc_exist (core->anal, cc)) {
					  ...
					  R_LOG_WARN ("afc: Unknown calling convention '%s' for '%s'\n"
							  "See afcl for available types", cc, asmOs);
				  } else {
		...
		case 'o': { // "afco"
			...
			if (R_STR_ISNOTEMPTY (dbpath) && r_file_exists (dbpath)) {
				...
				r_cons_printf ("Usage: afco [dbpath] - open calling conventions defined in local file.\n");
			}
			free (dbpath);
			break;
		}
	...
	case 'B': // "afB" // set function bits
		if (input[2] == ' ') {
			...
			if (fcn) { // bits=0 means unset
				...
				R_LOG_WARN ("afB: Cannot find function to set bits at 0x%08"PFMT64x, core->offset);
			}
		} else {
			r_cons_printf ("Usage: afB [bits] # bits can be: 0, 8, 16, 32 or 64. when using 0, disables the hint\n");
		}
		break;
	case 'b': // "afb"
		switch (input[2]) {
	...
	case 'n': // "afn"
		...
		case ' ': { // "afn "
			...
			if (*name == '?') {
				r_cons_printf ("Usage: afn newname [off]   # set new name to given function\n");
			} else {
				...
				if (!*name || !__setFunctionName (core, off, name, false)) {
					R_LOG_WARN ("Cannot find function at 0x%08"PFMT64x, off);
				}
			}
			free (name);
			}
	...
	case 'x': // "afx"
		...
		case ' ': // "afx "
		{
			...
			// list xrefs from current address
			{
				...
				if (fcn) {
					...
					R_LOG_WARN ("afx: Cannot find function at 0x%08"PFMT64x, addr);
				}
			}
			if (input[2] == 'j') {
				pj_end (pj);
		...
		default:
			R_LOG_WARN ("Wrong command. Look at af?");
			break;
		}
		break;
	case 'F': // "afF"
...
//				r_core_cmd0 (core, "ar A0,A1,A2,A3");
			...
			if (*name && name[1]) {
				...
				if (r) {
					...
					if (rf) {
						...
						R_LOG_WARN ("unknown conditional or flag register");
					}
				}
			} else {
				RRegFlags *rf = r_reg_cond_retrieve (core->dbg->reg, NULL);
	...
	case 'n': // "arn"
		if (*(str + 1) == '\0') {
			R_LOG_WARN ("Oops. try arn [PC|SP|BP|SN|A0|A1|A2|A3|A4|R0|R1|ZF|SF|NF|OF]");
			break;
		}
		...
		if (arg) {
			*arg = 0;
			...
			if (r) {
				...
				R_LOG_WARN ("ar: Unknown register '%s'", regname);
			}
			free (ostr);
			return;
		}
...
#define return_tail(x) { tail_return_value = x; goto tail_return; }
	...
	if (R_STR_ISEMPTY (_pcname)) {
		R_LOG_WARN ("Cannot find =PC in current reg profile.");
		return 0;
	}
	...
	for (; ; r_anal_op_fini (&op)) {
		if (r_cons_is_breaked ()) {
			R_LOG_WARN ("[+] ESIL emulation interrupted at 0x%08"PFMT64x, addr);
			return_tail (0);
		}
		...
		if (esiltimeout > 0) {
			...
			if (elapsedTime >= esiltimeout) {
				R_LOG_WARN ("[ESIL] Timeout exceeded.");
				return_tail (0);
			}
		}
		if (!esil) {
		...
		if (esil->exectrap) {
			if (!r_io_is_valid_offset (core->io, addr, R_PERM_X)) {
				...
				esil->trap_code = addr;
				R_LOG_WARN ("[ESIL] Trap, trying to execute on non-executable memory");
				return_tail (1);
			}
		} else {
		...
		if (op.size < 1 || ret < 1) {
			...
			if (breakoninvalid) {
				R_LOG_WARN ("[ESIL] Stopped execution in an invalid instruction (see e??esil.breakoninvalid)");
				return_tail (0);
			}
		...
		if (ret) {
			...
			if (op.type == R_ANAL_OP_TYPE_CJMP) {
				...
				ut64 pc = r_reg_getv (core->anal->reg, pcname);
				if (err) {
					R_LOG_WARN ("Missing PC register in the current profile.");
					break;
				}
				if (pc == addr + op.size) {
					// do not opdelay here
			...
			if (op.delay && !isNextFall) {
				...
				if (ret > 0) {
					...
					case R_ANAL_OP_TYPE_RET:
						...
						esil->trap = R_ANAL_TRAP_EXEC_ERR;
						esil->trap_code = addr;
						R_LOG_WARN ("[ESIL] Trap, trying to execute a branch in a delay slot");
						return_tail (1);
						break;
					}
					...
					R_LOG_WARN ("Invalid instruction at 0x%08"PFMT64x, naddr);
				}
				r_anal_op_fini (&op2);
			}
			tail_return_value = 1;
		...
		ut64 pc = r_reg_getv (core->anal->reg, pcname);
		if (pc == UT64_MAX) {
			R_LOG_WARN ("Invalid program counter PC=-1");
			break;
		}
...
R_API bool r_core_esil_step_back(RCore *core) {
	...
	if (!core->anal->esil || !core->anal->esil->trace) {
		r_cons_printf ("Run `aeim` to initialize the esil VM and enable e dbg.trace=true\n");
		return false;
	}
	RAnalEsil *esil = core->anal->esil;
	if (esil->trace->idx > 0) {
	...
	case 'i': // "aii"
		...
		if (input[1]) {
			if (input[1] == ' ') {
				...
				r_cons_printf ("Usagae: aii [namespace] # see afii - imports\n");
			}
		} else {
...
static void cmd_esil_mem(RCore *core, const char *input) {
	...
	if (*input == '-') {
		if (esil->stack_fd > 2) {	//0, 1, 2 are reserved for stdio/stderr
			...
			R_LOG_WARN ("Cannot deinitialize %s", name);
		}
	...
	if (!(stack_map = r_io_map_add (core->io, esil->stack_fd, R_PERM_RW, 0LL, addr, size))) {
		r_io_fd_close (core->io, esil->stack_fd);
		R_LOG_WARN ("Cannot create map for tha stack, fd %d got closed again", esil->stack_fd);
		esil->stack_fd = 0;
		return;
	}
...
#	define hasNext(x) (x&1) ? (addr<addr_end) : (ops<ops_end)
	...
	for (ops = ptr = 0; ptr < buf_sz && hasNext (mode); ops++, ptr += len) {
		...
		if (R_STR_ISNOTEMPTY (esilstr)) {
			if (len < 1) {
				R_LOG_WARN ("Invalid 0x%08"PFMT64x" instruction %02x %02x",
...
static void cmd_aespc(RCore *core, ut64 addr, ut64 until_addr, int ninstr) {
	...
	buf = malloc (bsize);
	if (!buf) {
		R_LOG_WARN ("Cannot allocate %d byte(s)", bsize);
		return;
	}
	if (addr == -1) {
	...
	for (i = 0, j = 0; j < ninstr; i++, j++) {
		...
		if (i >= (bsize - 32)) {
			i = 0;
			R_LOG_WARN ("Warning: Chomp %d of %d", i, bsize);
		}
		if (!i) {
		...
		if (ret < 1) {
			R_LOG_WARN ("Failed analysis at 0x%08"PFMT64x, addr);
			break;
		}
		// skip calls and such
		switch (aop.type) {
...
static void r_anal_aefa(RCore *core, const char *arg) {
	...
	if (!from || from == UT64_MAX) {
		if (fcn) {
			...
			r_cons_printf ("Usage: aefa [from] # if no from address is given, uses fcn.addr\n");
			return;
		}
	}
	r_cons_printf ("Emulate from 0x%08"PFMT64x" to 0x%08"PFMT64x"\n", from, to);
	r_cons_printf ("Resolve call args for 0x%08"PFMT64x"\n", to);
...
static void __anal_esil_function(RCore *core, ut64 addr) {
	...
	if (fcn) {
		...
		R_LOG_WARN ("Cannot find function at 0x%08"PFMT64x, addr);
	}
...
static void cmd_anal_esil(RCore *core, const char *input, bool verbose) {
	...
	case 'p': // "aep"
		...
		case 'c': // "aepc"
			if (input[2] == ' ' || input[2] == '=') {
				...
				r_cons_printf ("Usage: aepc [address]  # same as 'ar PC=..'\n");
			}
			break;
		case 'k':
			{
	...
	case '*':
		...
		if (core->anal->esil) {
			...
			R_LOG_WARN ("esil vm not initialized. run `aei`");
		}
		break;
	case ' ':
	case 'q':
	...
	case 's': // "aes"
		...
		case 'B': // "aesB"
			...
			if (n) {
				...
				r_cons_printf ("Usage: aesB [until-addr] [nth-opcodes] @ [from-addr]\n");
			}
			}
			break;
		case 'u': // "aesu"
		...
		case 'o': // "aeso"
			if (input[2] == 'u') { // "aesou"
				...
				r_cons_printf ("Usage: aesou [addr] # step over until given address\n");
			}
			break;
		case 'p': //"aesp"
			n = strchr (input, ' ');
	...
	case 'c': // "aec"
		if (input[1] == '?') { // "aec?"
			...
			for (; !maxsteps || countsteps < maxsteps; countsteps++) {
				...
				if (!op) {
					R_LOG_WARN ("invalid instruction at 0x%08"PFMT64x, addr);
					break;
				}
				if (op->type == R_ANAL_OP_TYPE_SWI) {
					r_cons_printf ("syscall instruction at 0x%08" PFMT64x "\n", addr);
					break;
				} else if (op->type == R_ANAL_OP_TYPE_TRAP) {
					r_cons_printf ("trap instruction at 0x%08" PFMT64x "\n", addr);
					break;
				}
				r_anal_op_free (op);
				op = NULL;
			...
			for (; !maxsteps || countsteps < maxsteps; countsteps++) {
				...
				if (op->type == R_ANAL_OP_TYPE_CALL || op->type == R_ANAL_OP_TYPE_UCALL) {
					r_cons_printf ("stop in call instruction at 0x%08" PFMT64x "\n", addr);
					break;
				}
				r_anal_op_free (op);
				op = NULL;
	...
	case 'k': // "aek"
		...
		case ' ':
			if (esil && esil->stats) {
				...
				R_LOG_WARN ("esil.stats is empty. Run 'aei'");
			}
			break;
		case '-':
			if (esil) {
	...
	case 'b': // "aeb"
		{
			...
			if (fcn) {
				...
				if (bb) {
					...
					R_LOG_WARN ("No basic block in this address");
				}
			} else {
				R_LOG_WARN ("No function in this address");
			}
		// ab~ninstr[1]
		}
		break;
	...
	case 't': // "aet"
		...
		case 's': // "aets"
			...
			case '+': // "aets+"
				if (!esil) {
					R_LOG_WARN ("Error: ESIL is not initialized. Use `aeim` first.");
					break;
				}
				if (esil->trace) {
					R_LOG_WARN ("ESIL trace already started");
					break;
				}
			...
			case '-': // "aets-"
				...
				if (!esil->trace) {
					R_LOG_WARN ("No ESIL trace started");
					break;
				}
			...
			break;
		default:
			R_LOG_WARN ("Unknown command. Use `aetr`");
			break;
		}
		break;
	case 'A': // "aeA"
	...
	case 'd': // "aod"
		if (input[1] == 'a') { // "aoda"
			...
			if (d && *d) {
				...
				R_LOG_WARN ("Unknown mnemonic");
			}
		} else {
			R_LOG_WARN ("Use: aod[?a] ([opcode])    describe current, [given] or all mnemonics");
		}
		break;
	case '*':
	...
	case 'f': // "aof"
		if (strlen (input + 1) > 1) {
			...
			if (ret > 0) {
				...
				R_LOG_WARN ("Warning: Unable to analyze instruction");
			}
		}
		break;
	default:
		...
		if (map && (map->perm & R_PERM_X)) {
			...
			if (r_io_map_size (map) > bf->size) {
				...
				if (bf->size > map->delta) {
					...
					R_LOG_WARN ("Opps something went wrong aac");
					return;
				}
			} else {
				addr = r_io_map_begin (map);
...
static void cmd_anal_blocks(RCore *core, const char *input) {
	...
	if (!arg) {
		...
		r_list_foreach (list, iter, map) {
			...
			if (!from && !to) {
				R_LOG_WARN ("Cannot determine search boundaries");
...
static void _anal_calls(RCore *core, ut64 addr, ut64 addr_end, bool printCommands, bool importsOnly) {
	...
	if (!buf || !block0 || !block1) {
		R_LOG_ERROR ("Error: cannot allocate buf or block");
		free (buf);
		free (block0);
		free (block1);
		return;
...
static void cmd_sdbk(Sdb *db, const char *input) {
	...
	if (out) {
		...
		R_LOG_WARN ("|ERROR| Usage: ask [query]");
	}
}
		...
		if (input[1] == ' ') {
			...
			if (sc_number != 0) {
				...
				if (si) {
					...
					R_LOG_WARN ("Unknown syscall number");
				}
			}
		} else {
			list = r_syscall_list (core->anal->syscall);
	...
	case 'f':
		if (input[1] == 'f') { // "axff"
			...
			if (fcn) {
				...
				R_LOG_WARN ("Cannot find any function");
			}
			pj_free (pj);
		} else { // "axf"
			if (input[1] == '?') { // "axf?"
...
static void cmd_anal_hint(RCore *core, const char *input) {
	...
	case 'o': // "aho"
		if (input[1] == '*') {
			...
			if (type != -1) {
				...
				R_LOG_WARN ("Unknown opcode type. Try: io, acmp, add, sync, call, cjmp, cmp, nop, ...");
			}
		} else {
	...
	case 'b': // "ahb" set bits
		if (input[1] == '?') {
			...
			if (!strcmp (input + 2, "*")) {
				R_LOG_WARN ("Delete all asm.bits hints is not yet supported.");
			} else {
				ut64 off = input[2]? r_num_math (core->num, input + 2): core->offset;
				r_anal_hint_unset_bits (core->anal, off);
			}
	...
	case 'd': // "ahd" set opcode string
		if (input[1] == ' ') {
			...
			r_cons_printf ("Usage: ahd popall\n");
		}
		break;
		...
		case ' ': {
			...
			if (ret >= 0) {
				...
				if (offimm != 0) {
					...
					if (!otype) {
						R_LOG_WARN ("wrong type for opcode offset");
					}
					r_list_free (otypes);
				}
			}
...
static bool convert_dot_to_image(RCore *core, const char *dot_file, const char *save_path) {
	...
	if (save_path && *save_path) {
		...
		if (viewer) {
			...
			R_LOG_WARN ("Cannot find a valid picture viewer");
			goto end;
		}
	}
	r_core_cmd0 (core, cmd);
	...
	case ' ': { // "agn"
		...
		if (n_args > 1) {
			...
			if (strncmp (body, "base64:", B_LEN) == 0) {
				...
				free (body);
				if (!newbody) {
					R_LOG_WARN ("Cannot allocate buffer");
					r_str_argv_free (args);
					break;
				}
				body = newbody;
...
R_API void r_core_agraph_print(RCore *core, int use_utf, const char *input) {
	...
	case 'i': // "aggi" - open current core->graph in interactive mode
	{
		...
		if (ran) {
			...
			R_LOG_WARN ("This graph contains no nodes");
		}
		break;
	}
...
static void cmd_anal_graph(RCore *core, const char *input) {
	...
	case 'f': // "agf"
		...
		case 'v': // "agfv"
			r_cons_printf ("\rRendering graph...\n");
	...
	case 'x': {// "agx" cross refs
		...
		if (!graph) {
			R_LOG_WARN ("Couldn't create graph");
			break;
		}
	...
	if (!n) {
		...
		if (cfg_debug) {
			...
			r_list_foreach (list, iter, map) {
				...
				if (!from && !to) {
					R_LOG_WARN ("Cannot determine xref search boundaries");
				} else if (to - from > UT32_MAX) {
					R_LOG_WARN ("Skipping huge range");
				} else {
					if (rad == 'j') {
		...
		R_LOG_WARN ("Invalid number of arguments");
	}
	free (ptr);
	...
	if (is_debug) {
		...
		r_list_foreach (list, iter2, map2) {
			...
			if ((to - from) > MAX_SCAN_SIZE) {
				R_LOG_WARN ("Warning: Skipping large region");
				continue;
			}
			r_list_foreach (list, iter, map) {
...
static void cmd_anal_abt(RCore *core, const char *input) {
	...
	case 'e':
		...
		if (!p) {
			r_cons_printf ("Usage: abte [addr] # emulate from beginning of function to the given address.\n");
			return;
		}
	...
	case ' ': {
		...
		if (addr == UT64_MAX || addr == 0) {
			R_LOG_WARN ("Invalid or missing address passed as argument");
		} else {
			RAnalBlock *block = r_anal_get_block_at (core->anal, core->offset);
			if (!block) {
				R_LOG_WARN ("No basic block at");
				break;
			}
			RList *path = r_anal_block_shortest_path (block, addr);
			if (path) {
...
static int cmd_anal_all(RCore *core, const char *input) {
	...
	case 'c': // "aac"
		...
		case 'i': // "aaci"
			...
			break;
		case '?': // "aac?"
			r_cons_printf ("Usage: aac, aac* or aaci (imports xrefs only)\n");
			break;
		default: // "aac"
	...
	case 'F': // "aaF" "aaFa"
		if (!input[1] || input[1] == ' ' || input[1] == 'a') {
			...
			r_cons_printf ("Usage: aaF[a] - analyze functions in flag bounds (aaFa uses af/a2f instead of af+/afb+)\n");
		}
		break;
	case 'n': // "aan"
		switch (input[1]) {
	...
	case 'a':
		if (strchr (input, '?')) {
			...
			if (!strncmp (input, "aaaaa", 5)) {
				r_cons_printf ("An r2 developer is coming to your place to manually analyze this program. Please wait for it\n");
	...
	case 't': { // "aat"
		...
		if (off && *off) {
			...
			if (fcn) {
				...
				R_LOG_WARN ("Cannot find function at %08"PFMT64x, addr);
			}
		} else {
			if (r_list_empty (core->anal->fcns)) {
				R_LOG_WARN ("Couldn't find any functions");
				break;
			}
			r_list_foreach (core->anal->fcns, it, fcn) {
				if (r_cons_is_breaked ()) {
		...
		if (input[1] == 'f') { // "aaef"
			...
			if (!strcmp ("range", r_config_get (core->config, "anal.in"))) {
				...
				if (to > from) {
					...
					r_cons_printf ("Assert: anal.from > anal.to\n");
				}
			} else {
				r_list_foreach (list, iter, map) {
					if (map->perm & R_PERM_X) {
...
static void cmd_anal_class_method(RCore *core, const char *input) {
	...
	case 'n': { // "acmn"
		...
		if (!*str) {
			R_LOG_WARN ("No class name given.");
			break;
		}
		...
		if (!end) {
			R_LOG_WARN ("No method name given.");
			free (cstr);
			break;
		}
		*end = '\0';
		...
		if (c == ' ' || c == 'n') {
			...
			if (!end) {
				if (c == ' ') {
					R_LOG_WARN ("No offset given.");
				} else if (c == 'n') {
					R_LOG_WARN ("No new method name given.");
				}
				free (cstr);
				break;
			}
	...
	switch (err) {
		case R_ANAL_CLASS_ERR_NONEXISTENT_CLASS:
			R_LOG_WARN ("Class does not exist.");
			break;
		case R_ANAL_CLASS_ERR_NONEXISTENT_ATTR:
			R_LOG_WARN ("Method does not exist.");
			break;
		default:
			break;
	}
	...
	case '-': { // "acv-"
		...
		if (!end || *end == '\0') {
			if (c == ' ') {
				...
				R_LOG_WARN ("No vtable id given. See acv [class name].");
			}
			free (cstr);
			break;
		}
...
static void cmd_anal_classes(RCore *core, const char *input) {
	...
	case 'n': { // "acn"
		...
		if (input[0] == '-') {
			...
			if (!end) {
				R_LOG_WARN ("No new class name given.");
			} else {
				char *new_name = end + 1;
				end = strchr (new_name, ' ');
				if (end) {
				...
				if (err == R_ANAL_CLASS_ERR_NONEXISTENT_CLASS) {
					...
					R_LOG_WARN ("A class with this name already exists.");
				}
			}
		} else {
	...
	if (go_on) {
		...
		if (key) {
			...
			if (fcn_type) {
				...
				R_LOG_WARN ("Cannot find any function type..lets just use some standards?");
			}
			free (key);
		} else {
			if (is_aCer) {
...
static bool core_anal_abf(RCore *core, const char* input) {
	if (strchr (input, '?')) {
		r_cons_printf ("Usage: abf ([addr])  # list incoming/from basic blocks\n");
		return false;
	}
	ut64 addr = r_num_math (core->num, input);
	if (!addr || addr == UT64_MAX) {
	...
	case 'p': // "ap"
		if (input[1] == '?') {
			...
			if (pos) {
				...
				R_LOG_WARN ("POS = %d", delta);
				R_LOG_WARN ("HIT = 0x%"PFMT64x, off - delta);
				r_cons_printf ("0x%08"PFMT64x"\n", off - delta);
			} else {
				R_LOG_WARN ("Cannot find prelude");
			}
			free (buf);
		}
		break;
