...
static void helpCmdTasks(RCore *core) {
	const char *help_msg[] = {
		"Usage:", "&[-|<cmd>]", "Управление задачами",
		"&", "", "Список всех запущенных задач",
		"&=", "", "Показать вывод всех задач ",
		"&=", " 3", "Показать результат задачи 3",
		"&j", "", "Список всех запущенных задач (в JSON)",
		"&?", "", "Показать эту справку",
		"&+", " aa", "Добавить в список задач",
		"&-", " 1", "Удалить задачу №1",
		"&", "-*", "Удалить все задачи",
		"&", " aa", "Запустить анализ в фоновом режиме",
		"&", " &&", "Запускать все задания в фоновом режиме",
		"&&", "", "Запускать все задачи наложения (и объединять задачи)",
		"&&&", "", "Запускать все задачи наложения до ^C",
		"", "", "TODO: Последняя команда должна выполнять asm.bits",
		"", "", "WARN: Эта функция экспериментальна. Используйте его с осторожностью",
		NULL
	};
...
static void helpCmdForeach(RCore *core) {
	const char *help_msg[] = {
		"@@", "", " # Команда foreach iterator:",
		"Repeat a command over a list of offsets", "", "",
		"x", " @@ sym.*", "Запустите 'x' над всеми флагами, соответствующими 'sym.' В текущем пространстве флагов",
		"x", " @@dbt[abs]", "Запускать команду по каждому адресу backtrace, bp или sp",
		"x", " @@.file", "\"\" по смещениям, указанным в файле (по одному смещению на строку)",
		"x", " @@=off1 off2 ..", "Ручной список смещений",
		"x", " @@/x 9090", "Временный набор cmd.hit для запуска команды по каждому результату поиска",
		"x", " @@k sdbquery", "\"\" На все смещения, возвращаемые этим sdbquery",
		"x", " @@t", "\"\" По всем потокам (см. dp)",
		"x", " @@b", "\"\" На все основные блоки текущей функции (см. afb)",
		"x", " @@i", "\"\" По всем командам текущей функции (см. pdr)",
		"x", " @@f", "\"\" На все функции (см. aflq)",
		"x", " @@f:write", "\"\" На всех соответствующих функциях напиcfnm в названии",
		"x", " @@c:cmd", "То же, что и @@ = `` без обратных кавычек",
		"x", " @@=`pdf~call[0]`", "Запустите 'x' при каждом смещении вызова текущей функции",
...
static void helpCmdAt(RCore *core) {
	const char *help_msg[] = {
		...
		"0", "", "Псевдоним для 's 0'",
		"0x", "addr", "Псевдоним для 's 0x..'",
		"#", "cmd", "Если # - число, повторить команду # times",
		"/*", "", "Начать многострочный комментарий",
		"*/", "", "Закончить многострочный комментарий",
		".", "cmd", "Выполнить вывод команды в виде скрипта r2",
		".:", "8080", "Ждать команд на порту 8080",
		".!", "rabin2 -re $FILE", "Вывести командный вывод как скрипт r2",
		"*", "", "Вывод команды в формате r2 script(CC*)",
		"j", "", "Вывод команды в формате JSON (pdj)",
		"~", "?", "Подсчитать количество строк (например, wc -l)",
		"~", "??", "Показать внутреннюю grep-помощь",
		"~", "..", "Внутреннее уменьшение",
		"~", "{}", "json отступ",
		"~", "{}..", "json отсут и уменьшение",
		"~", "word", "Grep для строк, соответствующих слову",
		"~", "!word", "Grep для строк, не соответствующих слову",
		"~", "word[2]", "Grep 3-й столбец строк, соответствующих слову",
		"~", "word:3[0]", "Grep 1-й столбец из 4-го строчного совпадающего слова",
		"@", " 0x1024", "Временный поиск по этому адресу (sym.main + 3)",
		"@", " addr[!blocksize]", "Временно установить новый размер блока",
		"@a:", "arch[:bits]", "Временная связь и биты",
		"@b:", "bits", "Временно установить asm.bits",
		"@e:", "k=v,k=v", "Временное изменение eval vars",
		"@r:", "reg", "Tmp ищет значение reg(f.ex pd@r:PC)",
		"@i:", "nth.op", "Временный поиск в N-й относительной инструкции",
		"@f:", "file", "Временно заменить блок содержимым файла",
		"@o:", "fd", "Временный переход на другой диск",
		"@s:", "string", "То же, что и выше, но из строки",
		"@x:", "909192", "Из шестнадцатеричной строки",
		"@..", "from to", "Временный набор из и для команд, поддерживающих диапазоны",
		"@@=", "1 2 3", "Запустите предыдущую команду со смещениями 1, 2 и 3 ",
		"@@", " hit*", "Запустите команду для каждого флага, соответствующего 'hit *'",
		"@@?", "[ktfb..]", "Показать справку для оператора итератора",
		"@@@", " [type]", "Выполните команду для каждого [типа] (см. @@@? Для справки) ",
		">", "file", "Вывод команды pipe в файл",
		">>", "file", "Добавить в файл",
		"H>", "file", "Вывод команды pipe в файл в формате HTML",
		"H>>", "file", "Добавить в файл с выходом команды в HTML ",
		"`", "pdi~push:0[0]`", "Заменить вывод команды внутри строки",
		"|", "cmd", "Вывод команды pipe (pd | less) (.dr *)",
		NULL
	};
	r_core_cmd_help (core, help_msg);
}
...
static void print_format_help(RCore *core) {
	const char *help_msg[] = {
		"pf:", PF_USAGE_STR, "",
		"Commands:", "", "",
		"pf", "?", "Показать эту справку",
		"pf", "??", "Форматирование символов",
		"pf", "???", "Примеры использования pf ",
		"pf", " fmt", "Показать данные, использующие заданную строку формата. См. 'pf \? \?' И 'pf \? \? \?'.",
		"pf.", "fmt_name", "Показать данные с использованием именованного формата",
		"pf.", "fmt_name.field_name", "Показать конкретное поле данных с использованием именованного формата ",
		"pfj ", "fmt_name|fmt", "Показать данные с использованием (названного) формата в JSON",
		"pf* ", "fmt_name|fmt", "Показать данные с использованием (именованного) формата в качестве флага r2 для создания команд",
		"pfd.", "fmt_name", "Показать данные с использованием именованного формата в виде команд graphviz",
		"pf.", "name [0|cnt]fmt", "Определение нового именованного формата",
		"pf.", "", "Список всех  форматов определения",
		"pf?", "fmt_name", "Показать определение именованного формата",
		"pfo", "", "Перечислить все файлы определения формата (fdf)",
		"pfo", " fdf_name", "Загрузить файл определения формата (fdf)",
		"pf.", "fmt_name.field_name=33", "Установить новое значение для указанного поля в именованном формате",
		"pfv.", "fmt_name[.field]", "Печатать значение (и) только для именованного формата. Полезно для однострочников",
		"pfs", " fmt_name|fmt", "Вывести размер (именованного) формата в байтах",
		NULL
	};
	r_core_cmd_help (core, help_msg);
}
...
static void print_format_help_help(RCore *core) {
	const char *help_msg[] = {
		...
		"Format:", "", "",
		" ", "b", "Byte (без знака)",
		" ", "B", "Разрешить перечислимое битовое поле (см. t?)",
		" ", "c", "char (знаковый)",
		" ", "d", "0x%%08x Шестнадцатеричное значение (4 байта) (см. %%i и %% ) ",
		" ", "D", "Разобрать один опкод ",
		" ", "e", "Временно обмениять endian ",
		" ", "E", "Разрешить имя перечисления (см. T?) ",
		" ", "f", "Float value (4 байта) ",
		" ", "i", "%%i Целое число со знаком (4 байта) (см. %% d и %% x)",
		" ", "n", "Next char указывает размер знакового значения (1, 2, 4 или 8 байт).",
		" ", "N", "Next char указывает размер беззнакового значения (1, 2, 4 или 8 байт (ов))",
		" ", "o", "0x%%08o Восьмеричное значение (4 байта)",
		" ", "p", "Ссылка указателя (2, 4 или 8 байтов)",
		" ", "q", "Quadword (8 байт)",
		" ", "r", "Регистр процессора `pf r (eax) plop`",
		" ", "s", "32-битный указатель на строку (4 байта)",
		" ", "S", "64-битный указатель на строку (8 байтов)",
		" ", "t", "Временная метка UNIX (4 байта)",
		" ", "T", "Показать десять первых байтов буфера",
		" ", "u", "Uleb128 (переменная длина)",
		" ", "w", "Word (2 байта без знака short в шестнадцатеричном) ",
		" ", "x", "0x%%08xШестнадцатеричное значение и флаг (fd @ addr) (см. %%d и %%i) ",
		" ", "X", "Показать отформатированные hexpairs",
		" ", "z", "\\0 Завершенная строка ",
		" ", "Z", "\\0 Завершенная широкая строка ",
		" ", "?", "Структура данных `pf? (Struct_name) example_name` ",
		" ", "*", "Следующий символ - указатель (отличия asm.bits)",
		" ", "+", "Показывать флаги для каждого смещения",
		" ", ":", "Пропустить 4 байта ",
		" ", ".", "Пропустить 1 байт ",
		NULL
	};
...
static void print_format_help_help_help(RCore *core) {
	const char *help_msg[] = {
		"pf:", PF_USAGE_STR, "",
		"Examples:", "", "",
		"pf", " 3xi foo bar", "3-массив структуры, каждый с именованными полями: 'foo' как hex, и 'bar' как int",
		"pf", " B (BitFldType)arg_name`", "Тип битового поля",
		"pf", " E (EnumType)arg_name`", "Тип перечисления",
		"pf.", "obj xxdz prev next size name", "Определите формат obj как xxdz",
		"pf", " obj=xxdz prev next size name", "То же, что и выше",
		"pf", " iwq foo bar troll", "Распечатать формат iwq с помощью foo, bar, troll как соответствующие имена для полей",
		"pf", " 0iwq foo bar troll", "То же, что указано выше, но рассматривается как объединение (все поля со смещением 0)",
		"pf.", "plop ? (troll)mystruct", "Использовать ранее заданный структурный тролль ",
		"pf", " 10xiz pointer length string", "Вывести размер массива xiz struct размером 10 с его именами полей ",
		"pf", " {integer}? (bifc)", "Печать целого числа следующего формата (bifc) ",
		"pf", " [4]w[7]i", "Вывести массив из 4 слов, а затем массив из 7 целых чисел",
		"pf", " ic...?i foo bar \"(pf xw yo foo)troll\" yo", "Печатать вложенные анонимные структуры",
		"pf", "n2", "Печатать signed short (2 байта). Используйте N вместо n для печати беззнаковых значений",
		NULL
	};
	r_core_cmd_help (core, help_msg);
}
...
static void cmd_print_pv(RCore *core, const char *input) {
	...
	const char *help_msg[] = {
		...
		"pv", "", "Печатать байты на основе asm.bits",
		"pv1", "", "Печатать 1 байт в памяти",
		"pv2", "", "Печатать 2 байта в памяти",
		"pv4", "", "Печатать 8 байта в памяти",
		"pv8", "", "Печатать 4 байта в памяти",
		"pvz", "", "Напечатать значение в виде строки (псевдоним для ps)",
		NULL
	};
	switch (input[0]) {
	case '1':
...
static void cmd_print_bars(RCore *core, const char *input) {
	...
	case '?': { // bars
		const char *help_msg[] = {
			"Usage:", "p=[bep?][qj] [nblocks] ([len]) ([offset]) ", "Показать энтропию / печатные символы / штрихи",
			"p=", "", "Печатать байты текущего блока в барах",
			"p=", "b", "Так же, как выше",
			"p=", "c", "Напечатать количество вызовов на блок",
			"p=", "d", "Выводить разные байты из блока",
			"p=", "e", "Напечатать энтропию для каждого размера файла / блока ",
			"p=", "F", "Напечатать число байтов 0xFF для каждого размера файла/размера блока",
			"p=", "i", "Напечатать количество недействительных инструкций на блок",
			"p=", "j", "Напечатать количество скачков и условных скачков в блоке",
			"p=", "m", "Напечатать количество флагов и меток в блоке",
			"p=", "p", "Напечатать количество печатных байтов для каждого размера файла / блока",
			"p=", "s", "Напечатать номер системного вызова и привилегированные инструкции ",
			"p=", "z", "Напечатать число символов в строках в блоке ",
			"p=", "0", "Напечатать число байтов 0x00 для каждого размера файла/размера блока",
			NULL
		};
		r_core_cmd_help (core, help_msg);
		  }
...
static int cmd_print(void *data, const char *input) {
	...
	case '-': // "p-"
		...
		case '?': {
			const char *help_msg[] = {
				"Usage:", "p%%[jh] [pieces]", "Блоки Bar|json|гистограммы ",
				"p-", "", "Показать панель ascii-art метаданных в границах файлов",
				"p-j", "", "Показать формат json",
				"p-h", "", "Показать гистограммный анализ метаданных на блок",
				NULL
			};
			r_core_cmd_help (core, help_msg);
		}
...
// r_cons_printf ("|Usage: pi[defj] [num]\n");
	...
	case 'd': // "pd"
	{
		...
		case '?': // "pd?"
			processed_cmd = true;
			const char *help_msg[] = {
				"Usage:", "p[dD][ajbrfils] [sz] [arch] [bits]", " # Печать Разборки",
				"NOTE: ", "len", "Параметр может быть отрицательным",
				"NOTE: ", "", "Нажатие клавиши ENTER по пустой команде будет повторять последнюю команду pd и также будет завершаться до конца разобранного диапазона.",
				"pd", " N", "Разобрать инструкции N",
				"pd", " -N", "Разобрать N инструкций назад ",
				"pD", " N", "Разобрать N байт",
				"pda", "", "Разобрать все возможные коды операций (байт на байт)",
				"pdb", "", "Разобрать основной блок",
				"pdc", "", "Вывод псевдо дизассемблера в синтаксисе C-like",
				"pdC", "", "Показать комментарии в инструкциях N",
				"pdk", "", "Разобрать все методы класса",
				"pdj", "", "Разобрать на json",
				"pdr", "", "Рекурсивный дизассемблировать по функциональному графику",
				"pdf", "", "Разобрать функцию ",
				"pdi", "", "Как 'pi', со смещением и байтами",
				"pdl", "", "Показать размеры команд",
				// "pds", "", "disassemble with back sweep (greedy disassembly backwards)",
				"pds", "[?]", "Разобрать суммарно (строки, вызовы, прыжки, ссылки) (см. Pdsf и pdfs)",
				"pdt", "", "Разобрать трассировки отладчика (см. Atd)",
				NULL
			};
			r_core_cmd_help (core, help_msg);
			pd_result = 0;
	...
	case 's': // "ps"
		...
		case '?': {
			const char *help_msg[] = {
				"Usage:", "ps[zpw] [N]", "Печать строки",
				"ps", "", "Печать строки",
				"psi", "", "Печать строки в curseek",
				"psb", "", "Печатать строки в текущем блоке",
				"psx", "", "Показать строку с экранированными символами",
                "psz", "", "печатать строку с нулевым концом",
                "psp", "", "print pascal string",
                "psu", "", "print utf16 unicode (json)",
                "psw", "", "print wide string",
                "psj", "", "print string в формате JSON",
				NULL
			};
			r_core_cmd_help (core, help_msg);
		}
	...
	case 'x': // "px"
	{
		...
		case '?': {
			const char *help_msg[] = {
				"Usage:", "px[afoswqWqQ][f]", " # Распечатать heXadecimal",
				"px", "", "Показать hexdump",
                "px/", "", "то же, что x/ in gdb (help x)",
                "pxa", "", "показать аннотированный hexdump",
                "pxA", "", "показать цветовую карту анализа op",
                "pxb", "", "dump биты в hexdump форме",
                "pxc", "", "показать hexdump с комментариями",
				"pxd", "[124]", "целый дамп со знаком (1 байт, 2 и 4)",
                "pxe", "", "Эмоции hexdump! :)",
                "pxi", "", "Компактное двоичное представление HexII",
                "pxf", "", "показать hexdump текущей функции",
                "pxh", "", "показать шестнадцатеричный дамп полуслов (16 бит)",
                "pxH", "", "то же, что указано выше, но по одному в строке",
                "pxl", "", "отобразить N строк (строк) hexdump",
                "pxo", "", "показать восьмеричный дамп",
				"pxq", "", "показать шестнадцатеричный дамп четырехъядерных слов (64 бит)",
                "pxQ", "", "то же, что указано выше, но по одному в строке",
                "pxr", "[j]", "показать слова со ссылками на флаги и код",
                "pxs", "", "показать шестнадцатеричный в разреженном режиме",
                "pxt", "[*.] [origin]", "Показать таблицу указателя дельты в командах r2",
                "pxw", "", "показать шестнадцатеричный дамп слов (32bit)",
                "pxW", "", "то же, что указано выше, но по одному в строке",
				NULL
			};
			r_core_cmd_help (core, help_msg);
		}
	...
	case 't': // "pt"
		...
		case '?': {
			const char *help_msg[] = {
				"Usage: pt", "[dn]", "Печать временных меток",
				"pt", "", "Печатать время UNIX (32 бит `cfg.bigendian`) С 1 января 1970 года",
				"ptd", "", "Печатать время DOS (32 бит `cfg.bigendian`) С 1 января 1980 года",
				"pth", "", "Печатать время HFC (32 бит `cfg.bigendian`) С 1 января 1904 года",
				"ptn", "", "Печатать время NTFC (64 бит `cfg.bigendian`) С 1 января 1601 года",
				NULL
			};
			r_core_cmd_help (core, help_msg);
		}
	...
	case 'z': // "pz"
		if (input[1] == '?') {
			const char *help_msg[] = {
				"Usage: pz [len]", "", "Печать увеличенных блоков (размер файла / N)",
				...
				"pzp", "", "Количество печатаемых символов",
				"pzf", "", "количество флагов в блоке",
                "pzs", "", "строки в диапазоне",
                "pz0", "", "количество байтов со значением '0'",
                "pzF", "", "количество байтов со значением 0xFF",
                "pze", "", "вычислить энтропию и расширить диапазон 0-255",
                "pzh", "", "head (первое значение байта); Это режим по умолчанию",
	...
	default: {
		const char *help_msg[] = {
			...
			"p=", "[?][bep] [blks] [len] [blk]", "Показать энтропию/печатные символы/штрихи символов",
			"p2", " [len]", "8x8 2bpp-tiles",
			"p3", " [file]", "Распечатать стереограмму (3D)",
			"p6", "[de] [len]", "base64 расшифровка/шифровка",
			"p8", "[?][j] [len]", "8-битный шестнадцатеричный список байтов",
			"pa", "[edD] [arg]", "Pa: собрать pa [dD]: disasm или pae: esil из шестнадцатеричных пар ",
			"pA", "[n_ops]", "Показать адрес и тип n_ops",
			"p", "[b|B|xb] [len] ([skip])", "Bindump N bits пропуская M",
			"pb", "[?] [n]", "Битовый поток N бит",
			"pB", "[?] [n]", "Битовый поток N байт",
			"pc", "[?][p] [len]", "Формат вывода C (или python)",
			"pC", "[d] [rows]", "Разбор печати в столбцах (см. Hex.cols и pdi)",
			"pd", "[?] [sz] [a] [b]", "Разобрать N опкодов (pd) или N байтов (pD)",
			"pf", "[?][.nam] [fmt]", "Распечатать отформатированные данные (имя_файла, имя_файла $<expr>)",
			"ph", "[?][=|hash] ([len])", "Вычислять хеш для блока",
			"p", "[iI][df] [len]", "Печатать N ops/bytes (f=func) (см. pi? и pdi)",
			"pm", "[?] [magic]", "Напечатать данные libmagic (см. Pm? и / m?)",
			"pr", "[?][glx] [len]", "Печатать N необработанных байтов (в строках или шестнадцатеричных блоках, 'g'unzip)",
			"p", "[kK] [len]", "Печатать ключ в randomart (K для мозаики)",
			"ps", "[?][pwz] [len]", "Print pascal / wide / строки с нулевым символом ",
			"pt", "[?][dn] [len]", "Печать различных временных меток ",
			"pu", "[?][w] [len]", "Печать N URL закодированные байт (w = широкий)",
			"pv", "[?][jh] [mode]", "Показать переменную / указатель / значение в памяти",
			"p-", "[?][jh] [mode]", "Bar | json | блоки гистограммы (режим: e? Search.in) ",
			"px", "[?][owq] [len]", "Hexdump из N байтов (o = восьмеричный, w = 32bit, q = 64bit)",
			"pz", "[?] [len]", "вид печати трансфокатор (см pz? для помощи)",
