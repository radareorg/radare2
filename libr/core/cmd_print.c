		...
		default:{ // p--
			if (off >= at && off < ate) {
				...
				if (use_color) {
					if (s) {
						if (s->perm & R_PERM_X) {
							r_cons_print (r_cons_singleton ()->context->pal.bg_trufae);
						} else {
							r_cons_print (r_cons_singleton ()->context->pal.bg_true);
						}
					} else {
						r_cons_print (r_cons_singleton ()->context->pal.bg_false);
					}
				}
