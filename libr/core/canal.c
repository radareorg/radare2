...
// main analysis loop can correctly switch between ARM32 and Thumb modes.
// This must run after bin loading (which creates initial hints from symbols)
// and before function analysis (aac/af).
R_API void r_core_anal_arm_thumb_scan(RCore *core) {
	R_RETURN_IF_FAIL (core && core->anal);
	const char *arch = r_config_get (core->config, "asm.arch");
	if (!r_str_startswith (arch, "arm")) {
		return;
	}
	...
	r_list_foreach (ranges, iter, map) {
		...
		if (end - addr > 32 * 1024 * 1024) {
			// skip very large regions to avoid slowdown
			continue;
		}
		int cur_bits = bits;
			...
			while (i < toread - 3 && !r_cons_is_breaked (core->cons)) {
				...
				if (cur_bits == 32) {
					...
					if ((insn & 0xfe000000) == 0xfa000000) {
						// compute target: PC + sign_extend(imm24:H:0)
						st32 offset = (st32)((insn & 0x00ffffff) << 2);
						if (offset & 0x02000000) {
							offset |= (st32)0xfc000000; // sign extend
						}
						int H = (insn >> 24) & 1;
						offset += H * 2;
						ut64 target = (addr + i) + 8 + offset; // PC = addr + 8 in ARM
						// BLX immediate always switches to Thumb
						r_anal_hint_set_bits (core->anal, target, 16);
						hints_added++;
					}
				} else {
					...
					// First halfword: 1111 0Sxx xxxx xxxx (0xf000..0xf7ff)
					if ((hw0 & 0xf800) == 0xf000 && i + 4 <= toread) {
						ut16 hw1 = r_read_le16 (buf + i + 2);
						insn_size = 4;
						// BL: hw1 = 1101 Hxxx xxxx xxxx (0xd000..0xdfff)
						// BLX: hw1 = 1100 Hxxx xxxx xxxx (0xc000..0xcfff)
						if ((hw1 & 0xd000) == 0xd000) {
							// BL stays in Thumb - no hint needed (already thumb)
							...
							if (S) {
								offset |= (st32)0xfe000000; // sign extend
							}
							ut64 target = ((addr + i) + 4 + offset) & ~3ULL; // align to 4
							r_anal_hint_set_bits (core->anal, target, 32);
							hints_added++;
						}
					}
				}
				...
				if ((i & 0xff) == 0) {
					RAnalHint *h = r_anal_hint_get (core->anal, addr + i);
					if (h) {
						if (h->bits) {
							cur_bits = h->bits;
						}
	...
	free (buf);
	if (hints_added > 0) {
		R_LOG_INFO ("ARM thumb scan: %d mode switch hints added", hints_added);
	}
}
