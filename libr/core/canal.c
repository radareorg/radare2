	...
	if (refs) {
		...
		R_VEC_FOREACH (refs, ref) {
			...
			if (rt == R_ANAL_REF_TYPE_DATA && check_string_at (core, ref->addr)) {
				...
				r_anal_xrefs_set (core->anal, NULL, ref->at, ref->addr, R_ANAL_REF_TYPE_STRN | R_ANAL_REF_TYPE_READ);
			}
		}
		RVecAnalRef_free (refs);
	}
		...
		case R_ANAL_OP_TYPE_LEA:
			...
			if (cur && arch == R2_ARCH_ARM64) {
				if (CHECKREF (ESIL->cur)) {
					...
					r_anal_xrefs_set (core->anal, NULL, cur, ESIL->cur, type);
				}
			} else if (archIsX86) {
				...
				if ((target && dst == ntarget) || !target) {
					if (CHECKREF (dst)) {
						if (dst && r_io_is_valid_offset (core->io, dst, !core->anal->opt.noncode)) {
							r_anal_xrefs_set (core->anal, NULL, cur, dst, R_ANAL_REF_TYPE_STRN | R_ANAL_REF_TYPE_READ);
						} else {
							r_anal_xrefs_set (core->anal, NULL, cur, ESIL->cur, R_ANAL_REF_TYPE_STRN | R_ANAL_REF_TYPE_READ);
						}
					}
				}
				...
				if (CHECKREF (ESIL->cur)) {
					if (op.ptr && r_io_is_valid_offset (core->io, op.ptr, !core->anal->opt.noncode)) {
						r_anal_xrefs_set (core->anal, NULL, cur, op.ptr, R_ANAL_REF_TYPE_STRN | R_ANAL_REF_TYPE_READ);
					} else {
						r_anal_xrefs_set (core->anal, NULL, cur, ESIL->cur, R_ANAL_REF_TYPE_STRN | R_ANAL_REF_TYPE_READ);
					}
				}
			...
			if (archIsArm) {
				...
				if ((target && dst == ntarget) || !target) {
					if (CHECKREF (dst)) {
						...
						r_anal_xrefs_set (core->anal, NULL, cur, dst, ref_type);
					}
				}
				if (cfg_anal_strings) {
				...
				if ((target && dst == ntarget) || !target) {
					if (dst > 0xffff && opsrc1 && (dst & 0xffff) == (opsrc1->imm & 0xffff) && myvalid (core, dst)) {
						...
						if (CHECKREF (dst) || CHECKREF (cur)) {
							r_anal_xrefs_set (core->anal, NULL, cur, dst, R_ANAL_REF_TYPE_DATA);
							if (cfg_anal_strings) {
								add_string_ref (core, op.addr, dst);
							}
		...
		case R_ANAL_OP_TYPE_LOAD:
			{
				...
				if (dst != UT64_MAX && CHECKREF (dst)) {
					if (myvalid (core, dst)) {
						r_anal_xrefs_set (core->anal, NULL, cur, dst, R_ANAL_REF_TYPE_DATA | R_ANAL_REF_TYPE_READ);
						if (cfg_anal_strings) {
							add_string_ref (core, op.addr, dst);
						}
					}
		...
		case R_ANAL_OP_TYPE_JMP:
			{
				...
				if (CHECKREF (dst)) {
					if (myvalid (core, dst)) {
						r_anal_xrefs_set (core->anal, NULL, cur, dst, R_ANAL_REF_TYPE_CODE | R_ANAL_REF_TYPE_EXEC);
					}
				}
			}
			break;
		case R_ANAL_OP_TYPE_CALL:
			{
				...
				if (CHECKREF (dst) || (target && dst == ntarget)) {
					if (myvalid (core, dst)) {
						r_anal_xrefs_set (core->anal, NULL, cur, dst, R_ANAL_REF_TYPE_CALL | R_ANAL_REF_TYPE_EXEC);
					}
		...
		case R_ANAL_OP_TYPE_MJMP:
			{
				...
				if (CHECKREF (dst)) {
					if (myvalid (core, dst)) {
						...
						r_anal_xrefs_set (core->anal, NULL, cur, dst, ref | R_ANAL_REF_TYPE_EXEC);
						if (!xrefs_only) {
