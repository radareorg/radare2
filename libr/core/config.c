...
R_API int r_core_config_init(RCore *core) {
    ...
    SETI("asm.nbytes", 8, "Number of bytes for each opcode at disassembly");
    SETICB("asm.bits", 32, &cb_asmbits, "Word size in bits at assembler");
    SETICB("asm.lineswidth", 10, &cb_asmlineswidth, "Number of columns for program flow arrows");
    ...
    SETCB("dbg.trace", "true", &cb_trace, "Enable debugger trace (see asm.trace)");
