...
static void print_newline(PDCState *state, ut64 addr, int indent) {
	...
	if (state->pj) {
		...
		if (state->show_asm) {
			int asm_pad;
			char *asm_str = disat (state->core, addr, &asm_pad);
			r_strbuf_appendf (state->out, "0x%08" PFMT64x " | %s", addr, asm_str);
			r_strbuf_pad (state->out, ' ', asm_pad);
...
static void print_goto(PDCState *state, RAnalBlock *bb, ut64 dst_addr, ut64 curr_addr, int indent) {
	...
	if (!sdb_exists (state->goto_cache, src_dst_key) &&
		...
		if (dst_addr != bb->addr) {
			...
			if (state->show_asm) {
				print_str (state, " 0x%08" PFMT64x " | ", bb->addr);
				if (state->pj) {
					r_strbuf_pad (state->codestr, ' ', 30);
				} else {
					r_strbuf_pad (state->out, ' ', 30);
				}
				print_str (state, " | ");
			}
...
R_API int r_core_pseudo_code(RCore *core, const char *input) {
	...
	char *fs = r_core_cmd_strf (core, "afs@0x%08" PFMT64x, state.fcn->addr);
	{
		...
		if (state.show_addr || state.show_asm) {
			PRINTF (" 0x%08" PFMT64x " | ", state.fcn->addr);
			if (state.pj) {
				r_strbuf_pad (state.codestr, ' ', 30);
			} else {
				r_strbuf_pad (state.out, ' ', 30);
		...
		if (codelen > 0) {
			...
			if (bb->jump == UT64_MAX) {
				...
				if (state.show_asm) {
					...
					if (state.pj) {
						r_strbuf_pad (state.codestr, ' ', 30 - 3);
					} else {
						r_strbuf_pad (state.out, ' ', 30 - 3);
					}
					PRINTF (" | ");
				}
				if (r0) {
