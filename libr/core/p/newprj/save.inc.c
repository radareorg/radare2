...
static bool r_core_newprj_save(RCore *core, const char *file) {
	...
	/* Serialize source metadata, ownership, and materialized projections from
	 * one transaction generation.  The writer accessors lock recursively. */
	...
	r_th_lock_leave (core->flags->lock);
