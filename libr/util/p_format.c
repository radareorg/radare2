...
static void r_print_format_byte(const RPrint* p, int endian, int mode,
	...
	if (MUSTSET) {
		...
		if (!SEEVALUE) p->printf ("0x%08"PFMT64x" = ", seeki+((elem>=0)?elem:0));
		if (size==-1)
...
static void r_print_format_char(const RPrint* p, int endian, int mode,
	...
	if (MUSTSET) {
		...
		if (!SEEVALUE) p->printf ("0x%08"PFMT64x" = ", seeki+((elem>=0)?elem*2:0));
		if (size==-1)
...
static int r_print_format_string(const RPrint* p, ut64 seeki, ut64 addr64, ut64 addr, int is64, int mode) {
	...
	if (MUSTSEEJSON) {
		...
		if (!SEEVALUE) p->printf ("0x%08"PFMT64x" -> 0x%08"PFMT64x" ", seeki, addr);
		p->printf ("%s", buffer);
	}
	return 0;
}
...
static void r_print_format_time(const RPrint* p, int endian, int mode,
	...
	if (MUSTSET) {
		...
		*(timestr+24) = '\0';
		if (!SEEVALUE) p->printf ("0x%08"PFMT64x" = ", seeki+((elem>=0)?elem*4:0));
		if (size==-1) {
			p->printf ("%s", timestr);
		} else {
			if (!SEEVALUE) p->printf ("[ ");
