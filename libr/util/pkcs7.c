...
static bool r_pkcs7_parse_signeddata (RPKCS7SignedData *sd, RASN1Object *object) {
	ut32 shift = 3;
	if (!sd || !object || object->list.length < 3) {
		return false;
	}
...
R_API RCMS *r_pkcs7_parse_cms (const ut8 *buffer, ut32 length) {
	...
	ut32 pos = object->list.length - 1;
	if (!object || object->list.length < 2 || !object->list.objects ||
		!object->list.objects[0] || !object->list.objects[pos] ||
		object->list.objects[pos]->list.length < 1) {
	...
	if (object->list.objects[pos]) {
		r_pkcs7_parse_signeddata (&container->signedData, object->list.objects[pos]->list.objects[0]);
	}
	r_asn1_free_object (object);
	return container;
}
