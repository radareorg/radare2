...
struct r_flag_store_shadow_t {
	const RFlag *source_flags;
	ut32 lastid;
	HtPP *ht_name;
	HtUP *ht_meta;
	RSkipList *by_addr;
...
static void set_name(RFlagItem *item, char *pooled_name) {
	...
	const bool old_name_pooled = item->name_pooled;
...
static HtPP *flag_ht_name_new_internal(bool owns_items) {
	HtPPOptions opt = {
		...
		.calcsizeV = NULL,
		.freefn = owns_items? ht_free_flag: NULL,
...
static HtPP *flag_ht_name_new(void) {
	return flag_ht_name_new_internal (true);
}
...
static RFlagsAtOffset *flag_shadow_flags_at_addr(RFlagStoreShadow *shadow, ut64 addr) {
	RFlagsAtOffset key = { .addr = addr };
	return r_skiplist_get_leq (shadow->by_addr, &key);
}

static bool flag_shadow_addr_add(RFlagStoreShadow *shadow, RFlagItem *item) {
	RFlagsAtOffset *flags = flag_shadow_flags_at_addr (shadow, item->addr);
	if (flags && flags->addr == item->addr) {
		return r_list_append (flags->flags, item) != NULL;
	}
	flags = R_NEW (RFlagsAtOffset);
	flags->addr = item->addr;
	flags->flags = r_list_new ();
	if (!flags->flags || !r_list_append (flags->flags, item)
		|| !r_skiplist_insert (shadow->by_addr, flags)) {
		flag_skiplist_free (flags);
		return false;
	}
	return true;
}

static void flag_shadow_addr_del(RFlagStoreShadow *shadow, RFlagItem *item) {
	RFlagsAtOffset *flags = flag_shadow_flags_at_addr (shadow, item->addr);
	if (!flags || flags->addr != item->addr) {
		return;
	}
	r_list_delete_data (flags->flags, item);
	if (r_list_empty (flags->flags)) {
		r_skiplist_delete (shadow->by_addr, flags);
	}
}

static bool flag_shadow_addr_replace(RFlagStoreShadow *shadow, RFlagItem *item, RFlagItem *replacement) {
	RFlagsAtOffset *flags = flag_shadow_flags_at_addr (shadow, item->addr);
	if (!flags || flags->addr != item->addr) {
		return false;
	}
	RListIter *iter = NULL;
	RFlagItem *current;
	r_list_foreach (flags->flags, iter, current) {
		if (current == item) {
			break;
		}
	}
	...
	if (item->addr == replacement->addr) {
		iter->data = replacement;
		return true;
	}
	if (!flag_shadow_addr_add (shadow, replacement)) {
		return false;
	}
	r_list_delete (flags->flags, iter);
	if (r_list_empty (flags->flags)) {
		r_skiplist_delete (shadow->by_addr, flags);
	}
	return true;
...
static bool flag_shadow_addr_move(RFlagStoreShadow *shadow, RFlagItem *item, ut64 addr) {
	if (item->addr == addr) {
		return true;
	}
	const ut64 old_addr = item->addr;
	item->addr = addr;
	if (!flag_shadow_addr_add (shadow, item)) {
		item->addr = old_addr;
		return false;
	}
	item->addr = old_addr;
	flag_shadow_addr_del (shadow, item);
	item->addr = addr;
	return true;
}

static void flag_shadow_addr_rebind(RFlagStoreShadow *shadow, RFlagItem *staged, RFlagItem *original) {
	RFlagsAtOffset *flags = flag_shadow_flags_at_addr (shadow, staged->addr);
	R_RETURN_IF_FAIL (flags && flags->addr == staged->addr);
	RListIter *iter;
	RFlagItem *item;
	r_list_foreach (flags->flags, iter, item) {
		if (item == staged) {
			iter->data = original;
			return;
		}
...
static RFlagItem *flag_shadow_item_clone(const RFlagItem *item) {
	RFlagItem *copy = R_NEW0 (RFlagItem);
	copy->id = item->id;
	copy->name_pooled = item->name_pooled;
	copy->name = item->name_pooled? item->name: (item->name? strdup (item->name): NULL);
	if (item->name && !copy->name) {
		goto fail;
	}
	if (item->realname == item->name) {
		copy->realname = copy->name;
		copy->realname_pooled = copy->name_pooled;
	} else {
		copy->realname_pooled = item->realname_pooled;
		copy->realname = item->realname_pooled? item->realname
			: (item->realname? strdup (item->realname): NULL);
		if (item->realname && !copy->realname) {
			goto fail;
		}
	}
	copy->rawname_pooled = item->rawname_pooled;
	copy->rawname = item->rawname_pooled? item->rawname
		: (item->rawname? strdup (item->rawname): NULL);
	if (item->rawname && !copy->rawname) {
		goto fail;
	}
	copy->addr = item->addr;
	copy->size = item->size;
	copy->space = item->space;
	copy->demangled = item->demangled;
	return copy;
fail:
	r_flag_item_free (copy);
	return NULL;
}

static void flag_shadow_update_free(void *data) {
	RFlagShadowUpdate *update = data;
	if (update) {
		r_flag_item_free (update->staged);
		free (update);
	}
}

static RListIter *flag_shadow_item_find(RList *items, const RFlagItem *needle) {
	RListIter *iter;
	RFlagItem *item;
...
static RFlagShadowUpdate *flag_shadow_update_find(RFlagStoreShadow *shadow, const RFlagItem *staged) {
	RListIter *iter;
	RFlagShadowUpdate *update;
	r_list_foreach (shadow->updates, iter, update) {
		if (update->staged == staged) {
			return update;
		}
	}
	return NULL;
...
typedef struct {
	RFlagStoreShadow *shadow;
	bool success;
} FlagShadowCloneCtx;

static bool flag_shadow_name_clone_cb(void *user, const void *key, const void *value) {
	FlagShadowCloneCtx *ctx = user;
	if (!ht_pp_insert (ctx->shadow->ht_name, (void *)key, (void *)value)) {
		ctx->success = false;
		return false;
	}
	return true;
...
static bool flag_shadow_meta_clone_cb(void *user, const ut64 key, const void *value) {
	FlagShadowCloneCtx *ctx = user;
	if (!ht_up_insert (ctx->shadow->ht_meta, key, (void *)value)) {
		ctx->success = false;
		return false;
	}
	return true;
...
static bool flag_shadow_addr_clone(RFlagStoreShadow *shadow, const RFlag *flags) {
	RSkipListNode *skip_iter;
	RFlagsAtOffset *source;
	r_skiplist_foreach (flags->by_addr, skip_iter, source) {
		RFlagsAtOffset *copy = R_NEW (RFlagsAtOffset);
		copy->addr = source->addr;
		copy->flags = r_list_new ();
		if (!copy->flags) {
			free (copy);
			return false;
		}
		RListIter *iter;
		RFlagItem *item;
		r_list_foreach (source->flags, iter, item) {
			if (!r_list_append (copy->flags, item)) {
				flag_skiplist_free (copy);
				return false;
			}
		}
		if (!r_skiplist_insert (shadow->by_addr, copy)) {
			flag_skiplist_free (copy);
			return false;
		}
	}
...
static HtPPKv *flag_shadow_find_exact(RFlagStoreShadow *shadow, const char *name) {
	bool found = false;
	HtPPKv *kv = ht_pp_find_kv (shadow->ht_name, (void *)name, &found);
	return found? kv: NULL;
}

R_API const RFlagItem *r_flag_store_shadow_get_exact(RFlagStoreShadow *shadow, const char *name) {
	R_RETURN_VAL_IF_FAIL (shadow && name, NULL);
	HtPPKv *kv = flag_shadow_find_exact (shadow, name);
	return kv? kv->value: NULL;
}

R_API RFlagStoreShadow *r_flag_store_shadow_prepare(const RFlag *flags) {
	R_RETURN_VAL_IF_FAIL (flags, NULL);
	RFlagStoreShadow *shadow = R_NEW0 (RFlagStoreShadow);
	shadow->source_flags = flags;
	shadow->ht_name = flag_ht_name_new_internal (false);
	shadow->ht_meta = ht_up_new0 ();
	shadow->by_addr = r_skiplist_new (flag_skiplist_free, flag_skiplist_cmp);
	shadow->created_items = r_list_new ();
	shadow->retired_items = r_list_new ();
	shadow->retired_meta = r_list_new ();
	shadow->updates = r_list_newf (flag_shadow_update_free);
	if (!shadow->ht_name || !shadow->ht_meta || !shadow->by_addr
		|| !shadow->created_items || !shadow->retired_items || !shadow->retired_meta
		|| !shadow->updates) {
		r_flag_store_shadow_free (shadow);
		return NULL;
	}
	RFlag *mutable_flags R_UNUSED = (RFlag *)flags;
	R_CRITICAL_ENTER (mutable_flags);
	shadow->lastid = flags->lastid;
	FlagShadowCloneCtx ctx = {
		.shadow = shadow,
		.success = true
	};
	ht_pp_foreach (flags->ht_name, flag_shadow_name_clone_cb, &ctx);
	if (ctx.success) {
		ht_up_foreach (flags->ht_meta, flag_shadow_meta_clone_cb, &ctx);
	}
	if (ctx.success) {
		ctx.success = flag_shadow_addr_clone (shadow, flags);
	}
	R_CRITICAL_LEAVE (mutable_flags);
	if (!ctx.success) {
		r_flag_store_shadow_free (shadow);
		return NULL;
	}
...
R_API bool r_flag_store_shadow_set_exact(RFlagStoreShadow *shadow, const char *name, ut64 addr, ut32 size, const RSpace *space) {
	R_RETURN_VAL_IF_FAIL (shadow && !shadow->committed && name && *name, false);
	if (!r_name_check (name)) {
		return false;
	}
	HtPPKv *kv = flag_shadow_find_exact (shadow, name);
	if (kv) {
		RFlagItem *item = kv->value;
		if (item->addr == addr && item->size == size && item->space == space) {
			return true;
		}
		RFlagShadowUpdate *update = flag_shadow_update_find (shadow, item);
		if (update || flag_shadow_item_find (shadow->created_items, item)) {
			if (!flag_shadow_addr_move (shadow, item, addr)) {
				return false;
			}
			item->size = size;
			item->space = (RSpace *)space;
			return true;
		}
		RFlagItem *staged = flag_shadow_item_clone (item);
		if (!staged) {
			return false;
		}
		update = R_NEW0 (RFlagShadowUpdate);
		update->original = item;
		update->staged = staged;
		RListIter *update_iter = r_list_append (shadow->updates, update);
		if (!update_iter) {
			flag_shadow_update_free (update);
			return false;
		}
		staged->addr = addr;
		staged->size = size;
		staged->space = (RSpace *)space;
		if (!flag_shadow_addr_replace (shadow, item, staged)) {
			r_list_delete (shadow->updates, update_iter);
			return false;
		}
		kv->key = staged->name;
		kv->key_len = (ut32)strlen (staged->name);
		kv->value = staged;
		return true;
	}
	if (shadow->lastid == UT32_MAX) {
		return false;
	}
	...
	item->id = shadow->lastid + 1;
	item->name = owned_name;
	item->realname = owned_name;
	item->addr = addr;
	item->size = size;
	item->space = (RSpace *)space;
	RListIter *created = r_list_append (shadow->created_items, item);
	if (!created) {
		r_flag_item_free (item);
		return false;
	}
	if (!flag_shadow_addr_add (shadow, item)
		|| !ht_pp_insert (shadow->ht_name, item->name, item)) {
		flag_shadow_addr_del (shadow, item);
		r_list_delete (shadow->created_items, created);
		r_flag_item_free (item);
		return false;
	}
	shadow->lastid++;
...
R_API bool r_flag_store_shadow_del_exact(RFlagStoreShadow *shadow, const char *name) {
	R_RETURN_VAL_IF_FAIL (shadow && !shadow->committed && name, false);
	HtPPKv *kv = flag_shadow_find_exact (shadow, name);
	if (!kv) {
		return true;
	}
	RFlagItem *item = kv->value;
	RListIter *created = flag_shadow_item_find (shadow->created_items, item);
	if (created) {
		flag_shadow_addr_del (shadow, item);
		if (!ht_pp_delete (shadow->ht_name, name)) {
			return false;
		}
		r_list_delete (shadow->created_items, created);
		r_flag_item_free (item);
	...
	RFlagShadowUpdate *update = flag_shadow_update_find (shadow, item);
	RFlagItem *original = update? update->original: item;
	RListIter *retired = r_list_append (shadow->retired_items, original);
	if (!retired) {
		return false;
	}
	RFlagItemMeta *meta = ht_up_find (shadow->ht_meta, original->id, NULL);
	if (meta && !r_list_append (shadow->retired_meta, meta)) {
		r_list_delete (shadow->retired_items, retired);
		return false;
	}
	if (meta) {
		ht_up_delete (shadow->ht_meta, original->id);
	}
	flag_shadow_addr_del (shadow, item);
	if (!ht_pp_delete (shadow->ht_name, name)) {
		return false;
...
static void flag_shadow_updates_commit(RFlagStoreShadow *shadow) {
	RListIter *iter;
	RFlagShadowUpdate *update;
	r_list_foreach (shadow->updates, iter, update) {
		if (update->deleted) {
		...
		RFlagItem *original = update->original;
		RFlagItem *staged = update->staged;
		HtPPKv *kv = flag_shadow_find_exact (shadow, staged->name);
		R_WARN_IF_FAIL (kv && kv->value == staged);
		flag_shadow_addr_rebind (shadow, staged, original);
		original->addr = staged->addr;
		original->size = staged->size;
		original->space = staged->space;
		kv->key = original->name;
		kv->key_len = (ut32)strlen (original->name);
		kv->value = original;
	}
}

R_API void r_flag_store_shadow_swap(RFlag *flags, RFlagStoreShadow *shadow) {
	R_RETURN_IF_FAIL (flags && shadow && shadow->source_flags == flags && !shadow->committed);
	R_CRITICAL_ENTER (flags);
	flag_shadow_updates_commit (shadow);
	shadow->ht_name->opt.freefn = ht_free_flag;
	shadow->ht_meta->opt.freefn = ht_free_meta;
	flags->ht_name->opt.freefn = NULL;
	flags->ht_meta->opt.freefn = NULL;
	ut32 lastid = flags->lastid;
	flags->lastid = shadow->lastid;
	shadow->lastid = lastid;
	HtPP *ht_name = flags->ht_name;
	flags->ht_name = shadow->ht_name;
	shadow->ht_name = ht_name;
	HtUP *ht_meta = flags->ht_meta;
	flags->ht_meta = shadow->ht_meta;
	shadow->ht_meta = ht_meta;
	RSkipList *by_addr = flags->by_addr;
	flags->by_addr = shadow->by_addr;
	shadow->by_addr = by_addr;
	shadow->committed = true;
	R_DIRTY_SET (flags);
	R_CRITICAL_LEAVE (flags);
}

R_API void r_flag_store_shadow_free(RFlagStoreShadow *shadow) {
	if (shadow) {
		r_skiplist_free (shadow->by_addr);
		ht_pp_free (shadow->ht_name);
		ht_up_free (shadow->ht_meta);
		RList *items = shadow->committed? shadow->retired_items: shadow->created_items;
		RListIter *iter;
		RFlagItem *item;
		r_list_foreach (items, iter, item) {
			r_flag_item_free (item);
		...
		if (shadow->committed) {
			RFlagItemMeta *meta;
			r_list_foreach (shadow->retired_meta, iter, meta) {
				HtUPKv kv = { .value = meta };
				ht_free_meta (&kv);
			}
		}
		r_list_free (shadow->created_items);
		r_list_free (shadow->retired_items);
		r_list_free (shadow->retired_meta);
		r_list_free (shadow->updates);
	}
	free (shadow);
}
