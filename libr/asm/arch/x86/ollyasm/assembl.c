...
static void Parseasmoperand(t_asmoperand *op) {
	...
	// Segment register or address.
		...
		(scan == SCAN_SYMB && idata == '[')) { // Segment register or address
		bracket = 0;
		if (scan == SCAN_SEG) {
			j = idata;
			Scanasm (0);
...
int Assemble (char *cmd, ulong ip, t_asmmodel *model, int attempt,
	...
	if (rep == SCAN_REP)
		snprintf (name, sizeof (name) - 1, "REP %s", sdata);
	else if (rep == SCAN_REPE)
		snprintf (name, sizeof (name) - 1, "REPE %s", sdata);
	else if (rep == SCAN_REPNE)
		snprintf (name, sizeof (name) - 1, "REPNE %s", sdata);
...
retrylongjump:
	...
	for (pd = cmddata; pd->mask != 0; pd++) {
		...
		else bytesize = 0; // Word/dword size only
	...
	if (match != 0) { // Command not found
		...
		else strcpy (errtext, "Erroneous command");
		goto error;
	};
	...
	for (j = 0; j < 3; j++) { // Up to 3 operands
		...
		case SCM: // Segment register in command byte
			if (op->index == SEG_FS || op->index == SEG_GS) {
				...
				else tcode[i] = (char)((op->index << 3) | 0x81);
				tmask[i] = 0xFF;
			} else if (op->index < 6) {
