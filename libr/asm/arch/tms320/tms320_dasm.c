...
insn_head_t * lookup_insn_head(tms320_dasm_t * dasm) {
	...
	while (e_list && (e_list[0] && e_list[1])) {
		if ((dasm->opcode & e_list[0]) == e_list[1]) {
			...
			dasm->head = ht_pp_find (dasm->map, key, NULL);
			break;
		}
		e_list += 2;
	}
...
static void tsm320_free_kv(HtPPKv *kv) {
	free (kv->key);
}
...
int tms320_dasm_init(tms320_dasm_t * dasm) {
	...
	dasm->map = ht_pp_new (NULL, tsm320_free_kv, NULL);
	if (!dasm->map) {
		return 0;
	}
	for (i = 0; i < ARRAY_SIZE(c55x_list); i++) {
		const char *key = sdb_fmt ("%c", c55x_list[i].byte);
		ht_pp_insert (dasm->map, key, &c55x_list[i]);
	}
