	...
	switch(reg_type) {
		...
		case 100:
			if(strncasecmp(reg_arg, "d(ALLx", 6)) {
				fprintf(stderr, "invalid register! %s\n", reg_arg); *err_code = -1; return NULL;
			}
		...
		case 82:
			if(!strncasecmp(reg_arg, "RA", 2) ||
			   !strncasecmp(reg_arg, "RL", 2)
			) {
				res = get_reg_name_1(ins_bits);

			} else if(!strncasecmp(reg_arg, "RLP", 3) ||
				  !strncasecmp(reg_arg, "RxP", 3)
			) {
				res = get_reg_name_1(ins_bits + 1);
...
static st8 *do_decode(ut32 ins_off, ut32 ins_pos, ut32 two_ins, ut32 *next_ins_pos, st32 *ins_hash_code, int *err_code)
{
	...
	*next_ins_pos = 0;
	...
	if(hash_aux == 0x1E1 || hash_aux == 0x1E2) {
		...
		ins_aux = decode_ins(hash_aux, ins_pos, ins_off, &ins_len_dec, &reg_len_dec, &ret_ins_bits, magic_value, two_ins, err_code);
		if(*err_code < 0)
			return NULL;
