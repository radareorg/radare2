...
// AVR assembler realization starts
// by Alexander Bolshev aka @dark_k3y, LGPL -- 2015.
...
static int parse_specialreg(char *reg){
	int found = -1;
	int len = strlen(reg);
	int i;
	...
	if(len > 0){
		...
		// radare tolower instruction in rasm, so we use 'y' instead of 'Y' etc.
		if(found == -1 && reg[1] == '+'){
			if(reg[1] == 'y' && len > 2)
				found = OPERAND_YPQ;
...
// this function searches from instruction in instructionSet table (see avr_disasm.h for more info)
		...
		if(strncmp(instr[0], instructionSet[i].mnemonic, MAX_TOKEN_SIZE) == 0){
			...
			if(instructionSet[i].numOperands == args){ 
				...
				if(args == 2){
					...
					if(instructionSet[i].opcodeMask >= 0x9004 && instructionSet[i].opcodeMask <= 0x9007){
						if(instructionSet[i].operandTypes[1] == parse_specialreg(instr[2]))
							return i;
					// handling ld & ldd instruction with 2 args
					}else if(instructionSet[i].mnemonic[0] == 'l' && instructionSet[i].mnemonic[1] == 'd'
							&& (instructionSet[i].mnemonic[2] == 'd' || instructionSet[i].mnemonic[2] == '\0')){
						if(instructionSet[i].operandTypes[1] == parse_specialreg(instr[2]))
							return i;
					// handling lds command, distinguishing long from 16-bit version
					}else if(instructionSet[i].mnemonic[0] == 'l' && instructionSet[i].mnemonic[1] == 'd'
							&& instructionSet[i].mnemonic[2] == 's' && instructionSet[i].operandTypes[1] == OPERAND_LONG_ABSOLUTE_ADDRESS){
					...
					}else if(instructionSet[i].mnemonic[0] == 's' && instructionSet[i].mnemonic[1] == 't' 
							&& (instructionSet[i].mnemonic[2] == 'd' || instructionSet[i].mnemonic[2] == '\0')){
						if(instructionSet[i].operandTypes[0] == parse_specialreg(instr[1]))
							return i;
					// handling sts long command 
					}else if(instructionSet[i].mnemonic[0] == 's' && instructionSet[i].mnemonic[1] == 't'
							&& instructionSet[i].mnemonic[2] == 's' && instructionSet[i].operandTypes[0] == OPERAND_LONG_ABSOLUTE_ADDRESS){
...
static int parse_registerpair(char *operand){
	int res = -1;
	char *first, *second;
	int fnum, snum;
...
static int assemble_operand(RAsm *a, char *operand, int type, uint32_t *res){
	int ret = -1;
    	...
    	if(instr_idx >= 0){
    		...
    		//		ORing with the last 16 bits(/2 for jmp/call)
    			...
    			if(assemble_operand(a, tokens[1], instructionSet[instr_idx].operandTypes[0], &op1) >= 0){    				
	    			if(instructionSet[instr_idx].operandTypes[0] == OPERAND_LONG_ABSOLUTE_ADDRESS){ // jmp and call has 4-byte opcode
	    				op1 = op1/2;
