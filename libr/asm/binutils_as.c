...
bool binutils_encode(RArchSession *a, RAnalOp *op, const char *buf, const char *as, const char *env, const char *header, const char *cmd_opt) {
	...
	char *cmd = r_str_newf ("%s %s %s -o %s", as, cmd_opt, ipath, opath);
	if (!r_sys_cmd (cmd)) {
		int len = 0;
		const ut8 *begin, *end;
		close (ofd);
