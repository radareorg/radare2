...
R_API int r_run_config_env(RRunProfile *p) {
	...
	if (p->_docore || p->_maxfd || p->_maxproc || p->_maxstack)
		R_LOG_WARN ("Warning: setrlimits not supported for this platform");
#endif
	if (p->_connect) {
	...
	if (p->_r2preload) {
		if (p->_preload) {
			R_LOG_WARN ("Warning: Only one library can be opened at a time");
		}
#ifdef __WINDOWS__
...
#define _POSIX_SPAWN_DISABLE_ASLR 0x0100
	...
	//		eprintf ("PID: Cannot determine pid with 'system' directive. Use 'program'.\n");
			...
			if (p->_pidfile) {
				R_LOG_WARN ("Warning: pidfile doesnt work with 'system'.");
			}
			rc = r_sys_cmd (p->_system);
		}
		time_end (p->_time, time_begin);
