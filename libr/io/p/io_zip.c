...
static inline const char *ziperrmsg(int zip_errorp) {
	switch (zip_errorp) {
	case ZIP_ER_INVAL: return "Invalid file name (NULL)";
	case ZIP_ER_OPEN: return "File could not be opened";
	case ZIP_ER_NOENT: return "File does not exist";
	case ZIP_ER_READ: return "Read error occurred";
	case ZIP_ER_NOZIP: return "File is not a valid ZIP archive";
	case ZIP_ER_INCONS: return "ZIP file had some inconsistencies";
	default: return "Something bad happened, get your debug on";
	}
}
...
static struct zip *r_io_zip_open_archive(const char *archivename, ut32 perm, int mode, int rw) {
	...
	if (!za) {
		R_LOG_ERROR ("%s", ziperrmsg (zip_errorp));
	}
	return za;
}
...
static RList *r_io_zip_get_files(char *archivename, ut32 perm, int mode, int rw) {
	...
	for (i = 0; i < num_entries; i++) {
		...
		r_list_append (files, strdup (sb.name));
	}
	zip_close (za);
	return files;
}
...
static RIODesc *r_io_zip_open(RIO *io, const char *file, int rw, int mode) {
	...
	char *zip_filename_alloc = NULL;
	char *filename_in_zipfile = NULL;

	char *pikaboo = strstr (zip_uri, "://");
	if (pikaboo) {
		char *dslash = strstr (pikaboo + 3, "//");
		if (dslash) {
			zip_filename_alloc = strdup (dslash);
			zip_filename = zip_filename_alloc;
		}
		if (!zip_filename) {
			if (r_str_startswith (zip_uri, "ipa://")) {
				zip_filename_alloc = strdup (pikaboo + 3);
				zip_filename = zip_filename_alloc;
						...
						if (bin_name) {
							...
							if (slash) {
								...
								if (!strcmp (name, chkstr)) {
									...
									zip_filename_alloc = r_str_newf ("//%s", chkstr);
									zip_filename = zip_filename_alloc;
									free (chkstr);
									free (bin_name);
									break;
				...
				zip_filename_alloc = strdup (pikaboo + 1);
				zip_filename = zip_filename_alloc;
			}
		} else {
			free (zip_filename_alloc);
	...
	if (zip_filename && zip_filename[1] && zip_filename[2]) {
		...
		*zip_filename++ = 0;
		...
		char *sep = strstr (zip_filename, "//");
		if (sep && sep[2]) {
			*sep++ = 0;
			*sep++ = 0;
			filename_in_zipfile = strdup (sep);
	...
	if (!filename_in_zipfile) {
		...
		if (zfo) {
			if (zfo->entry == -1) {
				if (rw & R_PERM_W) {
					...
					R_LOG_ERROR ("File '%s' not found in archive", filename_in_zipfile);
		...
		if (!res) {
			R_LOG_ERROR ("Failed to open the archive %s and file %s", zip_filename, filename_in_zipfile);
		}
	}
...
static ut64 r_io_zip_lseek(RIO *io, RIODesc *fd, ut64 offset, int whence) {
	...
	RIOZipFileObj *zfo = fd->data;
	ut64 seek_val = r_buf_tell (zfo->b);
	switch (whence) {
	case R_IO_SEEK_SET:
