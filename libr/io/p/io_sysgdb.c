...
static char *runcmd(RIOSysGdb *state, const char *cmd) {
	char buf[4096] = { 0 };
	if (cmd) {
		if (state->usefirst) {
...
static int __write(RIO *io, RIODesc *fd, const ut8 *buf, int count) {
	...
	RIOSysGdb *state = (RIOSysGdb *)fd->data;
	int i;
	for (i = 0; i < count; i++) {
		ut64 addr = io->off + i;
...
static RIODesc *__open(RIO *io, const char *pathname, int rw, int mode) {
	if (__plugin_open (io, pathname, 0)) {
		RIOSysGdb *state = R_NEW0 (RIOSysGdb);
		if (!state) {
			return NULL;
		}
		...
		if (reply) {
			...
			if (state->use_lldb) {
				if (state->use_connect) {
					...
					runcmd (state, cmd);
					free (cmd);
					runcmd (state, "process launch --stop-at-entry\n");
				}
			}
			return r_io_desc_new (io, &r_io_plugin_sysgdb, pathname, rw, mode, state);
		}
...
static int sysgdb_getpid(RIOSysGdb *state) {
	char *res = runcmd (state, state->use_lldb? "process status": "info proc");
	char *sp = strchr (res, ' ');
...
#undef IFREG
		...
		char *res = runcmd (state, "info proc mappings");
