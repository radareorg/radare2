...
static int r_io_mmap_write(RIO *io, RIODesc *fd, const ut8 *buf, int count) {
	...
	if (!r_io_mmap_refresh_buf (mmo) ) {
		R_LOGFI ("io_mmap: failed to refresh the mmap backed buffer.\n");
...
static bool r_io_mmap_truncate(RIOMMapFileObj *mmo, ut64 size) {
	...
	if (res && !r_io_mmap_refresh_buf (mmo)) {
		R_LOGFI ("r_io_mmap_truncate: Error trying to refresh the mmap'ed file.");
		res = false;
	} else if (res) {
		R_LOGFI ("r_io_mmap_truncate: Error trying to resize the file.");
	}
	return res;
}
