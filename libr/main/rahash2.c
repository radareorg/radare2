...
static void do_hash_seed(const char *seed) {
	...
	if (!strncmp (sptr, "s:", 2)) {
		...
		if (s.len < 1) {
			...
			s.len = strlen (sptr);
			R_LOG_WARN ("Warning: This is not an hexpair, assuming a string, prefix it with 's:' to skip this message.");
		}
	}
}
...
static int do_hash(const char *file, const char *algo, RIO *io, int bsize, int rad, int ule, const ut8 *compare) {
	...
	if (incremental) {
		...
		if (s.buf) {
			R_LOG_WARN ("Warning: Seed ignored on per-block hashing.");
		}
		for (i = 1; i < R_HASH_ALL; i <<= 1) {
			ut64 f, t, ofrom, oto;
			if (algobit & i) {
			...
			if (r_crypto_set_key (cry, s.buf, s.len, 0, direction)) {
				...
				if (result) {
					if (write (1, result, result_size) != result_size) {
						R_LOG_WARN ("Warning: cannot write result");
					}
					free (result);
				}
			} else {
