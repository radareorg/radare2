	...
	if (R_STR_ISNOTEMPTY (mr.pfile) && r_file_is_directory (mr.pfile)) {
		...
		/* load <file>.r2 */
		{
			...
			if (r_file_exists (path)) {
				...
				if (isint && r_kons_yesno (cons, 'n', "Do you want to run the '%s' script? (y/N) ", path)) {
		...
		for (;;) {
			...
			if (ret != -1 && r_cons_is_interactive ()) {
				...
				if (r_core_task_running_tasks_count (&r->tasks) > 0) {
					if (r_kons_yesno (cons, 'y', "There are running background tasks. Do you want to kill them? (Y/n)")) {
				...
				if (mr.debug) {
					if (no_question_debug) {
						...
						if (r_kons_yesno (cons, 'y', "Do you want to quit? (Y/n)")) {
							if (r_config_get_b (r->config, "dbg.exitkills") &&
									r_kons_yesno (cons, 'y', "Do you want to kill the process? (Y/n)")) {
								r_debug_kill (r->dbg, r->dbg->pid, r->dbg->tid, 9); // KILL
							} else {
				...
				if (R_STR_ISNOTEMPTY (prj)) {
					...
					if (no_question_save) {
						...
						if (r_kons_yesno (cons, 'y', "%s", question)) {
							r_core_project_save (r, prj);
						}
						free (question);
					}
				...
				if (r_config_get_b (r->config, "scr.confirmquit")) {
					if (!r_kons_yesno (cons, 'n', "Do you want to quit? (Y/n)")) {
						continue;
					}
				}
			} else {
