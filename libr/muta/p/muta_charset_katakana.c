...
typedef struct { const char *in2; const char *out; } KDMap;
...
static bool check(const char *algo){return !strcmp(algo,"katakana");}
static bool update(RMutaSession*cj,const ut8*b,int l){
  ...
  if(cj->dir==R_CRYPTO_DIR_DECRYPT){
    ...
    for(int i=0;i<l;i++){ r_muta_session_append(cj,&b[i],1);} 
  }
  return true;
}
