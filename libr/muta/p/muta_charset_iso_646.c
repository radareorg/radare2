...
static const char *decode_byte(ut8 b){
    for(const MutaCharsetMap*m=map;m->str;m++){ if(m->byte==b) return m->str; }
    return NULL;
}
static bool encode_utf8(const char*s,ut8*out){
    for(const MutaCharsetMap*m=map;m->str;m++){ if(!strcmp(m->str,s)){ *out=m->byte; return true; } }
    return false;
}
