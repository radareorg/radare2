...
static bool check(const char *algo){ return !strcmp(algo,"jis7"); }
static bool update(RMutaSession*cj,const ut8*b,int l){
    ...
    if(cj->dir==R_CRYPTO_DIR_DECRYPT){
        for(int i=0;i<l;i++){ ut8 ch=b[i]; if(ch<0x20 && ch!='\n' && ch!='\t') ch='.'; r_muta_session_append(cj,&ch,1);} 
    } else {
...
RMutaPlugin r_muta_plugin_charset_jis7={
    .meta={.name="charset_jis7",.license="MIT",.desc="JIS 7-bit Roman (ASCII-like)"},
