...
static bool update(RMutaSession*cj,const ut8*buf,int len){
  ...
  if(cj->dir==R_CRYPTO_DIR_DECRYPT){
    for(int i=0;i<len;i++){ const char*s=dec(buf[i]); r_strbuf_append(sb, s? s: "."); }
    const char*out=r_strbuf_get(sb); r_muta_session_append(cj,(const ut8*)out,(int)strlen(out));
  } else {
    for(int i=0;i<len;i++){ ut8 b; char ch[2]={(char)buf[i],0}; if(!enc(ch,&b)) b='?'; r_muta_session_append(cj,&b,1);} 
  }
