...
#include <r_muta.h>
static bool check(const char *algo){return !strcmp(algo,"arabic_iso");}
static bool update(RMutaSession*cj,const ut8*b,int l){
    if(!cj||!b||l<0) return false;
    if(cj->dir==R_CRYPTO_DIR_DECRYPT){ r_muta_session_append(cj,b,l); }
    else { r_muta_session_append(cj,b,l);} return true;}
static bool end(RMutaSession*cj,const ut8*b,int l){return update(cj,b,l);} 
RMutaPlugin r_muta_plugin_charset_arabic_iso={
    .meta={.name="charset_arabic_iso",.license="MIT",.desc="arabic ISO stub"},
