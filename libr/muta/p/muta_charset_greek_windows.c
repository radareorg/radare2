...
#include <r_muta.h>
typedef struct { const char* u; ut8 b; } Map;
static const Map m[] = {
  {"\n",0x0A},{"\t",0x09},
...
static const char* dec(ut8 b){ for(const Map*mm=m; mm->u; mm++){ if(mm->b==b) return mm->u; } return NULL; }
static bool enc(const char*s, ut8*out){ for(const Map*mm=m; mm->u; mm++){ if(!strcmp(mm->u,s)){ *out=mm->b; return true; } } return false; }
static bool update(RMutaSession*cj,const ut8*buf,int len){ if(!cj||!buf||len<0) return false; RStrBuf*sb=r_strbuf_new("");
  if(cj->dir==R_CRYPTO_DIR_DECRYPT){ for(int i=0;i<len;i++){ const char*s=dec(buf[i]); r_strbuf_append(sb, s? s: "."); } const char*out=r_strbuf_get(sb); r_muta_session_append(cj,(const ut8*)out,(int)strlen(out)); }
  else { for(int i=0;i<len;i++){ ut8 b; char ch[2]={(char)buf[i],0}; if(!enc(ch,&b)) b='?'; r_muta_session_append(cj,&b,1);} }
