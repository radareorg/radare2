...
static bool replay_binding_apply_pty(const RDebugReplayBinding *binding, const ut8 *bytes, ut64 remaining) {
	...
	if (R_STR_ISNOTEMPTY (binding->slave_name)) {
		...
		if (slave_fd >= 0) {
			...
			bool ok = tcflush (slave_fd, TCIFLUSH) == 0 && replay_binding_write_all (binding, bytes, remaining);
	...
	return tcflush (binding->host_fd, TCIFLUSH) == 0 && replay_binding_write_all (binding, bytes, remaining);
#else
	return false;
#endif
}
