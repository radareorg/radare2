		...
		r_list_foreach (dbg->maps, iter, map) {
			if (addr >= map->addr && addr < map->addr_end) {
				...
				if (!(perm & R_BP_PROT_EXEC)) {
					R_LOG_WARN ("Warning: setting bp within mapped memory without exec perm");
				}
				break;
			}
		}
		if (!valid) {
			R_LOG_WARN ("Warning: module's base addr + delta is not a valid address");
			free (module_name);
			return NULL;
		}
	}
...
R_API bool r_debug_goto_cnum(RDebug *dbg, ut32 cnum) {
	if (cnum > dbg->session->maxcnum) {
		R_LOG_ERROR ("Error: out of cnum range");
		return false;
	}
