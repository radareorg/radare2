...
int linux_handle_signals(RDebug *dbg, int tid) {
	siginfo_t siginfo = { 0 };
	//eprintf("ID1 = %d\n", siginfo.si_pid); SHOWS RIGHT PID
	int ret = r_debug_ptrace (dbg, PTRACE_GETSIGINFO, tid, 0, (r_ptrace_data_t)&siginfo);//int ret = r_debug_ptrace (dbg, PTRACE_GETSIGINFO, tid, 0, (r_ptrace_data_t)(size_t)&siginfo);
	//eprintf("ID2 = %d\n", siginfo.si_pid); SHOWS WRONG PID
	if (ret == -1) {
