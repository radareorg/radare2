...
//TODO research more about mach_port_deallocate, how it works and where it
//should be called. We must be careful how we manage this
//since we are messing with kernel land
static void xnu_thread_free (xnu_thread_t *thread) {
	...
	free (thread);
#if 0
	kr = mach_port_deallocate (mach_task_self (), thread->tid);
	if (!kr != KERN_SUCCESS) {
		eprintf ("Fail to deallocate mach_port in r2xnu_thread_free\n");
		return;
	}
#endif
}

static int xnu_get_basicinfo_name_thread(RDebug *dbg,
					xnu_thread_t *thread, thread_t tid) {
#if TARGET_OS_IPHONE
	return R_FALSE;
#else
	...
	kr = thread_info (tid, THREAD_BASIC_INFO,
	...
	if (kr != KERN_SUCCESS) {
        ...
        kr = thread_info (tid, THREAD_IDENTIFIER_INFO,
...
//TODO review for memleak and possible errors
static xnu_thread_t *xnu_get_thread_with_info (RDebug *dbg, thread_t tid){
	...
	thread->tid = tid;
	ret = xnu_get_basicinfo_name_thread (dbg, thread, tid);
	if (ret == R_FALSE) {
		thread->name = strdup ("unknown");
	}
	return thread;
...
static int xnu_update_thread_info(RDebug *dbg, xnu_thread_t *thread) {
	int ret;
	free (thread->name);
	ret = xnu_get_basicinfo_name_thread (dbg, thread, thread->tid);
	if (ret == R_FALSE) {
		thread->name = strdup ("unknown");
	}
	return R_TRUE;
...
//TODO audit this function and review
...
//TODO implement the exception logic. It's everything since
//here we need to handle bad access, breakpoint, syscalls and everything
