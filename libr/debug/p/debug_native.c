...
static int r_debug_native_wait(RDebug *dbg, int pid) {
	...
	if (WIFSTOPPED (status)) {
		...
		if (ret != pid) status = R_DBG_REASON_NEW_PID;
...
static int windows_reg_read (RDebug *dbg, int type, ut8 *buf, int size) {
	...
	CloseHandle(thread);
	if (type == R_REG_TYPE_FPU || type == R_REG_TYPE_MMX ||
		type == R_REG_TYPE_XMM) {
	#if __MINGW64__
		...
		int i;
		if (showfpu) {
			eprintf ("cwd = 0x%08x  ; control   ",
				(ut32)ctx.FloatSave.ControlWord);
...
static RList *r_debug_native_map_get(RDebug *dbg) {
	...
	while (!feof (fd)) {
		...
		sscanf (line, "%s %s %d %d 0x%s %3s %d %d", &region[2], &region2[2],
				&ign, &ign, unkstr, perms, &ign, &ign);
		pos_c = strchr (line, '/');
		if (pos_c) strncpy (path, pos_c, sizeof(path) - 1);
		else path[0] = '\0';
#else
		char null[64]; // XXX: this can overflow
		sscanf (line, "%s %s %s %s %s %s", &region[2], perms, null, null,
			null, path);
