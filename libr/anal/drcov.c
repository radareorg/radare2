...
typedef struct drcov_slurp_ctx_t {
	RStrBuf *sb;
} DrcovSlurpCtx;

static bool drcov_slurp_cb(void *user, ut64 addr, ut16 size) {
	DrcovSlurpCtx *ctx = (DrcovSlurpCtx *)user;
	(void)size;
	if (!ctx || !ctx->sb) {
		return false;
	}
	r_strbuf_appendf (ctx->sb, "0x%"PFMT64x"\n", addr);
	return true;
}

R_API char *r_anal_drcov_slurp(RAnal *anal, const char *path, size_t *len) {
	R_RETURN_VAL_IF_FAIL (anal && path, NULL);
	DrcovSlurpCtx ctx = { 0 };
	ctx.sb = r_strbuf_new (NULL);
	if (!ctx.sb) {
		return NULL;
	}
	int loaded = drcov_parse (anal, path, drcov_slurp_cb, &ctx, false);
	if (loaded == DRCOV_PARSE_NOT_DRCOV) {
		r_strbuf_free (ctx.sb);
		return NULL;
	}
	if (loaded < 0) {
		r_strbuf_free (ctx.sb);
	...
	char *out = r_strbuf_drain (ctx.sb);
	if (out) {
		r_str_trim_tail (out);
	}
	if (len) {
