...
static int java_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *data, int len, RArchOpMask mask);
...
static int java_switch_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *data, int len) {
	ut8 op_byte = data[0];
...
static ut64 extract_bin_op(ut64 ranal2_op_type) {
	...
	switch (bin_op_val) {
	case R_ANAL_JAVA_BINOP_XCHG:return R_ARCH_OP_TYPE_XCHG;
	case R_ANAL_JAVA_BINOP_CMP: return R_ARCH_OP_TYPE_CMP;
	case R_ANAL_JAVA_BINOP_ADD: return R_ARCH_OP_TYPE_ADD;
	case R_ANAL_JAVA_BINOP_SUB: return R_ARCH_OP_TYPE_SUB;
	case R_ANAL_JAVA_BINOP_MUL: return R_ARCH_OP_TYPE_MUL;
	case R_ANAL_JAVA_BINOP_DIV: return R_ARCH_OP_TYPE_DIV;
	case R_ANAL_JAVA_BINOP_SHR: return R_ARCH_OP_TYPE_SHR;
	case R_ANAL_JAVA_BINOP_SHL: return R_ARCH_OP_TYPE_SHL;
	case R_ANAL_JAVA_BINOP_SAL: return R_ARCH_OP_TYPE_SAL;
	case R_ANAL_JAVA_BINOP_SAR: return R_ARCH_OP_TYPE_SAR;
	case R_ANAL_JAVA_BINOP_OR : return R_ARCH_OP_TYPE_OR;
	case R_ANAL_JAVA_BINOP_AND: return R_ARCH_OP_TYPE_AND;
	case R_ANAL_JAVA_BINOP_XOR: return R_ARCH_OP_TYPE_XOR;
	case R_ANAL_JAVA_BINOP_NOT: return R_ARCH_OP_TYPE_NOT;
	case R_ANAL_JAVA_BINOP_MOD: return R_ARCH_OP_TYPE_MOD;
	case R_ANAL_JAVA_BINOP_ROR: return R_ARCH_OP_TYPE_ROR;
	case R_ANAL_JAVA_BINOP_ROL: return R_ARCH_OP_TYPE_ROL;
	default: break;
	}
	return R_ARCH_OP_TYPE_UNK;
}

static ut64 extract_unknown_op(ut64 ranal2_op_type) {
	if ((ranal2_op_type & R_ANAL_JAVA_CODEOP_JMP) == R_ANAL_JAVA_CODEOP_JMP) {
		return R_ARCH_OP_TYPE_UJMP;
	}
	if ((ranal2_op_type & R_ANAL_JAVA_CODEOP_CALL) == R_ANAL_JAVA_CODEOP_CALL) {
		return R_ARCH_OP_TYPE_UCALL;
	}
	if ((ranal2_op_type & R_ANAL_JAVA_LDST_OP_PUSH) == R_ANAL_JAVA_LDST_OP_PUSH) {
		return R_ARCH_OP_TYPE_UPUSH;
	}
	return R_ARCH_OP_TYPE_UNK;
}

static ut64 extract_code_op(ut64 ranal2_op_type) {
	ut64 conditional = R_ANAL_JAVA_COND_OP & ranal2_op_type ? R_ARCH_OP_TYPE_COND : 0;
	ut64 code_op_val = ranal2_op_type & (R_ANAL_JAVA_CODE_OP | 0x1FF);
	switch (code_op_val) {
	case R_ANAL_JAVA_CODEOP_CALL: return conditional | R_ARCH_OP_TYPE_CALL;
	case R_ANAL_JAVA_CODEOP_JMP: return conditional | R_ARCH_OP_TYPE_JMP;
	case R_ANAL_JAVA_CODEOP_RET: return conditional | R_ARCH_OP_TYPE_RET;
	case R_ANAL_JAVA_CODEOP_LEAVE: return R_ARCH_OP_TYPE_LEAVE;
	case R_ANAL_JAVA_CODEOP_SWI: return R_ARCH_OP_TYPE_SWI;
	case R_ANAL_JAVA_CODEOP_TRAP: return R_ARCH_OP_TYPE_TRAP;
	case R_ANAL_JAVA_CODEOP_SWITCH: return R_ARCH_OP_TYPE_SWITCH;
	}
	return R_ARCH_OP_TYPE_UNK;
}
...
static ut64 map_java_op_to_anal_op_type(ut64 t) {
	...
	if (t2 != R_ARCH_OP_TYPE_UNK) {
		return t2;
	}
	switch (t) {
	case R_ANAL_JAVA_NULL_OP: return R_ARCH_OP_TYPE_NULL;
	case R_ANAL_JAVA_NOP: return R_ARCH_OP_TYPE_NOP;
	case R_ANAL_JAVA_BINOP_ADD: return R_ARCH_OP_TYPE_ADD;
	case R_ANAL_JAVA_BINOP_AND: return R_ARCH_OP_TYPE_AND;
	case R_ANAL_JAVA_BINOP_MUL: return R_ARCH_OP_TYPE_MUL;
	case R_ANAL_JAVA_BINOP_XOR: return R_ARCH_OP_TYPE_XOR;
	case R_ANAL_JAVA_BINOP_XCHG: return R_ARCH_OP_TYPE_MOV;
	case R_ANAL_JAVA_OBJOP_NEW: return R_ARCH_OP_TYPE_UCALL;
	case R_ANAL_JAVA_OBJOP_SIZE: return R_ARCH_OP_TYPE_UCALL;
	case R_ANAL_JAVA_ILL_OP: return R_ARCH_OP_TYPE_ILL;
	default:
...
static int java_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *data, int len, RArchOpMask mask) {
	...
	*/
	...
	if (op->type == R_ARCH_OP_TYPE_POP) {
		op->stackop = R_ARCH_STACK_INC;
		op->stackptr = 8;
	}
		op->direction = R_ARCH_OP_DIR_EXEC;
	if (op->type == R_ARCH_OP_TYPE_PUSH) {
		op->stackop = R_ARCH_STACK_INC;
		op->stackptr = -8;
	}
	...
	} else if (op->type  == R_ARCH_OP_TYPE_JMP) {
		op->jump = addr + (short)(USHORT (data, 1));
		IFDBG eprintf ("%s jmpto 0x%04"PFMT64x".\n", JAVA_OPS[op_byte].name, op->jump);
	} else if ((op->type & R_ARCH_OP_TYPE_CALL) == R_ARCH_OP_TYPE_CALL ) {
