...
static void anop_esil (RAnal *a, RAnalOp *op, ut64 addr, const ut8 *buf, int len, csh *handle, cs_insn *insn) {
	...
	const char *si = (a->bits == 16) ? "si" : (a->bits == 32) ? "esi" : "rsi";
	struct Getarg gop = {
		.handle = *handle,
		.insn = insn,
		.bits = a->bits
	...
	const char *counter = (a->bits == 16) ? "cx" : (a->bits == 32) ? "ecx" : "rcx";
...
#endif
		cs_insn *insn = NULL;
	int mode = (a->bits == 64) ? CS_MODE_64 : (a->bits == 32) ? CS_MODE_32 : (a->bits == 16) ? CS_MODE_16 : 0;
	int n, ret;
