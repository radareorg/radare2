...
static void anop_esil(RAnal *a, RAnalOp *op, ut64 addr, const ut8 *buf, int len, csh *handle, cs_insn *insn) {
	...
	case X86_INS_TEST:
		{
			...
			if (insn->id == X86_INS_TEST) {
				...
				esilprintf (op, 
					"%s,%s,==,$z,zf,:=,%d,$b,cf,:=,$p,pf,:=,%d,$s,sf,:=,%d,$so,of,:=",
	...
	case X86_INS_DEC:
		...
		// are set according to the result.
		{
			ut32 bitsize;
			src = getarg (&gop, 0, 1, "--", SRC_AR, &bitsize);
			esilprintf (op, "%s,%d,$so,of,:=,%d,$s,sf,:=,$z,zf,:=,$p,pf,:=,3,$b,af,:=", src, bitsize - 1, bitsize - 1);
		}
		break;
	case X86_INS_PSUBB:
	case X86_INS_PSUBW:
	...
	case X86_INS_SUB:
		{
			...
			// represents a "borrow"
			esilprintf (op, "%s,%s,%d,$so,of,:=,%d,$s,sf,:=,$z,zf,:=,$p,pf,:=,%d,$b,cf,:=,3,$b,af,:=",
