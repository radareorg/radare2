...
static char *disassemble(RAnal *a, RArchOp *op, const ut8 *buf, int len) {
	ut8 bytes[8] = { 0 };
...
static int analop(RAnal *a, RArchOp *op, ut64 addr, const ut8 *buf, int len, RArchOpMask mask) {
	...
	default:
		...
		switch (buf[1]) {
		case 0x00:
			op->type = R_ARCH_OP_TYPE_CJMP; // BCC
			break;
		case 0xf3:
		...
		case 0x2d:
			...
			break;
		case 0xbf:
			op->type = R_ARCH_OP_TYPE_CALL; // bsr
			break;
		case 0xb9:
			op->type = R_ARCH_OP_TYPE_UJMP; // jsr reg
			break;
		}
	}
#if 0
