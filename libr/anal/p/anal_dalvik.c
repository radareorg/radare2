...
static int dalvik_disassemble(RAnal *a, RAnalOp *op, const ut8 *buf, int len) {
	...
	ut64 offset;
	//a->dataalign = 2;
	RStrBuf *mnemonic = r_strbuf_new (op->mnemonic);
	...
	if (buf_asm) {
		r_strbuf_set (mnemonic, buf_asm);
		op->mnemonic = r_strbuf_get (mnemonic);
	}
	strasm = NULL;
	if (size <= len) {
		...
		case fmtopvAAtBBBB: // "sput-*"
			...
			vB = (buf[3] << 8) | buf[2];
			if (buf[0] == 0x1a) {
				offset = R_ANAL_GET_OFFSET (a, 's', vB);
				if (offset == -1) {
				...
				flag_str = R_ANAL_GET_NAME (a, 'c', vB);
				if (!flag_str) {
				...
				flag_str = R_ANAL_GET_NAME (a, 'f', vB);
				if (!flag_str) {
		...
		case fmtoptopvAvBoCCCC:
			...
			vB = (buf[1] & 0xf0) >> 4;
			vC = (buf[3]<<8) | buf[2];
			offset = R_ANAL_GET_OFFSET (a, 'o', vC);
			if (offset == -1) {
		...
		case fmtopAAtBBBB:
			vA = (int) buf[1];
			vB = (buf[3] << 8) | buf[2];
			offset = R_ANAL_GET_OFFSET (a, 't', vB);
			if (offset == -1) {
		...
		case fmtopvAvBtCCCC:
			...
			if (buf[0] == 0x20 || buf[0] == 0x23) { //instance-of & new-array
				flag_str = R_ANAL_GET_NAME (a, 'c', vC);
				if (flag_str) {
				...
				flag_str = R_ANAL_GET_NAME (a, 'f', vC);
				if (flag_str) {
					snprintf (str, sizeof (str), " v%i, v%i, %s", vA, vB, flag_str);
				} else {
		...
		case fmtopvCCCCmBBBB:
			...
			if (buf[0] == 0x25) { // filled-new-array/range
				...
				flag_str = R_ANAL_GET_NAME (a, 's', vB);
				if (flag_str) {
					snprintf (str, sizeof (str), " {v%i..v%i}, %s", vC, vC + vA - 1, flag_str);
				}
				else {
				...
				flag_str = R_ANAL_GET_NAME (a, 'm', vB);
				if (flag_str) {
					snprintf (str, sizeof (str), " {v%i..v%i}, %s", vC, vC + vA - 1, flag_str);
				}
				else {
		...
		case fmtop45CC:
			...
			flag_str = R_ANAL_GET_NAME (a, 'p', vH);
			if (flag_str) {
				strasm = r_str_appendf (strasm, ", %s", flag_str);
			} else {
		...
		r_strbuf_set (mnemonic, r_str_get_fail (strasm, "invalid"));
		op->mnemonic = r_strbuf_get (mnemonic);


	} else if (len > 0) {
		r_strbuf_set (mnemonic, "invalid");
		op->mnemonic = r_strbuf_get (mnemonic);
		op->size = len;
		size = len;
	}
	...
	if (size + op->nopcode < 0) {
		op->size = 0;
	} else if (size + op->nopcode >= len) {
		op->size = len;
	} else {
		op->size = size + op->nopcode;
	}
	free (strasm);
	free ((char *)flag_str);
	return size;
...
static int dalvik_op(RAnal *anal, RAnalOp *op, ut64 addr, const ut8 *data, int len, RAnalOpMask mask) {
	...
	dalvik_disassemble (anal, op, data, len);
