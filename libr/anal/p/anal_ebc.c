...
static void ebc_anal_jmp8(RArchOp *op, ut64 addr, const ut8 *buf) {
	int jmpadr = (int8_t)buf[1];
	op->addr = addr;
	op->fail = addr + 2;
	op->jump = op->fail + (jmpadr * 2);
	op->type = TEST_BIT (buf[0], 7) ? R_ARCH_OP_TYPE_CJMP: R_ARCH_OP_TYPE_JMP;
}

static void ebc_anal_jmp(RArchOp *op, ut64 addr, const ut8 *buf) {
	op->fail = addr + 6;
	...
	if (buf[1] & 0x7) {
		...
		op->type = (TEST_BIT (buf[1], 7)) ? R_ARCH_OP_TYPE_CJMP: R_ARCH_OP_TYPE_JMP;
	}
}

static void ebc_anal_call(RArchOp *op, ut64 addr, const ut8 *buf) {
	op->fail = addr + 6;
...
static int ebc_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *buf, int len, RArchOpMask mask) {
	ebc_command_t cmd;
