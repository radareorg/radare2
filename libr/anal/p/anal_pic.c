...
typedef void (*pic_midrange_inst_handler_t) (RAnal *anal, RArchOp *op,
					     ut64 addr,
...
#define INST_HANDLER(OPCODE_NAME)					\
	static void _inst__##OPCODE_NAME (RAnal *anal, RArchOp *op,	\
					  ut64 addr,			\
					  PicMidrangeOpArgsVal *args)
...
static int anal_pic_midrange_op(RAnal *anal, RArchOp *op, ut64 addr,
...
static void pic18_cond_branch(RArchOp *op, ut64 addr, const ut8 *buf, char *flag) {
	op->type = R_ARCH_OP_TYPE_CJMP;
...
static int anal_pic_pic18_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *buf, int len) {
...
static int anal_pic_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *buf, int len, RArchOpMask mask) {
