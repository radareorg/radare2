...
static ut64 disarm_12bit_offset(RArchOp *op, unsigned int insoff) {
	ut64 off = insoff;
...
static RAnalValue *anal_pcrel_disp_mov(RAnal* anal, RArchOp* op, ut8 disp, int size) {
	RAnalValue *ret = r_anal_value_new ();
	if (size==2) {
		ret->base = op->addr + 4;
		ret->delta = disp << 1;
...
//= PC+4+R<reg>
static RAnalValue *anal_regrel_jump(RAnal* anal, RArchOp* op, ut8 reg) {
	RAnalValue *ret = r_anal_value_new ();
...
static int first_nibble_is_0(RAnal* anal, RArchOp* op, ut16 code) { //STOP
...
static int movl_reg_rdisp(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst, *src;
	op->type = R_ARCH_OP_TYPE_STORE;
...
static int first_nibble_is_2(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst = NULL, *src0 = NULL, *src1 = NULL;
	if (IS_MOVB_REG_TO_REGREF (code)) {	// 0010nnnnmmmm0000 mov.b <REG_M>,@<REG_N>
		...
		op->type = R_ARCH_OP_TYPE_ACMP;	//maybe not?
		src0 = anal_fill_ai_rg (anal, GET_SOURCE_REG (code));
...
static int first_nibble_is_3(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst = NULL, *src0 = NULL, *src1 = NULL;
...
static int first_nibble_is_4(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst = NULL;
	...
	if (IS_JSR (code)) {
		// op->type = R_ARCH_OP_TYPE_UCALL; //call to reg
		op->type = R_ARCH_OP_TYPE_RCALL; //call to reg
		op->delay = 1;
		dst = anal_fill_ai_rg (anal, GET_TARGET_REG (code));
		r_strbuf_setf (&op->esil, "1,SETD,pc,2,+,pr,=,r%d,pc,=", GET_TARGET_REG (code));
	} else if (IS_JMP (code)) {
		op->type = R_ARCH_OP_TYPE_UJMP; //jmp to reg
		dst = anal_fill_ai_rg (anal, GET_TARGET_REG (code));
		op->delay = 1;
		op->eob = true;
		...
		op->type = (code & 1)? R_ARCH_OP_TYPE_ROR:R_ARCH_OP_TYPE_ROL;
...
static int movl_rdisp_reg(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst, *src;
	op->type = R_ARCH_OP_TYPE_LOAD;
	dst = anal_fill_ai_rg (anal, GET_TARGET_REG (code));
...
static int first_nibble_is_6(RAnal* anal, RArchOp* op, ut16 code) {
...
static int add_imm(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst, *src;
	op->type = R_ARCH_OP_TYPE_ADD;
...
static int first_nibble_is_8(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst = NULL, *src = NULL;
	if (IS_BT_OR_BF (code)) {
		op->type = R_ARCH_OP_TYPE_CJMP; //Jump if true or jump if false insns
...
static int movw_pcdisp_reg(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst, *src;
	op->type = R_ARCH_OP_TYPE_LOAD;
	dst = anal_fill_ai_rg (anal, GET_TARGET_REG (code));
...
static int bra(RAnal* anal, RArchOp* op, ut16 code) {
...
static int bsr(RAnal* anal, RArchOp* op, ut16 code) {
...
static int first_nibble_is_c(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst = NULL, *src0 = NULL, *src1 = NULL;
	if (IS_TRAP (code)) {
...
static int movl_pcdisp_reg(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst, *src;
	op->type = R_ARCH_OP_TYPE_LOAD;
...
static int mov_imm_reg(RAnal* anal, RArchOp* op, ut16 code) {
	RAnalValue *dst, *src;
	op->type = R_ARCH_OP_TYPE_MOV;
	dst = anal_fill_ai_rg (anal, GET_TARGET_REG (code));
...
//nibble=f;
static int fpu_insn(RAnal* anal, RArchOp* op, ut16 code) {
...
static int (*first_nibble_decode[])(RAnal*,RArchOp*,ut16) = {
	first_nibble_is_0,
	movl_reg_rdisp,
	first_nibble_is_2,
	first_nibble_is_3,
 ...
 */
static int sh_op(RAnal *anal, RArchOp *op, ut64 addr, const ut8 *data, int len, RArchOpMask mask) {
	if (!op || !data || len < 2) {
		return 0;
	}
	op->addr = addr;
