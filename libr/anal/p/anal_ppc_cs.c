...
static const char* getspr(struct Getarg *gop, int n) {
    ...
    case SPR_HID6:
        return "hid6";
    default:
        snprintf(cspr, sizeof(cspr), "spr_%lu", spr);
        break;
    }
    return cspr;
}
    ...
    if (n < 1) {
        ...
        case PPC_INS_BA:
            ...
            case PPC_BC_LT:
                ...
                op->fail = addr + 4;
                if (ARG(1)[0] == '\0') esilprintf (op, "cr0,0,<,?{,%s,pc,=,BREAK,},", ARG(0));
                else esilprintf (op, "0,%s,<,?{,%s,pc,=,BREAK,},", ARG(0), ARG(1));
                break;
            case PPC_BC_LE:
                op->type = R_ANAL_OP_TYPE_CJMP;
                op->fail = addr + 4;
            ...
            case PPC_BC_GT:
                op->type = R_ANAL_OP_TYPE_CJMP;
                op->fail = addr + 4;
                if (ARG(1)[0] == '\0') esilprintf (op, "0,cr0,>,?{,%s,pc,=,BREAK,},", ARG(0));
                else esilprintf (op, "0,%s,>,?{,%s,pc,=,BREAK,},", ARG(0), ARG(1));
                break;
            case PPC_BC_NE:
                op->type = R_ANAL_OP_TYPE_CJMP;
        ...
        case PPC_INS_BDNZ:
            ...
            op->jump = (ut64)insn->detail->ppc.operands[0].imm;
            op->fail = addr+4;
            esilprintf (op, "ctr,0,!=,?{,%s,pc,=,BREAK,},4,pc,+=", ARG(0));
            break;
        case PPC_INS_BDNZA:
            op->type = R_ANAL_OP_TYPE_CJMP;
            op->jump = (ut64)insn->detail->ppc.operands[0].imm;
