...
static int analop_vle(RAnal *a, RArchOp *op, ut64 addr, const ut8 *buf, int len) {
	vle_t* instr = NULL;
	vle_handle handle = {0};
	op->size = 2;
	if (len > 1 && !vle_init (&handle, buf, len) && (instr = vle_next (&handle))) {
		...
		case R_ARCH_OP_TYPE_CJMP:
			op->cond = instr->cond; //R_ARCH_OP_COND_NE;
			op->eob = true;
			op->jump = addr + instr->fields[instr->n - 1].value;
			op->fail = addr + op->size;
			break;
		...
		case R_ARCH_OP_TYPE_XOR:
			break;
		default:
			//eprintf ("Missing an R_ARCH_OP_TYPE (%"PFMT64u")\n", op->type);
			break;
		}
		vle_free (instr);
		return op->size;
...
static int decompile_vle(RAnal *a, RArchOp *op, ut64 addr, const ut8 *buf, int len) {
	vle_t* instr = 0;
	vle_handle handle = {0};
	if (len < 2) {
		return -1;
...
static int decompile_ps(RAnal *a, RArchOp *op, ut64 addr, const ut8 *buf, int len) {
	ppcps_t instr = {0};
	if (len < 4) {
		eprintf ("not eno\n");
		return -1;
...
static int analop(RAnal *a, RArchOp *op, ut64 addr, const ut8 *buf, int len, RArchOpMask mask) {
	...
	if (n < 1) {
		...
		case PPC_INS_BCLR:
		case PPC_INS_BCLRL:
			op->type = R_ARCH_OP_TYPE_CRET;		//I'm a condret
			op->fail = addr + op->size;
			switch (insn->detail->ppc.bc) {
			case PPC_BC_INVALID:
