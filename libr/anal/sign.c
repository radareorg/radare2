...
static void serialize(RAnal *a, RSignItem *it, char *k, char *v) {
	...
	if (v) {
		...
		snprintf (v, R_SIGN_VAL_MAXSZ, "%d|%s|%s|%s|%"PFMT64d,
			bytes? bytes->size: 0,
			bytes? hexbytes: "0",
			bytes? hexmask: "0",
			graph? hexgraph: "0",
