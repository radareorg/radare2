...
R_API RAnalBlock * r_anal2_get_bb(RAnal *anal, RAnalInfos *state, ut64 addr) {
    ...
    IFDBG eprintf("[==] r_anal2_get_bb: op size @ 0x%04x seen 0x%04x\n", state->current_op->size, test);
...
R_API RList * r_anal2_perform_analysis( RAnal *anal, RAnalInfos *state, ut64 addr) {
	...
	return r_anal2_analysis_driver (anal, state, addr);
}

R_API RList * r_anal2_analysis_driver( RAnal *anal, RAnalInfos *state, ut64 addr ) {
	...
	r_anal2_perform_pre_anal (anal, state, state->current_addr);
	
	while (!state->done && bytes_consumed < len) {

		
		state->current_bb = r_anal2_state_search_bb (state, state->current_addr);
		// check state for bb
		
        if (state->current_bb) {
            // TODO something special should happen here.
            
            r_anal2_perform_revisit_bb_cb (anal, state, state->current_addr);
            bytes_consumed += state->current_bb->op_sz;
			if ( state->done) break;
            continue;
		}
	
		r_anal2_perform_pre_anal_op_cb (anal, state, state->current_addr);
		if (state->done) break;

	   	r_anal2_get_op (anal, state, state->current_addr);
		r_anal2_perform_post_anal_op_cb (anal, state, state->current_addr);
		if (state->done) break;
		

		r_anal2_perform_pre_anal_bb_cb (anal, state, state->current_addr);
		if (state->done) break;
		

		r_anal2_get_bb (anal, state, state->current_addr);
		...
		r_anal2_perform_post_anal_bb_cb (anal, state, state->current_addr);
		if (state->done) {
			break;
		}
