...
static RStrBuf *get_resolved_expr(RAnalEsilDFGFilter *filter, RAnalEsilDFGNode *node) {
	...
	while ((token = r_str_tok_r (NULL, ",", &save_ptr))) {
		RGraphNode *gn = sdb_ptr_get (filter->results, prev_token, 0);
		if (!gn) {
			r_strbuf_appendf (res, ",%s,", prev_token);
		} else {
			bool cont = false;
			char *c = prev_token;
			while (r_list_length (gn->out_nodes) == 2) {
				...
				RStrBuf *r = r_strbuf_new (c);
				r_strbuf_append (r, ",");
				r_strbuf_append (r, r_strbuf_get (node->content));
				gn = sdb_ptr_get (filter->results, r_strbuf_get (r), 0);
				if (!gn) {
