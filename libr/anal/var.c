...
R_API void r_anal_extract_rarg(RAnal *anal, RAnalOp *op, RAnalFunction *fcn, int *reg_set, int *count) {
	...
	// only an fp or vector instruction can carry a floating-point argument, so
	// the integer bulk of a function never walks the second sequence
	...
	const bool scan_fpargs = op->family == R_ANAL_OP_FAMILY_FPU
		...
		// a spill of an fp argument is tagged by what it does, not by the
		// register it moves: arm64 gives "str d0, [sp]" the cpu family
		|| optype == R_ANAL_OP_TYPE_STORE
		|| optype == R_ANAL_OP_TYPE_LOAD;
	const struct {
		bool fp;
		int slot_base;
		int count;
		...
		{ true, R_ANAL_CC_FPSLOT_BASE,
			scan_fpargs? r_anal_cc_max_fparg (anal, fcn->callconv): 0, "double" },
	};
	size_t seq;
...
R_API void r_anal_var_list_show(RAnal *anal, RAnalFunction *fcn, int kind, int mode, PJ *pj) {
	...
	if (kind == R_ANAL_VAR_KIND_REG) {
		// leaves the vec sorted by argnum
