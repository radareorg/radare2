...
static int read_ahead(RAnal *anal, ut64 addr, ut8 *buf, int len) {
	const int cache_len = sizeof (anal->r_anal_cache->linear.cache);
	...
	if (len > cache_len) {
		...
		memcpy (anal->r_anal_cache->linear.cache, buf, cache_len);
		anal->r_anal_cache->linear.cache_addr = addr;
		return a;
	}

	ut64 addr_end = UT64_ADD_OVFCHK (addr, len)? UT64_MAX: addr + len;
	ut64 cache_addr_end = UT64_ADD_OVFCHK (anal->r_anal_cache->linear.cache_addr, cache_len)? UT64_MAX: anal->r_anal_cache->linear.cache_addr + cache_len;
	bool isCached = ((addr != UT64_MAX) && (addr >= anal->r_anal_cache->linear.cache_addr) && (addr_end < cache_addr_end));
	if (isCached) {
		memcpy (buf, anal->r_anal_cache->linear.cache + (addr - anal->r_anal_cache->linear.cache_addr), len);
	} else {
		anal->iob.read_at (anal->iob.io, addr, anal->r_anal_cache->linear.cache, sizeof (anal->r_anal_cache->linear.cache));
		memcpy (buf, anal->r_anal_cache->linear.cache, len);
		anal->r_anal_cache->linear.cache_addr = addr;
	}
	return len;
}
...
static int fcn_recurse(RAnal *anal, RAnalFunction *fcn, ut64 addr, ut64 len, int depth) {
	...
	anal->r_anal_cache->linear.cmpval = UT64_MAX;
	struct {
		int cnt;
		int idx;
		int after;
	...
	anal->r_anal_cache->linear.lea_jmptbl_ip = UT64_MAX;
...
repeat:
			...
			if (anal->opt.jmptbl) {
				...
				if (is_delta_pointer_table (anal, fcn, op->addr, op->ptr, &jmptbl_addr, &casetbl_addr, jmp_aop)) {
					...
					if (ready) {
						...
						if (ret) {
							anal->r_anal_cache->linear.lea_jmptbl_ip = addr;
						}
					}
				}
		...
		case R_ANAL_OP_TYPE_SUB:
			if (op->val != UT64_MAX && op->val > 0) {
				...
				anal->r_anal_cache->linear.cmpval = op->val;
			}
			break;
		case R_ANAL_OP_TYPE_CMP: {
			ut64 val = (is_x86 || is_v850)? op->val : op->ptr;
			if (val) {
				anal->r_anal_cache->linear.cmpval = val;
				bb->cmpval = anal->r_anal_cache->linear.cmpval;
				bb->cmpreg = op->reg;
		...
		case R_ANAL_OP_TYPE_UCJMP:
			...
			if (anal->opt.jmptbl) {
				if (op->ptr != UT64_MAX) {
					...
					table_size = anal->r_anal_cache->linear.cmpval + 1;
					default_case = op->fail; // is this really default case?
					if (anal->r_anal_cache->linear.cmpval != UT64_MAX && default_case != UT64_MAX && (op->reg || op->ireg)) {
						// TODO -1
						if (op->ireg) {
		...
		case R_ANAL_OP_TYPE_RJMP:
			if (is_arm && last_is_mov_lr_pc) {
				...
				int ptsz = anal->r_anal_cache->linear.cmpval? anal->r_anal_cache->linear.cmpval + 1: 4;
				if ((int)anal->r_anal_cache->linear.cmpval > 0) {
		...
		case R_ANAL_OP_TYPE_IRJMP:
			...
			// switch statement
			if (anal->opt.jmptbl && anal->r_anal_cache->linear.lea_jmptbl_ip != op->addr) {
				ut8 buf[32]; // 32 bytes is enough to hold any instruction.
				...
				if (op->ptr != UT64_MAX && op->ireg) { // direct jump
					...
					if (op->ptrsize == 1) { // TBB
						...
						if (pred_cmpval != UT64_MAX) {
							...
							table_size += anal->r_anal_cache->linear.cmpval;
						}
					...
					if (op->ptrsize == 2) { // LDRH on thumb/arm
						...
						if (pred_cmpval != UT64_MAX) {
							...
							tablesize += anal->r_anal_cache->linear.cmpval;
						}
						ret = try_walkthrough_jmptbl (anal, fcn, bb, depth - 1, op->addr, 0, op->addr + op->size,
			...
			if (anal->r_anal_cache->linear.lea_jmptbl_ip == op->addr) {
				anal->r_anal_cache->linear.lea_jmptbl_ip = UT64_MAX;
			}
			if (anal->opt.ijmp) {
