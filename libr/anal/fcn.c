...
static int fcn_recurse(RAnal *anal, RAnalFunction *fcn, ut64 addr, ut64 len, int depth) {
	...
	anal->r_anal_cache->lea_jmptbl_ip = UT64_MAX;
...
repeat:
			...
			if (anal->opt.jmptbl) {
				...
				if (is_delta_pointer_table (anal, fcn, op->addr, op->ptr, &jmptbl_addr, &casetbl_addr, jmp_aop)) {
					...
					if (ready) {
						...
						if (ret) {
							anal->r_anal_cache->lea_jmptbl_ip = addr;
						}
					}
				}
		...
		case R_ANAL_OP_TYPE_IRJMP:
			...
			// switch statement
			if (anal->opt.jmptbl && anal->r_anal_cache->lea_jmptbl_ip != op->addr) {
				ut8 buf[32]; // 32 bytes is enough to hold any instruction.
			...
			if (anal->r_anal_cache->lea_jmptbl_ip == op->addr) {
				anal->r_anal_cache->lea_jmptbl_ip = UT64_MAX;
			}
			if (anal->opt.ijmp) {
