...
static void function_context_register_param_free(RAnalFunctionContextRegisterParam *param) {
	if (!param) {
		return;
	}
	free (param->name);
...
static void function_context_stack_slot_free(RAnalFunctionContextStackSlot *slot) {
	if (!slot) {
		return;
	}
	free (slot->name);
	free (slot->type);
	free (slot->base.name);
	free (slot->param_name);
	free (slot->source_reg);
	free (slot);
}

static const RAnalFunctionContextParam *function_context_param_at(RList *params, int index) {
	RListIter *iter;
...
static bool function_context_is_default_arg_name(const char *name) {
...
static char *function_context_dup_var_regname(RAnal *anal, const RAnalVar *var) {
	if (!anal || !var) {
		return NULL;
	}
...
static st64 function_context_stack_offset(const RAnalFunction *fcn, const RAnalVar *var) {
...
static RAnalVar *function_context_find_register_home_source(RList *rvars, RAnalVar *slot) {
	RListIter *iter;
	RAnalVar *var;
...
static RAnalFunctionContextStackSlotRole function_context_classify_stack_slot(const RAnalVar *var, RAnalVar *home_source) {
	R_RETURN_VAL_IF_FAIL (var, R_ANAL_FUNCTION_CONTEXT_STACK_SLOT_ROLE_UNKNOWN);
	if (home_source) {
		return R_ANAL_FUNCTION_CONTEXT_STACK_SLOT_ROLE_PARAM_HOME;
	}
	if (var->isarg) {
		return R_ANAL_FUNCTION_CONTEXT_STACK_SLOT_ROLE_STACK_ARG;
	}
	if (var->kind == R_ANAL_VAR_KIND_BPV || var->kind == R_ANAL_VAR_KIND_SPV) {
		return R_ANAL_FUNCTION_CONTEXT_STACK_SLOT_ROLE_LOCAL;
	}
	return R_ANAL_FUNCTION_CONTEXT_STACK_SLOT_ROLE_UNKNOWN;
}

static RAnalFunctionContextRegisterParam *function_context_collect_register_param(RAnal *anal, RAnalFunctionContext *ctx, RAnalVar *var) {
	const RAnalFunctionContextParam *signature_param = NULL;
	RAnalFunctionContextRegisterParam *param = NULL;
	const int param_index = r_anal_var_get_argnum (var);

	R_RETURN_VAL_IF_FAIL (anal && ctx && var, NULL);
	param = R_NEW0 (RAnalFunctionContextRegisterParam);
	if (!param) {
		return NULL;
	}
	param->param_index = param_index;
	signature_param = function_context_param_at (ctx->params, param_index);
	if (signature_param && R_STR_ISNOTEMPTY (signature_param->name) && function_context_is_default_arg_name (var->name)) {
		param->name = strdup (signature_param->name);
	} else if (R_STR_ISNOTEMPTY (var->name)) {
		param->name = strdup (var->name);
	}
	if (R_STR_ISNOTEMPTY (var->type)) {
		param->type = strdup (var->type);
	} else if (signature_param && R_STR_ISNOTEMPTY (signature_param->type)) {
		param->type = strdup (signature_param->type);
	}
	param->reg = function_context_dup_var_regname (anal, var);
	if ((R_STR_ISNOTEMPTY (var->name) && !param->name)
		|| (R_STR_ISNOTEMPTY (var->type) && !param->type)
		|| !param->reg) {
		function_context_register_param_free (param);
		return NULL;
	}
	return param;
}

static RAnalFunctionContextStackSlot *function_context_collect_stack_slot(RAnal *anal, RAnalFunctionContext *ctx, RAnalFunction *fcn, RAnalVar *var, RAnalVar *home_source) {
	const RAnalFunctionContextParam *signature_param = NULL;
	RAnalFunctionContextStackSlot *slot = NULL;
	int param_index = -1;

	R_RETURN_VAL_IF_FAIL (anal && ctx && fcn && var, NULL);
	slot = R_NEW0 (RAnalFunctionContextStackSlot);
	if (!slot) {
		return NULL;
	}
	...
	slot->base.kind = (var->kind == R_ANAL_VAR_KIND_BPV)
		? R_ANAL_FUNCTION_CONTEXT_STACK_BASE_FRAME_POINTER
		: R_ANAL_FUNCTION_CONTEXT_STACK_BASE_STACK_POINTER;
	slot->offset = function_context_stack_offset (fcn, var);
	slot->role = function_context_classify_stack_slot (var, home_source);

	if (home_source) {
		param_index = r_anal_var_get_argnum (home_source);
		signature_param = function_context_param_at (ctx->params, param_index);
		slot->param_index = param_index;
		slot->source_reg = function_context_dup_var_regname (anal, home_source);
		if (signature_param && R_STR_ISNOTEMPTY (signature_param->name)) {
			slot->param_name = strdup (signature_param->name);
		} else if (R_STR_ISNOTEMPTY (home_source->name)) {
			slot->param_name = strdup (home_source->name);
		}
		if (!slot->type && signature_param && R_STR_ISNOTEMPTY (signature_param->type)) {
			slot->type = strdup (signature_param->type);
		}
	} else if (var->isarg) {
		param_index = r_anal_var_get_argnum (var);
		slot->param_index = param_index;
		if (param_index >= 0) {
			signature_param = function_context_param_at (ctx->params, param_index);
			if (signature_param && R_STR_ISNOTEMPTY (signature_param->name)) {
				...
				slot->param_name = strdup (var->name);
			}
			if (!slot->type && signature_param && R_STR_ISNOTEMPTY (signature_param->type)) {
				slot->type = strdup (signature_param->type);
			}
	...
	if ((R_STR_ISNOTEMPTY (var->name) && !slot->name)
		...
		|| (slot->param_index >= 0 && R_STR_ISNOTEMPTY (slot->param_name) && !slot->param_name)
		|| (home_source && !slot->source_reg)) {
		function_context_stack_slot_free (slot);
		return NULL;
	}
	return slot;
}
...
R_API void r_anal_function_context_free(RAnalFunctionContext *ctx) {
	...
	r_list_free (ctx->register_params);
	r_list_free (ctx->stack_slots);
...
R_API RAnalFunctionContext *r_anal_function_context_collect(RAnal *anal, RAnalFunction *fcn) {
	...
	ctx = R_NEW0 (RAnalFunctionContext);
	if (!ctx) {
		return NULL;
	}
	ctx->params = r_list_newf ((RListFree)function_param_free);
	ctx->register_params = r_list_newf ((RListFree)function_context_register_param_free);
	ctx->stack_slots = r_list_newf ((RListFree)function_context_stack_slot_free);
	if (!ctx->params || !ctx->register_params || !ctx->stack_slots) {
	...
	if (signature) {
		ctx->signature = signature->signature;
		ctx->ret_type = signature->ret_type;
		ctx->callconv = signature->callconv;
		ctx->noreturn = signature->noreturn;
		signature->signature = NULL;
		signature->ret_type = NULL;
		signature->callconv = NULL;
		if (signature->params) {
			r_list_free (ctx->params);
			ctx->params = signature->params;
			signature->params = NULL;
		}
		r_anal_function_signature_free (signature);
	}
	if (!ctx->callconv && R_STR_ISNOTEMPTY (fcn->callconv)) {
		ctx->callconv = strdup (fcn->callconv);
	}
	ctx->noreturn = ctx->noreturn || fcn->is_noreturn;
	...
	r_list_foreach (cache.rvars, iter, var) {
		...
		RAnalFunctionContextRegisterParam *param = function_context_collect_register_param (anal, ctx, var);
		if (!param) {
			r_anal_function_vars_cache_fini (&cache);
			r_anal_function_context_free (ctx);
			return NULL;
		}
		r_list_append (ctx->register_params, param);
	}
	...
	r_list_foreach (cache.bvars, iter, var) {
		...
		RAnalVar *home_source = function_context_find_register_home_source (cache.rvars, var);
		RAnalFunctionContextStackSlot *slot = function_context_collect_stack_slot (anal, ctx, fcn, var, home_source);
		if (!slot) {
			r_anal_function_vars_cache_fini (&cache);
			r_anal_function_context_free (ctx);
			return NULL;
		}
		r_list_append (ctx->stack_slots, slot);
	}
