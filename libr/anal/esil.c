...
R_API int r_anal_esil_set_interrupt(RAnalEsil *esil, int interrupt, RAnalEsilInterruptCB interruptcb) {
	...
	if (!sdb_num_exists (esil->interrupts, i)) {
		R_LOGFI ("can't set interrupt-handler for interrupt %d\n", interrupt);
		return false;
	}
	return true;
}
...
R_API int r_anal_esil_fire_interrupt(RAnalEsil *esil, int interrupt) {
	...
	i = sdb_itoa ((ut64)interrupt, t, 16);
	if (!sdb_num_exists (esil->interrupts, i)) {
		//R_LOGFI ("0x%08"PFMT64x" Invalid interrupt/syscall 0x%08x\n", esil->address, interrupt);
		return false;
	}
	...
	if (src) {
		if (r_anal_esil_get_parm (esil, src, &num)) {
			...
			if (isregornum (esil, src, &num)) {
				...
				R_LOGFI ("0x%08"PFMT64x" esil_neg: unknown reg %s\n", esil->address, src);
			}
		}
	} else {
...
static int esil_interrupt_linux_i386(RAnalEsil *esil) { 		//move this into a plugin
	...
	if (sn != 0x80) {
		R_LOGFI ("Interrupt 0x%x not handled.", sn);
...
R_API int r_anal_esil_dumpstack(RAnalEsil *esil) {
	...
	if (esil->trap) {
		R_LOGFI ("ESIL TRAP type %d code 0x%08x %s\n",
...
static int esil_mod(RAnalEsil *esil) {
	...
	if (src && r_anal_esil_get_parm (esil, src, &s)) {
		if (dst && r_anal_esil_get_parm (esil, dst, &d)) {
			if (s == 0) {
				if (esil->verbose > 0) {
					R_LOGFI ("0x%08"PFMT64x" esil_mod: Division by zero!\n", esil->address);
				}
	...
	if (src && r_anal_esil_get_parm (esil, src, &s)) {
		if (dst && r_anal_esil_reg_read (esil, dst, &d, NULL)) {
			if (s) {
				...
				// R_LOGFI ("0x%08"PFMT64x" esil_diveq: Division by zero!\n", esil->address);
				esil->trap = R_ANAL_TRAP_DIVBYZERO;
				esil->trap_code = 0;
			}
			ret = 1;
...
static int esil_poke_n(RAnalEsil *esil, int bits) {
	...
	//R_LOGFI ("GONA POKE %d src:%s dst:%s\n", bits, src, dst);
		...
		if (count) {
			...
			if (regs > 0) {
				...
				for (i = 0; i < regs; i++) {
					...
					if (ret != BYTES_SIZE) {
						//R_LOGFI ("Cannot write at 0x%08" PFMT64x "\n", ptr);
						esil->trap = 1;
					}
					ptr += BYTES_SIZE;
					free (foo);
			...
			if (regs > 0) {
				...
				for (i = 0; i < regs; i++) {
					...
					if (ret == sizeof (ut32)) {
						...
						if (esil->verbose) {
							R_LOGFI ("Cannot peek from 0x%08" PFMT64x "\n", ptr);
						}
					}
					ptr += sizeof (ut32);
					free (foo);
