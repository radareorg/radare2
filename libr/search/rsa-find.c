...
R_API ut8* r_parse_next(ut8* start, ut32 *type, ut32 *len) {
	ut8 *val = NULL;
	*type = start[0]; 
	*len  = 0;
	if ((start[1] & 0x80) == 0) {
...
static int check_rsa_fields(ut8* start, int nbfields) {
	ut32 len=0, type;
	int i;
	...
	for (i = 0; i < nbfields; i++)
		if (!(start = r_parse_next (start,&type,&len)))
			return R_FALSE;
	return R_TRUE;
}
...
static ut8 *r_find_key_start(const ut8 *map, int offset) {
	int k;
...
R_API int r_search_rsa_update(void* s, ut64 from, const ut8 *buf, int len) {
	...
	for (i = 0; i < len - sizeof (versionmarker); i++) {
		...
		ut8 *key = r_find_key_start(buf, i);
		if (!key)
			continue;
