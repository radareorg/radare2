...
static inline grub_uint64_t grub_xfs_inode_block (struct grub_xfs_data *data, grub_uint64_t ino) {
...
static grub_err_t grub_xfs_read_inode (struct grub_xfs_data *data, grub_uint64_t ino, struct grub_xfs_inode *inode) {
...
static grub_disk_addr_t grub_xfs_read_block (grub_fshelp_node_t node, grub_disk_addr_t fileblock) {
	...
	if (node->inode.format == XFS_INODE_FORMAT_BTREE) {
		...
		do {
			...
			if (grub_disk_read (node->data->disk, grub_be_to_cpu64 (keys[i - 1 + nrec])
					<< (node->data->sblock.log2_bsize - GRUB_DISK_SECTOR_BITS),
...
static grub_ssize_t grub_xfs_read_file (grub_fshelp_node_t node,
     void (*read_hook) (grub_disk_addr_t sector, unsigned offset, unsigned length),
     int pos, grub_size_t len, char *buf) {
	return grub_fshelp_read_file (node->data->disk, node, read_hook,
			pos, len, buf, grub_xfs_read_block, grub_be_to_cpu64 (node->inode.size),
			node->data->sblock.log2_bsize - GRUB_DISK_SECTOR_BITS);
}

static char * grub_xfs_read_symlink (grub_fshelp_node_t node) {
...
grub_xfs_mode_to_filetype (grub_uint16_t mode) {
	grub_uint16_t p = grub_be_to_cpu16 (mode);
	if ((p & FILETYPE_INO_MASK) == FILETYPE_INO_DIRECTORY)
		return GRUB_FSHELP_DIR;
	if ((p & FILETYPE_INO_MASK) == FILETYPE_INO_SYMLINK)
		return GRUB_FSHELP_SYMLINK;
	if ((p & FILETYPE_INO_MASK) == FILETYPE_INO_REG)
		return GRUB_FSHELP_REG;
	return GRUB_FSHELP_UNKNOWN;
}

static int call_hook (grub_fshelp_node_t diro, grub_uint64_t ino, char *filename,
       int (*hook) (const char *filename, enum grub_fshelp_filetype filetype, grub_fshelp_node_t node)
) {
	struct grub_fshelp_node *fdiro;

	fdiro = grub_malloc (sizeof (struct grub_fshelp_node)
		- sizeof (struct grub_xfs_inode) + (1 << diro->data->sblock.log2_inode));
	if (!fdiro)
		return 0;
	...
	return hook (filename, grub_xfs_mode_to_filetype (fdiro->inode.mode), fdiro);
}

static int grub_xfs_iterate_dir (grub_fshelp_node_t dir,
		int (*hook) (const char *filename, enum grub_fshelp_filetype filetype, grub_fshelp_node_t node))
{
	...
	case XFS_INODE_FORMAT_INO:
		...
		int i, smallino = !diro->inode.data.dir.dirhead.smallino;
		...
		} else parent = diro->inode.data.dir.dirhead.parent.i8;

		/* Synthesize the direntries for `.' and `..'.  */
		if (call_hook (diro, diro->ino, ".", hook))
			return 1;

		if (call_hook (diro, parent, "..", hook))
			return 1;
		...
		for (i = 0; i < diro->inode.data.dir.dirhead.count; i++) {
			...
			name[de->len] = '\0';
			if (call_hook (diro, ino, name, hook))
				return 1;
			de = ((struct grub_xfs_dir_entry *) (((char *) de)+ sizeof (struct grub_xfs_dir_entry) + de->len
						+ ((smallino ? sizeof (grub_uint32_t) : sizeof (grub_uint64_t))) - 1));
		}
		}
		break;
	...
	case XFS_INODE_FORMAT_EXT:
		{
					...
					blk++)
			{
				...
				while (pos < (dirblk_size - (int) sizeof (struct grub_xfs_dir2_entry))) {
					...
					if (call_hook (diro, direntry->inode, filename, hook)) {
						grub_free (dirblock);
						return 1;
					}
	...
	default:
		grub_error (GRUB_ERR_NOT_IMPLEMENTED_YET,
			"XFS does not support inode format %d yet", diro->inode.format);
	}
	return 0;
}

static struct grub_xfs_data * grub_xfs_mount (grub_disk_t disk) {
	...
	/* Read the superblock.  */
	if (grub_disk_read (disk, 0, 0, sizeof (struct grub_xfs_sblock), &data->sblock))
		goto fail;
	...
	data = grub_realloc (data, sizeof (struct grub_xfs_data)
		- sizeof (struct grub_xfs_inode) + (1 << data->sblock.log2_inode));
...
static grub_err_t grub_xfs_dir (grub_device_t device, const char *path,
...
static grub_err_t grub_xfs_open (struct grub_file *file, const char *name) {
	struct grub_xfs_data *data;
	struct grub_fshelp_node *fdiro = 0;
	...
	if (fdiro != &data->diropen)
		memcpy (&data->diropen, fdiro, sizeof (struct grub_fshelp_node)
			- sizeof (struct grub_xfs_inode) + (1 << data->sblock.log2_inode));
...
static grub_ssize_t grub_xfs_read (grub_file_t file, char *buf, grub_size_t len) {
	struct grub_xfs_data *data = (struct grub_xfs_data *) file->data;
	return grub_xfs_read_file (&data->diropen, file->read_hook, file->offset, len, buf);
}

static grub_err_t grub_xfs_close (grub_file_t file) {
	grub_free (file->data);
	grub_dl_unref (my_mod);
	return GRUB_ERR_NONE;
}

static grub_err_t grub_xfs_label (grub_device_t device, char **label) {
	struct grub_xfs_data *data;
	grub_disk_t disk = device->disk;
	grub_dl_ref (my_mod);
	data = grub_xfs_mount (disk);
	*label = data?grub_strndup ((char *) (data->sblock.label), 12):0;
	grub_dl_unref (my_mod);
	grub_free (data);
	return grub_errno;
}

static grub_err_t grub_xfs_uuid (grub_device_t device, char **uuid) {
	struct grub_xfs_data *data;
	grub_disk_t disk = device->disk;
	grub_dl_ref (my_mod);
	data = grub_xfs_mount (disk);
