...
sdb:
	if [ ! -d "sdb" ] || ! (cd "sdb" 2>/dev/null && test "$$(git rev-parse HEAD 2>/dev/null)" = "$$(git rev-parse '2.5.0^{commit}' 2>/dev/null)"); then rm -rf "sdb"; ${MAKE} sdb_all; fi

sdb_all:
	git clone --no-checkout --depth=1 https://github.com/radareorg/sdb.git sdb
	cd sdb && git fetch --depth=1 origin 2.5.0
