...
otezip:
	if [ ! -d "otezip" -o "0.4.4" != "$(shell cd otezip 2>/dev/null && git rev-parse HEAD)" ]; then rm -rf "otezip"; ${MAKE} otezip_all; fi

otezip_all:
	git clone --no-checkout --depth=1 https://github.com/trufae/otezip.git otezip
	cd otezip && git fetch --depth=1 origin 0.4.4
