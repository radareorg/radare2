# This file is autogenerated by acr-wrap

WRAP_wrap_git_url:=https://github.com/trufae/otezip.git
WRAP_wrap_git_revision:=13455f964dcc62e707d549e37514fd3a68578a39
# patch_directory = otezip
WRAP_wrap_git_directory:=otezip
WRAP_wrap_git_depth:=1

.PHONY: otezip_clean otezip_all

otezip:
	if [ ! -d "otezip" -o "13455f964dcc62e707d549e37514fd3a68578a39" != "$(shell cd otezip 2>/dev/null && git rev-parse HEAD)" ]; then rm -rf "otezip"; ${MAKE} otezip_all; fi

otezip_all:
	git clone --no-checkout --depth=1 https://github.com/trufae/otezip.git otezip
	cd otezip && git fetch --depth=1 origin 13455f964dcc62e707d549e37514fd3a68578a39
	cd otezip && git checkout FETCH_HEAD

otezip_clean:
	rm -rf otezip
