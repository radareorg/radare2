...
bool test_r_str_encoded_json(void) {
	...
	const char *array = r_str_encoded_json (invalidJsonString, len, PJ_ENCODING_ARRAY);
	...
	const char *hex = r_str_encoded_json (invalidJsonString, len, PJ_ENCODING_HEX);
	mu_assert_streq (hex, "54686973206973206D7920E22073616D706C6520C2A920737472696E670A", "string as hexpairs");
	const char *b64 = r_str_encoded_json (invalidJsonString, len, PJ_ENCODING_BASE64);
	mu_assert_streq (b64, "VGhpcyBpcyBteSDiIHNhbXBsZSDCqSBzdHJpbmcK", "string as base64 encoded");
	const char *stripped = r_str_encoded_json (invalidJsonString, len, PJ_ENCODING_STRIP);
