...
bool test_r_io_cache(void) {
	...
	r_io_write (io, "ZZZZZZZZZZZZZZZ", 15);
	r_io_cache_init (io);
	...
	mu_assert_true (r_io_cache_write (io, 0, "AAAAA", 5), "Cache write at 0 failed");
	mu_assert_true (r_io_cache_write (io, 10, "BBBBB", 5), "Cache write at 10 failed");
	...
	mu_assert_memeq (buf, "AAAAAZZZZZBBBBB", sizeof (buf), "Cache read doesn't match expected output");
	memset (buf, 'Z', sizeof (buf));
	mu_assert_true (r_io_cache_write (io, 0, "CC", 2), "Overlapped cache write at 0 failed");
	mu_assert_true (r_io_cache_write (io, 4, "DD", 2), "Overlapped cache write at 4 failed");
	mu_assert_true (r_io_cache_write (io, 8, "EEE", 3), "Cache write at 4 failed");
	...
	mu_assert_memeq (buf, "CCAADDZZEEEBBBB", sizeof (buf), "Cache read doesn't match expected output");
	mu_assert_true (r_io_cache_write (io, 0, "FFFFFFFFFFFFFFF", 15), "Cache write failed");
	mu_assert_true (r_io_cache_read (io, 0, buf, sizeof (buf)), "Cache read failed");
	mu_assert_memeq (buf, "FFFFFFFFFFFFFFF", sizeof (buf), "Cache read doesn't match expected output");
	r_io_read_at (io, 0, buf, sizeof (buf));
	mu_assert_memeq (buf, "ZZZZZZZZZZZZZZZ", sizeof (buf), "IO read without cache doesn't match expected output");
	io->cached = R_PERM_R;
	r_io_read_at (io, 0, buf, sizeof (buf));
	mu_assert_memeq (buf, "FFFFFFFFFFFFFFF", sizeof (buf), "IO read with cache doesn't match expected output");
	r_io_cache_invalidate (io, 6, 1);
	memset (buf, 'Z', sizeof (buf));
	r_io_read_at (io, 0, buf, sizeof (buf));
	mu_assert_memeq (buf, "CCAADDZZEEEBBBB", sizeof (buf), "IO read after cache invalidate doesn't match expected output");
	r_io_cache_commit (io, 0, 15);
	memset (buf, 'Z', sizeof (buf));
	io->cached = 0;
	r_io_read_at (io, 0, buf, sizeof (buf));
	mu_assert_memeq (buf, "CCAADDZZEEEBBBB", sizeof (buf), "IO read after cache commit doesn't match expected output");
	r_io_free (io);
	mu_end;
}
