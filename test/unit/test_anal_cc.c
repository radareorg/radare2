...
static RAnal *ref_anal_bits(int bits) {
	RAnal *anal = r_anal_new ();
...
bool test_r_anal_cc_argslot_shadow(void) {
	...
	// a fixed slot counts frame words from the base, so it can name a slot inside the shadow area
...
bool test_r_anal_cc_argslot_regwidth(void) {
	RAnal *anal = ref_anal_bits (32);
	r_reg_set_profile_string (anal->reg,
		"=PC	pc\n=SP	sp\n"
		"gpr	r0	.64	0	0\ngpr	sp	.32	8	0\ngpr	pc	.32	12	0\n");
	RAnalCCArgSlot s;
	r_anal_cc_set (anal, "r0 wide(r0, stack)");
	mu_assert_true (r_anal_cc_argslot (anal, "wide", 1, -1, false, &s), "reg-first stack arg");
	mu_assert_eq (s.size, 8, "the first register argloc widens the slot");
	r_anal_cc_set (anal, "r0 late(stack)");
	sdb_set (anal->sdb_cc, "cc.late.arg1", "r0", 0);
	mu_assert_true (r_anal_cc_argslot (anal, "late", 0, -1, false, &s), "stack-first arg0");
	mu_assert_eq (s.size, 4, "a stack slot ends the width scan");
	r_anal_free (anal);
	mu_end;
}
