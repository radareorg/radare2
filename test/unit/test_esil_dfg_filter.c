...
bool test_filter_regs(void) {
	...
	REsilDFG *dfg = r_esil_dfg_expr (anal, NULL, "0x9090,ax,:=,0xff,ah,:=");

	// filter for ax register
	RStrBuf *filtered_expr = r_esil_dfg_filter (dfg, "ax");
	...
	filtered_expr = r_esil_dfg_filter (dfg, "ah");
	r_esil_parse (esil, r_strbuf_get (filtered_expr));
	...
	filtered_expr = r_esil_dfg_filter (dfg, "al");
	r_esil_parse (esil, r_strbuf_get (filtered_expr));
...
bool test_lemon_const_folder(void) {
	...
	REsilDFG *dfg = r_esil_dfg_expr (anal, NULL, "4,!,3,ebx,:=,!,1,+,eax,:=");
	r_esil_dfg_fold_const (anal, dfg);
	RStrBuf *filtered = r_esil_dfg_filter (dfg, "eax");
